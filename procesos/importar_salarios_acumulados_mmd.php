...
$consulta_acumulados="SELECT * FROM `data_acumulados` "
        . "ORDER BY `data_acumulados`.`Cedula` ASC,`data_acumulados`.`Fecha_Pago` ASC,`data_acumulados`.`NUMERO_NOMINA` ASC";
...
{          
   $salario_bruto=$vacac=$xiii=$gtorep=$xiii_gtorep=$liquida=$bono=$otros_ing=$prima=$s_s=$s_e=$islr=$islr_gr=$acreedor_suma=$Neto=0.0;
    ...
    $query="SELECT * FROM salarios_acumulados WHERE cedula='$Cedula' AND fecha_pago='$Fecha_Pago'";	
