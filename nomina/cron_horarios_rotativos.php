...
$empresa = new bd(SELECTRA_CONF_PYME); // planillaexpress_conf

$query   = "SELECT e.*, de.nombre_sistema 
            FROM   nomempresa e 
            LEFT JOIN datos_empresa de ON de.cod_empresa = e.codigo 
            WHERE e.bd <> '' ";

$result  = $empresa->query($query);

while ( $row = $empresa->fetch($result) )
{
    $codigo_empresa = $row['codigo'];
    $nombre_empresa = $row['nombre'];
    $bd_nomina      = $row['bd_nomina'];

	try {

		    $conexion = new bd($bd_nomina); 
			...
			while( $fila = $conexion->fetch($res) ) 
			{
				...
				if($frecuencia == 'Semanal')
				{
					...
					if($dia_semana == $inicio)
					{
						...
						while( $fila2 = $conexion->fetch($res2) )
						{
							...
							while($fila3 = $conexion->fetch($res3) )
							{
							...
							$sql3 = "SELECT ficha FROM nompersonal WHERE turno_id=".$turno_actual." AND ficha NOT IN ('".$excluir_fichas."')";
							...
							while( $fila3 = $conexion->fetch($res3) )
							{
								...
								if ( $conexion->query($sql4) === TRUE ) 
								{
										...
										while($i<7)
										{
											...
											if( ($res4->num_rows > 0) && ($fila4 = $conexion->fetch($res4)) )
											{
			...
			/*
			if($success)
				echo "<br>Rotaciones de la empresa <b>$nombre_empresa</b> realizadas exitosamente.<br><br>";
			else
				echo "<br>Error al realizar las rotaciones de la empresa <b>$nombre_empresa</b><br><br>";
			*/
	}catch (Exception $e ) {
	    // echo "<br>Service unavailable<br>";
	}
}
