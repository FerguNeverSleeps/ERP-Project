...
App.collections.instancias.otros_egresos = new App.collections.otros_egresos();
App.models.instancias.otros_egresos = new App.models.otros_egresos();
...
App.collections.instancias.otros_egresos.on("add", function(currentModel, currentCollection){

	var ViewOtrosEgresos = new App.views.OtrosEgresos({ 
								model: currentModel, 
								collection: currentCollection 
							});

	$("#tabla_montos").find("tbody").append(ViewOtrosEgresos.render().el);
	
});

App.collections.instancias.otros_egresos.on("remove", function(currentModel, currentCollection){
	var sumTotal = 0;
...
App.eventos.guardar = function() {
		...
		if( App.variablesGlobales.currentPosicionModelosFormaPago == App.collections.instancias.forma_pago.length){
			$("input[name='collection_movimientos_principales_']").val(JSON.stringify(App.collections.instancias.otros_egresos.models));
...
$(function(){
	$("#btnBuscarTerceros").click(function(ev){
        ev.preventDefault();

        var fnCallBack = function(registro){
        	$("input[name='id_caja_egreso_persona_terceros']").val(registro.json.id);
        	$("input[name='id_proveedor']").val(registro.json.id);
		    $("input[name='nombre']").val(registro.json.descripcion);
	        $("input[name='codigo']").val(registro.json.codigo);
	        $("select[name='tipo_documento']").val(registro.json.id_tipo_documento);
	        $("select[name='tipo_pago']").val(registro.json.id_caja_egre_tp_pago);

	        App.collections.instancias.otros_egresos.add(App.models.instancias.otros_egresos);
	  // 		App.collections.instancias.otros_egresos.fetch({
			// 	data:{
	...
	$("#guardar").click(function(ev){
    	...
    	if (App.collections.instancias.otros_egresos.length>0) {
    		validador=true;
    		App.collections.instancias.otros_egresos.each(function(model){
    		...
    		if(validador){
	    		App.collections.instancias.forma_pago.fetch({
					data:{
						opt:"TiposFormaPagoOtrosEgresos"
					}
				});
    		}
    	}
