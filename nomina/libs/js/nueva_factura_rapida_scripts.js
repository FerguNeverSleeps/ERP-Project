...
$(document).ready(function(){
    ...
    **/
    ...
    $.addTabla = function(codigo, descripcion, cantidad, preciosiniva, descuento, montodescuento, totalsiniva, iva, piva, totalconiva, almacen, producto, referencia_d, unidad_empaque,cantidad_bulto){
        //console.log("asdadsad",preciosiniva)
        ContarInputItem += 1;
        //alert("ASDASDS")
        ...
        var cantidad_bulto = cantidad_bulto;//parseFloat(data.productoSeleccionado.cantidad_bulto);
        try{
            cantidad_bulto = parseFloat(data.productoSeleccionado.cantidad_bulto);
        }catch(e){
            cantidad_bulto = parseFloat(cantidad_bulto);
        }
        ...
        var es_producto_detal = (_.isUndefined(data.productoSeleccionado.es_producto_detal)) ? 'no': 'si'
        ...
        campos += $.inputHidden("_es_producto_detal", es_producto_detal ,"[]");
        campos += $.inputHidden("_item_lista_precio",$("select[name='lista_preciod'] :selected").val(),"[]",codigo);
        if(es_producto_detal=='si'){
            campos += $.inputHidden("_filtro_producto_feinicio", data.productoSeleccionado.fecha_inicio ,"[]");
            campos += $.inputHidden("_filtro_producto_fefin", data.productoSeleccionado.fecha_fin ,"[]");
        }
        ...
        if (data.productoSeleccionado.cod_item_forma == "1") { // Si es un Producto
            ...
            var fnCallBack = function(response){
                    ...
                    //html += "<td style='text-align: left;' class=\"filter-column\" style=\"width:auto;\">"+codigo+"</td>";
            ...
            if(es_producto_detal=='si'){
                // Si es un producto detal no necesita ser precomprometido.
                fnCallBack("");
            }else{
                $.ajax({
                    type: "GET",
            ...
            html += "<td><img src=\"../../imagenes/"+foto+"\" width=\"50\" align=\"absmiddle\" height=\"50\"/></td>";
            html += "<td title=\"Haga click aqu&iacute; para ver detalles\" class=\"info_detalle\" style=\"cursor:pointer;background-color:#507e95;color:white;\"><a class=\"codigo\" rel=\"facebox\" style=\"color:white; text-align: center;\" href=\"#info\">"+producto+"</a><input type='hidden' name='idItem_' value='"+codigo+"'/></td>";
            html += "<td style='text-align: left;' class=\"filter-column\" style=\"width:auto;\">"+referencia_d+"</td>";
            html += "<td style='text-align: left;' class=\"filter-column\" style=\"width:auto;\">"+descripcion+"</td>";
            
            html += "<td style='text-align: left;' class=\"filter-column\" style=\"width:auto;\">"+unidad_empaque+"</td>";
    ...
    $(document).delegate("#seleccionarPedido", "click",function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $("#observacion_formato_salida").val(salida.observacion_salida);
    ...
    $("#seleccionar_todos_factura_detal").click(function(){
      var checked = $(this).is(":checked");
      $("input[name='factura_detal']").prop('checked', checked);
    });

    $(".btnInsertarFacturasDetal").click(function(e){
        e.preventDefault();
        id_facturas_detals = [];
        $("input[name='factura_detal']").each(function(i,e){  
            var id_factura_detal = $(this).val();
            var checked = $(this).is(":checked");
            if (checked) {
                id_facturas_detals.push(id_factura_detal);
            }
        });
        $.ajax({
            type: "GET",
            url:  "../../libs/php/ajax/ajax.php",
            data: "opt=seleccionarFacturaDetalPendiente&id_facturas_detals="+id_facturas_detals,
            success: function(response){
                reg = JSON.parse(response);
                _.each(reg.productos, function(producto){
                    iva=(producto._item_piva!=0)&&(producto._item_piva!='')?parseFloat(producto._item_piva):0;
                    $.addTabla(producto.id_item,producto._item_descripcion,producto._item_cantidad,producto._item_preciosiniva,producto._item_descuento,producto._item_montodescuento,producto._item_totalsiniva,iva,producto._item_preciosiniva*producto._item_cantidad*iva/100,parseFloat(producto._item_totalconiva),producto._item_almacen,producto.cod_item);  
                     $(".listado_factura_detal").find("tr.factura_detal_id_"+producto.id_factura_detal).remove();
                });
            }
        });
    });

    $(document).delegate("#seleccionarFacturaDetal", "click",function(){
        if(confirm('Incluir Factura Detal') == false){
            return false;
        }
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                while(resultado.length)
                {
                    ...
                    $.addTabla(resultado[i].id_item,resultado[i]._item_descripcion,resultado[i]._item_cantidad,resultado[i]._item_preciosiniva,resultado[i]._item_descuento,resultado[i]._item_montodescuento,resultado[i]._item_totalsiniva,iva,resultado[i]._item_preciosiniva*resultado[i]._item_cantidad*iva/100,parseFloat(resultado[i]._item_totalconiva),resultado[i]._item_almacen,resultado[i].cod_item,resultado[i].referencia,resultado[i].unidad_empaque);
                    i=i+1;
                }
            //fn_cantidad();
            }
    ...
    $(document).delegate("#seleccionarCotizacion", "click",function(){
        ...
        $.ajax({
            type: "GET",
            url:  "../../libs/php/ajax/ajax.php",
            data: "opt=seleccionarCotizacionPendiente&id_cotizacion="+codigo+"&almacen=1",//+(cod_almacen=="")?1:cod_almacen
            success: function(data){
                resultado=eval(data);
                i=0;
                var iva = 0;
                ...
                {
                    if(i == 0){
                        $("#observacion_formato_salida").val(resultado[i].observacion);
                        $("#termino_pago_id").val(resultado[i].termino_pago_id);
                    }
                    //iva=(resultado[i]._item_piva!=0)&&(resultado[i]._item_piva!='')?parseFloat(resultado[i]._item_piva):0;
                    /*
                        SE REALIZA CAMBIO DE DIVISA  
                    */
                    iva=parseFloat(resultado[i].iva);
                    MONTO_TASA = resultado[i].tasa_cambio;
                    //console.log(resultado[i]._item_preciosiniva)
                    resultado[i]._item_preciosiniva = resultado[i]._item_preciosiniva / MONTO_TASA;
                    //console.log(resultado[i]._item_preciosiniva)
                    resultado[i]._item_montodescuento = resultado[i]._item_montodescuento / MONTO_TASA;
                    resultado[i]._item_totalsiniva = resultado[i]._item_totalsiniva / MONTO_TASA;
                    ...
                    //resultado[i]._item_preciosiniva*resultado[i]._item_cantidad*iva/100;
                    //parseFloat(resultado[i]._item_totalconiva / MONTO_TASA);
                    ...
                    if(agregarItemCotizado == 1){
                        ...
                        iva,
                        ((resultado[i]._item_preciosiniva*resultado[i]._item_cantidad*iva/100)/MONTO_TASA),
                        parseFloat(resultado[i]._item_totalconiva/MONTO_TASA),
    ...
    **/
                ...
                callback: function(records, operation, success) { 
                    if (success) {
                        if(records.length > 1){
                            ...
                            /*console.log(data.productoSeleccionado)
    ...
    $("#lista_precio").change(function(){
        ...
        var jqThis = $(this);
        //alert(jqThis.data("lista_precio_prev"));
        
        //jqThis.data("lista_precio_prev",jqThis.val());

        //console.log(jqThis.data("lista_precio_prev"))
        //console.log($("#lista_precio").val())
        ...
        if (Util.verificaNivelOperacion(NIVEL_USUARIO,NIVEL_CAMBIO_LISTA)) {
            ...
            //alert("no autonizado")
            //aux = jqThis.data("lista_precio_prev").split(",");
            //aux2 = aux[0].split("[");
            //console.log(aux2,parseInt(aux2[1]))
            //aux3 = aux2[1].replace("\"", "");
            //aux3 = aux3.replace("\"", "");
            //console.log(aux3)
            //$('option[value='+jqThis.data("lista_precio_prev") +']', this).attr("selected",true);      
            Util.solicitaAutorizacion(NIVEL_CAMBIO_LISTA,function(){
                $.verificarBloqueoCampoPrecio();
                if(!_.isUndefined(data.productoSeleccionado.id_item)){
                    var preciosiniva = getTipoPrecio();
    ...
    $("input[name='copias']").numeric();
    //comentada la siguiente linea para que se lecciione segun la lista de precio definida al cliente
    //$("#lista_precio").val('["8","LIBRE"]');
});