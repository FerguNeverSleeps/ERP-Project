...
App.collections.instancias.otros_ingresos = new App.collections.otros_ingresos();
App.models.instancias.otros_ingresos = new App.models.otros_ingresos();
...
App.collections.instancias.otros_ingresos.on("add", function(currentModel, currentCollection){

	var ViewOtrosIngresos = new App.views.OtrosIngresos({ 
								model: currentModel, 
								collection: currentCollection 
							});

	$("#tabla_montos").find("tbody").append(ViewOtrosIngresos.render().el);
	
});

App.collections.instancias.otros_ingresos.on("remove", function(currentModel, currentCollection){
	var sumTotal = 0;
...
App.eventos.guardar = function() {
		...
		if( App.variablesGlobales.currentPosicionModelosFormaPago == App.collections.instancias.forma_pago.length){
			...
			$("input[name='otros_ingresos_']").val(JSON.stringify(App.collections.instancias.otros_ingresos.models));
...
$(function(){
	...
	$("#guardar").click(function(ev){
    	...
    	if (App.collections.instancias.otros_ingresos.length>0) {
    		validador=true;
    		App.collections.instancias.otros_ingresos.each(function(model){
    		...
    		if(validador){
	    		App.collections.instancias.forma_pago.fetch({
					data:{
						opt:"TiposFormaPagoOtrosIngresos"
					}
				});
    		}
    	}
