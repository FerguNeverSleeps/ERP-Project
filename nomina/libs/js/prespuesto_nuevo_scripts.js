...
$(function(){
    ...
    **/
    ...
    function addTabla(codigo, descripcion, cantidad, preciosiniva, descuento, montodescuento, totalsiniva, iva, piva, totalconiva, almacen, producto, referencia_d,cantidad_bulto_d,unidad_empaque_d,costo_actual_d,cubi4_d,cubi5_d,kilos_bulto_d,id_detalle_cotizacion_sel){
        ContarInputItem += 1;
        //console.log(id_detalle_cotizacion_sel)
        ...
        try{
            ...
            unidad_empaque_descripcion =unidad_empaque_d;//
        }
        ...
        var id_cotizacion_presupuesto_detalle = 0;
        /*if(typeof data.productoSeleccionado.id_cotizacion_presupuesto_detalle == undefined  || typeof data.productoSeleccionado.id_cotizacion_presupuesto_detalle == "undefined" ||  data.productoSeleccionado.id_cotizacion_presupuesto_detalle == undefined  ){
            id_cotizacion_presupuesto_detalle = 0;
        }
        else{
            id_cotizacion_presupuesto_detalle = id_detalle_cotizacion_sel;
        }*/
        if(typeof id_detalle_cotizacion_sel == undefined  || typeof id_detalle_cotizacion_sel == "undefined" ||  id_detalle_cotizacion_sel == undefined  ){
            id_cotizacion_presupuesto_detalle = 0;
        }
        else{
            id_cotizacion_presupuesto_detalle = id_detalle_cotizacion_sel;
        }
        //console.log("af",data.productoSeleccionado.id_cotizacion_presupuesto_detalle,id_cotizacion_presupuesto_detalle);
        ...
        campos += $.inputHidden("_id_cotizacion_presupuesto_detalle",id_cotizacion_presupuesto_detalle,"[]");
        ...
        campos += $.inputHidden("_item_cantidad",cantidad,"[]",codigo);
        campos += $.inputHidden("_item_preciosiniva",parseFloat(preciosiniva).toFixed(2),"[]",codigo);
        campos += $.inputHidden("_item_descuento",descuento_,"[]",codigo);
        campos += $.inputHidden("_item_montodescuento",montodescuento,"[]",codigo);
        campos += $.inputHidden("_item_totalsiniva",totalsiniva,"[]",codigo);
        campos += $.inputHidden("_item_piva",parseFloat(iva).toFixed(2),"[]",codigo);
        campos += $.inputHidden("_item_iva",piva,"[]",codigo);
        campos += $.inputHidden("_item_totalconiva",totalconiva.toFixed(2),"[]",codigo);
        ...
        campos += $.inputHidden("_id_cantidad_bulto",data.productoSeleccionado.cantidad_bulto,"[]",codigo);/*Adicionado como requisito especifico del CCP*/
        campos += $.inputHidden("_id_costo_actual",costo_actual,"[]",codigo);

        campos += $.inputHidden("_cantidad_bulto",cantidad_bultos_totales,"[]",codigo); // Cantidad por bulto.
        campos += $.inputHidden("_cantidad_bulto_kilos",cantidad_bulto_kilos,"[]",codigo); // Cantidad por bulto.
        campos += $.inputHidden("_unidad_empaque",unidad_empaque_descripcion,"[]",codigo);
        campos += $.inputHidden("_ganancia_item_individual",ganancia_item_individual,"[]",codigo);
        campos += $.inputHidden("_porcentaje_ganancia",porcentaje_ganancia,"[]",codigo);
        campos += $.inputHidden("_referencia",referencia,"[]",codigo);
        ...
        if (data.productoSeleccionado.cod_item_forma == "1") { // Si es un Producto
            $.ajax({
                ...
                complete: function(xhr){
                    ...
                    html += "<td style='text-align: center;'><img style=\"cursor: pointer;\" class=\"editar\" alt=\""+codigo+"\"  title=\"Editar Item\" src=\"../../libs/imagenes/edit.gif\">&nbsp;<img style=\"cursor: pointer;\" alt=\""+id_cotizacion_presupuesto_detalle+"\" class=\"eliminar\"  title=\"Eliminar Item\" src=\"../../libs/imagenes/delete.png\">"+campos+"</td>";
                    html += "</tr>";
    ...
    $(document).delegate("#seleccionarPedido", "click",function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                _.each(resultado, function(res, index){
                    iva = parseFloat(res._item_piva);
                    data.productoSeleccionado = res;
                    addTabla(res.id_item,res._item_descripcion,res._item_cantidad,res._item_preciosiniva,res._item_descuento,res._item_montodescuento,res._item_totalsiniva,iva,res._item_preciosiniva*res._item_cantidad*iva/100,parseFloat(res._item_totalconiva),res._item_almacen,res.cod_item);
                });
    ...
    $(document).delegate("#seleccionarCotizacion", "click",function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                i=0;
                $("#cotizacion_cargada").val("si");
                $("#id_cotizacion_cargada").val(codigo);
                while(resultado1.length)
                {   
                    if(i == 0){
                        ///$("#observacion_salida").val(resultado1[i].observacion);
                        $('#pago_'+resultado1[i].formapago).click();
                        var miselect=$("#termino_pago_id");
                        miselect.find('option').remove().end().append('').val('');

                        $.getJSON("../../../includes/clases/controladores/ControladorApp.php?accion=Accion/CajaFormaPago/get_list_termino_pago_combo&tipo="+resultado1[i].formapago+"&db_connect="+DB_CONNECT,function(data){
                            $.each(data, function(k,v){
                                $("#termino_pago_id").append("<option value=\""+v.termino_pago_id+"\">"+v.descripcion+"</option>");
                            });
                    ...
                    iva = parseFloat(resultado1[i]._item_piva);
                    MONTO_TASA = resultado1[i].tasa_cambio;
                    resultado1[i]._item_preciosiniva = resultado1[i]._item_preciosiniva / 1;
                    resultado1[i]._item_montodescuento = resultado1[i]._item_montodescuento / 1;
                    resultado1[i]._item_totalsiniva = resultado1[i]._item_totalsiniva / 1;
                    ...
                    addTabla(resultado1[i].id_item,
                        ...
                        iva,
                        ((resultado1[i]._item_preciosiniva*resultado1[i]._item_cantidad*iva/100)/1),
                        parseFloat(resultado1[i]._item_totalconiva/1),
                        ...
                        resultado1[i].id_cotizacion_presupuesto_detalle);//id_detalle_pedido
                    /*iva=(resultado[i]._item_piva!=0)&&(resultado[i]._item_piva!='')?parseFloat(resultado[i]._item_piva):0;
                    addTabla(resultado[i].id_item,resultado[i]._item_descripcion,resultado[i]._item_cantidad,resultado[i]._item_preciosiniva,resultado[i]._item_descuento,resultado[i]._item_montodescuento,resultado[i]._item_totalsiniva,iva,resultado[i]._item_preciosiniva*resultado[i]._item_cantidad*iva/100,parseFloat(resultado[i]._item_totalconiva),resultado[i]._item_almacen,resultado[i].cod_item);
                    i=i+1;*/
                    i=i+1;
                ...
                callback: function(records, operation, success) { 
                    if (success) {
                        if(records.length > 1){
                            ...
                            $("#PRECIO").html(data.productoSeleccionado.tprecio);
                            $.verificarBloqueoCampoPrecio();

                            var preciosiniva = getTipoPrecio() * MONTO_TASA;
