App.views.OtrosEgresos = Backbone.View.extend({
	tagName: "tr",
	...
	agregarRenglon: function(ev) {
		ev.preventDefault();
		App.collections.instancias.otros_egresos.add(new App.models.otros_egresos());
	},
		...
		$.ajax({
				...
				complete: function(xhr, status){
					...
					if(!_.str.isBlank(response)) {
						...
						this__.model.set("numero", reg_ultimo_cheque_activo[0].cheque);
						this__.model.set("cod_cheque", reg_ultimo_cheque_activo[0].cod_cheque);
						this__.model.set("cod_chequera", reg_ultimo_cheque_activo[0].cod_chequera);
						this__.model.set("a_orden_de", $("#nombre").val());
				...
				$.ajax({
					...
					complete: function(xhr, status){
						...
						if(!_.str.isBlank(response)){
							...
							modelCurrent.set("a_orden_de", $("#nombre").val());
	         ...
	         url:  "../../libs/php/ajax/ajax.php",
				...
				complete: function(xhr, status){
					...
					if(!_.str.isBlank(response)){
						...
						App.models.instancias.banco.set("a_orden_de", $("#nombre").val());
