...
$(function(){
    ...
    function fn_cantidad(){
        ...
        $(".grid table.lista tbody").find("tr").each(function(){
            ...
            //console.log("TOTALES: ",var_montoItemsFactura,var_ivaTotalFactura)
    ...
    **/
    $.inputHidden = function(Input,Value,ID, escapar){
        ...
        if(escapar==false){
            return '<input type="hidden" name="'+Input+''+ID+'" id="'+Input+'-'+escapar+'" value="'+Value+'">';
        }else{
            return "<input type='hidden' name='"+Input+""+ID+"' id='"+Input+"-"+escapar+"' value='"+Value+"'>";
        }
    ...
    $.agregarArticulo = function(){
        ...
        //console.log(totalsiniva,preciosiniva,cantidad,montodescuento)
    ...
    **/
    ...
    function addTabla(codigo, descripcion, cantidad, preciosiniva, descuento, montodescuento, totalsiniva, iva, piva, totalconiva, almacen, producto, referencia_d,cantidad_bulto_d,unidad_empaque,costo_actual_d,cubi4_d,cubi5_d,kilos_bulto_d,id_detalle_pedido_sel){/*Adicionado como requisito especifico del CCP*/
        //console.log(codigo, descripcion, cantidad, preciosiniva, descuento, montodescuento, totalsiniva, iva, piva, totalconiva, almacen, producto, referencia_d,cantidad_bulto,unidad_empaque)
        ...
        var cantidad_bulto = 0;//cantidad_bulto;//parseFloat(data.productoSeleccionado.cantidad_bulto);
        try{
            cantidad_bulto = parseFloat(data.productoSeleccionado.cantidad_bulto);
        }catch(e){
            cantidad_bulto = parseFloat(cantidad_bulto_d);
        }
        if(typeof data.productoSeleccionado.cantidad_bulto == undefined  || typeof data.productoSeleccionado.cantidad_bulto == "undefined" ||  data.productoSeleccionado.cantidad_bulto == undefined  ){
            cantidad_bulto = parseFloat(cantidad_bulto_d);
        }
        var unidad_empaque_descripcion =unidad_empaque;//data.productoSeleccionado.unidad_empaque || "";
        try{
            unidad_empaque_descripcion =data.productoSeleccionado.unidad_empaque || "";
        }catch(e){
            unidad_empaque_descripcion =unidad_empaque;//
        }
        if(typeof data.productoSeleccionado.unidad_empaque == undefined  || typeof data.productoSeleccionado.unidad_empaque == "undefined" ||  data.productoSeleccionado.unidad_empaque == undefined  ){
            
            unidad_empaque_descripcion =unidad_empaque;//
        }
        //console.log(cantidad_bulto)
       ...
       var id_detalle_pedido = 0;
       if(typeof id_detalle_pedido_sel == undefined  || typeof id_detalle_pedido_sel == "undefined" ||  id_detalle_pedido_sel){
        
        }
        else{
            id_detalle_pedido = id_detalle_pedido_sel;
        }

        var cantidad_bultos_totales = 0;
        if(TIPO_CANTIDAD_PEDIDA == "Pieza"){
            cantidad_bultos_totales = reglaCalculoPorBulto({
                                        "cantidad":       cantidad,
                                        "cantidad_bulto": cantidad_bulto
            });
        }
        else if(TIPO_CANTIDAD_PEDIDA == "Bulto"){
            cantidad_bultos_totales = $("input[name='filtro_bultos_total']").val();
            if($("input[name='filtro_bulto']").val() != "0.00"){
                cantidad = $("input[name='filtro_bulto']").val() * cantidad_bultos_totales;
                $("input[name='filtro_cantidad']").val(cantidad);    
            }            
        }
        ...
        var costo_actual = 0;
        //console.log(costo_actual,costo_actual_d)
        if(typeof data.productoSeleccionado.costo_actual == undefined  || typeof data.productoSeleccionado.costo_actual == "undefined" ||  data.productoSeleccionado.costo_actual == undefined  ){
            
            costo_actual = parseFloat(costo_actual_d);
        }
        else{
            costo_actual = parseFloat(data.productoSeleccionado.costo_actual);
        }


        //var preciosiniva = getTipoPrecio();
        //console.log(preciosiniva,costo_actual,cantidad,montodescuento)
        var ganancia_item_individual = ((preciosiniva-costo_actual)*cantidad)-montodescuento;
        //console.log(ganancia_item_individual)
        var porcentaje_ganancia = ganancia_item_individual*100/ ((preciosiniva*cantidad)-montodescuento);
        //console.log(porcentaje_ganancia)
        var cantidad_bulto_kilos = data.productoSeleccionado.kilos_bulto;
        if(typeof data.productoSeleccionado.kilos_bulto == undefined  || typeof data.productoSeleccionado.kilos_bulto == "undefined" ||  data.productoSeleccionado.kilos_bulto == undefined  ){
            cantidad_bulto_kilos = kilos_bulto_d;//"S/I";
        }
        ...
        try{
            ...
            //console.log("B");
            referencia = referencia_d;//"S/I";
        }
        if(typeof data.productoSeleccionado.referencia == undefined  || typeof data.productoSeleccionado.referencia == "undefined" ||  data.productoSeleccionado.referencia == undefined  ){
            referencia = referencia_d;//"S/I";
        }

        //console.log(referencia)
        //totalsiniva = totalsiniva;// - montodescuento;

        campos += $.inputHidden("_cod_item_precompromiso",ContarInputItem,"[]");
        campos += $.inputHidden("_id_detalle_pedido",id_detalle_pedido,"[]");
        ...
        campos += $.inputHidden("_item_totalconiva",totalconiva,"[]");//AQUI
        ...
        campos += $.inputHidden("_id_cantidad_bulto",cantidad_bulto,"[]");/* data.productoSeleccionado.cantidad_bulto Adicionado como requisito especifico del CCP*/
        ...
        if(typeof data.productoSeleccionado.cubi4 == undefined  || typeof data.productoSeleccionado.cubi4 == "undefined" ||  data.productoSeleccionado.cubi4 == undefined  ){
            totalm3 = cubi4_d;//"S/I";
        }
        var totalft3 = data.productoSeleccionado.cubi5;
        if(typeof data.productoSeleccionado.cubi5 == undefined  || typeof data.productoSeleccionado.cubi5 == "undefined" ||  data.productoSeleccionado.cubi5 == undefined  ){
            totalft3 = cubi5_d;//"S/I";
        }
        var peso_total_item = parseFloat(cantidad)/parseFloat(cantidad_bulto) * parseFloat(cantidad_bulto_kilos);//data.productoSeleccionado.cantidad_bulto*data.productoSeleccionado.kilos_bulto
        ...
        if (data.productoSeleccionado.cod_item_forma == "1") { // Si es un Producto
            $.ajax({
                ...
                complete: function(xhr){
                    ...
                    if($("select[name='validar_stock'] :selected").val()=="si" && data.productoSeleccionado.posee_talla_color == "no") {
                        if(result[0].id=="-99") {
                    ...
                    var styleDescuento = "";
                    if(MUESTRA_DESCUENTO == "no"){
                        styleDescuento = "display:none;";
                    }
                    else{
                        styleDescuento = "";
                    } 
                    ...
                    html += "<td style='text-align: right; padding-right: 20px;"+styleDescuento+"' class='montodescuento' id='td_monto_desc_"+codigo+"'>"+parseFloat(montodescuento).toFixed(3)+"</td>";
                    html += "<td style='text-align: right; padding-right: 20px;"+styleDescuento+"' id='td_porc_desc_"+codigo+"'>"+parseFloat(descuento_).toFixed(3)+"</td>";
                    ...
                    html += "<td style='text-align: center;'><img style=\"cursor: pointer;\" class=\"editar\" alt=\""+codigo+"\"  title=\"Editar Item\" src=\"../../libs/imagenes/edit.gif\">&nbsp;<img style=\"cursor: pointer;\" alt=\""+id_detalle_pedido+"\" class=\"eliminar\"  title=\"Eliminar Item\" src=\"../../libs/imagenes/delete.png\">"+campos+"</td>";
                    html += "</tr>";
     ...
     **/
     ...
     function validaNumerosEditAutorizacionDescuento(event){
                if(AUTORIZACION_CAMBIO_DESCUENTO == 0){
                    if (Util.verificaNivelOperacion(NIVEL_USUARIO,NIVEL_CAMBIO_DESCUENTO)) {
                        //alert("autonizado")
                        AUTORIZACION_CAMBIO_DESCUENTO = 1;
                        
                    }else{
                        Util.solicitaAutorizacion(NIVEL_CAMBIO_DESCUENTO,function(){
                            AUTORIZACION_CAMBIO_DESCUENTO = 1;
                        },function(){
                            AUTORIZACION_CAMBIO_DESCUENTO = 0;     
                        });                                 
                    }
                }
                if ((event.keyCode < 48 || event.keyCode > 57) && event.keyCode != 8 && event.keyCode != 190 ) {
     ...
     function validaNumerosEditAutorizacionDescuento2(event){
                if (event.keyCode != 13 ) {
    ...
    $(document).delegate("#seleccionarPedido", "click",function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                var salida = resultado.formato_salida;
                $("#pedido_cargado").val("si");
                $("#id_pedido_cargado").val(codigo);
                _.each(resultado.productos, function(res, index){
                    iva = parseFloat(res._item_piva);
                    data.productoSeleccionado = res;
                    addTabla(
                        res.id_item,
                        res._item_descripcion,
                        res._item_cantidad,
                        res._item_preciosiniva,
                        res._item_descuento,
                        res._item_montodescuento,
                        res._item_totalsiniva,
                        iva,
                        res._item_preciosiniva*res._item_cantidad*iva/100,
                        parseFloat(res._item_totalconiva),
                        res._item_almacen,
                        res.cod_item,
                        res.referencia_item,
                        res.cantidad_bulto,
                        ...
                        res.cubi5,
                        res.kilos_bulto,
                        res.id_detalle_pedido);//,res.referencia,res.unidad_empaque,res.cantidad_bulto
                });
                //console.log($("#transpaso_salida"))
                //$("#transpaso_salida").removeClass( "input-gastos" );

                $("#transpaso_salida").val(gastos.transporte_salida);
               // $("#transpaso_salida").addClass( "input-gastos" );
                $("#transporte").val(gastos.transporte);
                //$("#transporte").addClass( "input-gastos" );
                $("#empaques").val(gastos.empaques);
                //$("#empaques").addClass( "input-gastos" );
                $("#seguro").val(gastos.seguro);
                //$( "p" ).addClass( "myClass yourClass" );
                $("#flete").val(gastos.flete);
               // $( "p" ).addClass( "myClass yourClass" );
                $("#comisiones").val(gastos.comisiones);
                //$( "p" ).addClass( "myClass yourClass" );
                $("#otros").val(gastos.otros);
                //$( "p" ).addClass( "myClass yourClass" );
                $("#manejo").val(gastos.manejo);
                //$( "p" ).addClass( "myClass yourClass" );

                //$("input.input-gastos").numeric();
                //console.log($("#transpaso_salida"))
                ///$("#total_fob_gatos").val(gastos.total_fob_gasto);
                //fn_cantidad();

                //console.log(salida)
                $("#forma_salida_tipo").val(salida.tipo_salida);
                $("input[name='forma_salida_cliente']").attr("value",salida.nombre_cliente_salida);
                $("input[name='forma_salida_id_cliente']").attr("value",salida.cliente_salida);
                $("#forma_salida_via").val(salida.via_salida);
                $("input[name='forma_salida_marca']").attr("value",salida.marca_salida);
                $("#observacion_salida").val(salida.observacion_salida);
                $("#termino_pago_id").val(salida.termino_pago_id);
    ...
    $(document).delegate("#seleccionarCotizacion", "click",function(){
        ...
        $.ajax({
            type: "GET",
            url:  "../../libs/php/ajax/ajax.php",
            data: "opt=seleccionarCotizacionPendiente&id_cotizacion="+codigo+"&almacen="+cod_almacen,
            success: function(data){
                resultado1=eval(data); 
                i=0; 
                var resultaVerificacion = "";
                var agregarItemCotizado = 1;
                while(resultado1.length)
                {
                    if(i == 0){
                        $("#observacion_salida").val(resultado1[i].observacion);
                        $("#termino_pago_id").val((resultado1[i].termino_pago_id==null)?1:resultado1[i].termino_pago_id);
                    }
                  //  iva=(parseFloat(resultado1[i]._item_piva!=0))&&(resultado1[i]._item_piva!='')?parseFloat(resultado1[i]._item_piva):0;
                   
 iva = resultado1[i]._item_piva;
 
                    MONTO_TASA = resultado1[i].tasa_cambio;
                    resultado1[i]._item_preciosiniva = resultado1[i]._item_preciosiniva / MONTO_TASA;
                    resultado1[i]._item_montodescuento = resultado1[i]._item_montodescuento / MONTO_TASA;
                    resultado1[i]._item_totalsiniva = resultado1[i]._item_totalsiniva / MONTO_TASA;
                    ...
                    if(parseInt(resultado1[i]._item_cantidad) > parseInt(resultado1[i]._item_cantidad_existencia)){
                        resultaVerificacion = resultado1[i].referencia_item;
                        agregarItemCotizado = 0;
                        alert("No se agregara el producto "+resultaVerificacion+ " por  falta de existencia");                        
                    }
                    ///console.log("FIN-"+i,resultaVerificacion)
                    if(agregarItemCotizado == 1){
                   

                        addTabla(resultado1[i].id_item,
                        resultado1[i]._item_descripcion,
                        resultado1[i]._item_cantidad,
                        resultado1[i]._item_preciosiniva,
                        resultado1[i]._item_descuento,
                        resultado1[i]._item_montodescuento,
                        resultado1[i]._item_totalsiniva,
                        iva,
                        ((resultado1[i]._item_preciosiniva*resultado1[i]._item_cantidad*resultado1[i]._item_piva/100)/MONTO_TASA),
                        parseFloat(resultado1[i]._item_totalconiva/MONTO_TASA),
                        resultado1[i]._item_almacen,
                        resultado1[i].cod_item,
                        resultado1[i].referencia_item,
                        resultado1[i].cantidad_bulto,
                        resultado1[i].unidad_empaque,
                        resultado1[i].costo_actual,
                        resultado1[i].cubi4,
                        resultado1[i].cubi5,
                        resultado1[i].kilos_bulto,
                        0);//id_detalle_pedido
                    }
                    i=i+1;
    ...
    **/
    ...
    $.bloqueoFiltroHeader = function(bloquear) {
        if(bloquear) {
            ...
            $("select[name='cod_vendedor']").attr("readonly","readonly");
        } else {
            ...
            $("select[name='cod_vendedor']").removeAttr("readonly");
        }
    }
                    ...
                    if (success) {
                        ...
                        if(records.length > 1){
                            ...
                            //if(TIPO_CANTIDAD_PEDIDA == "Pieza"){
                                ...
                                //----$("input[name='filtro_bultos_total']").attr("readonly","readonly");
                            /*}
                            else if(TIPO_CANTIDAD_PEDIDA == "Bulto"){
                                $("input[name='filtro_bultos_total']").focus();
                                $("input[name='filtro_bultos_total']").select();
    ...
    //$('#filtro_descuento').keyup(function(e){ return validaNumerosEditAutorizacionDescuento2(e);  });
    $('#filtro_descuento').keydown(function(e){ return validaNumerosEditAutorizacionDescuento2(e);  });
    ...
    $.getPermisosDetalle = function(usuario){      
        $.ajax({
            type: "GET",
            url:  "../../libs/php/ajax/ajax.php",
            data: "opt=getPermisosDetalle&cod_permiso=2&cod_usuario="+usuario,
            success: function(data){          
                var resultado = eval(data);        
                permitido = parseInt(resultado[0].permitido);
                if(permitido == 1){
                    $("input[name='filtro_precio']").val(precioValue); 
                }else{                                          
                     $("input[name='filtro_precio']").val(precioIni); 
                }
                var preciosiniva = getTipoPrecio();
                preciosiniva = parseFloat($("input[name='filtro_precio']").val());
    ...
    $.verificarAcceso = function(usuario,clave){        
        $.ajax({
            type: "GET",
            url:  "../../libs/php/ajax/ajax.php",
            data: "opt=verificarAcceso&cod_usuario="+usuario+"&clave="+clave,
            success: function(data){          
                var resultado = eval(data); 
                if(resultado[0].cod_usuario){
                    $.getPermisosDetalle(resultado[0].cod_usuario);
                    return true;
                }else{  
    ...
    $.verificarExistenciaEnAlmacen = function(value){
        ...
        var cantidad_solicitada = parseInt($("input[name='filtro_cantidad']").val());
            ...
            //alert(data.productoSeleccionado.posee_talla_color)
            if (data.productoSeleccionado.posee_talla_color=="si") {
    ...
    $("input[name='filtro_bultos_total']").keypress(function(ev){
        var teclaTabMasP  = 13;
        var codeCurrent = ev.keyCode;
        var value = $(this).val() * $("input[name='filtro_bulto']").val();
        if(teclaTabMasP == codeCurrent) {
            var self = this;
            $.verificarExistenciaEnAlmacen(value);
        }
    ...
    $("input[name='filtro_bultos_total']").change(function(ev){//blur
        var value = $(this).val() * $("input[name='filtro_bulto']").val();
        if(value>0){
            TIPO_CANTIDAD_PEDIDA = "Bulto";
            $.verificarExistenciaEnAlmacen(value);
        }
    });

    $("input[name='filtro_cantidad']").change(function(ev){//blur
        var value = $(this).val();
        if(value>0){
            TIPO_CANTIDAD_PEDIDA = "Pieza";
            $.verificarExistenciaEnAlmacen(value);
    ...
    if(MUESTRA_DESCUENTO == "Si"){
        $("input[name='filtro_descuento']").keypress(function(ev){
            ...
            if(teclaTabMasP == codeCurrent){
                ...
                var preciosiniva = getTipoPrecio();
                preciosiniva = parseFloat($("input[name='filtro_precio']").val());
                //console.log(preciosiniva , PRECIO_MAXIMO , preciosiniva , PRECIO_MINIMO);
        ...
        $("input[name='filtro_precio']").keypress(function(ev){
            ...
            if(teclaTabMasP == codeCurrent){
                if(_.str.isBlank(value)){
                    $("input[name='filtro_precio']").val(0);
                }
    ...
    $("input[name='filtro_precio']").keypress(function(ev){
        ...
        codeCurrent = ev.keyCode;
        var value = $(this).val();       
        var permitido = 2;        
        var usuarios_win, clave_win;
        precioValue = $(this).val();
        
        if(clicked==true){
            return false;
        }
        if(codeCurrent == teclaTabMasP){
            var preciosiniva = getTipoPrecio();
                    preciosiniva = parseFloat($("input[name='filtro_precio']").val());
                    if(preciosiniva > PRECIO_MAXIMO || preciosiniva < PRECIO_MINIMO){
                        Util.solicitaAutorizacion(NIVEL_CAMBIO_LISTA,function(){
        ...
        /*if(!precioIni) precioIni = value;
        if(parseFloat(precioIni) > parseFloat(precioValue)){
            if(codeCurrent == teclaTabMasP){
                    $.ajax({
                        type: "GET",
                        url:  "../../libs/php/ajax/ajax.php",
                                          ...
                                          var win_user = new Ext.Window({
                                              ...
                                              buttons:[{
                                                  ...
                                                  handler: function(){                                   
                                                      ...
                                                      usuarios_win = $("#usuario_win").val();
                                                      clave_win = $("#clave_win").val();
                                                      if(usuarios_win && clave_win){

                                                              acceso =  $.verificarAcceso(usuarios_win, clave_win);

                                                               if(acceso){
                                                                  $.getPermisosDetalle(acceso);
                                                               }else{
                                                                   $.mensajeNotificacion({mensaje: "Usuario y/o contraseña inválido."});   
                                                                   win_user.close();
                                                               }

                                                  }else{
                                                         permitido = 2;
                                                         alert('Debe introducir un usuario y clave válidos');
                                                  }                                                    
                                              }
                                          }]
                                      });
