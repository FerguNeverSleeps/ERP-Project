...
class bd {
    ...
    public function query($sentencia, $charset='') {
        ...
        //$conexion->close(); // Con esta linea activa no se pueden obtener los errores si falla la consulta
        return $resultado;
    }
...
function buscar_palabra_exacta($modulo, $valor, $campo, $valor2, $campo2) {
    $consulta = "select * from " . $modulo . " where " . $campo . " LIKE '%" . $valor . "%' OR " . $campo2 . " LIKE '%" . $valor2 . "%' ";
        return $consulta;
}
...
function obtener_num_paginas($consulta, $LIMITE=75) {
    $conexion = conexion();
    $resultado = mysqli_query($conexion, $consulta) or die("no se pudo realizar la consulta " . mysqli_error($conexion));
...
function paginacion($pagina, $consulta,$LIMITE = 75) {
    ...
    //return $consulta2;
    $conexion = conexion();
    $resultado = mysqli_query($conexion, $consulta2) or die("no se pudo realizar la consulta");
    return $resultado;
}
...
{    
    ...
    $usuario  = DB_USUARIO;
    $clave    = DB_CLAVE;
    $conexion = mysqli_connect($host, $usuario, $clave, $_SESSION['bd']);
...
function boton2($cod_modulo, $nom_menu, $archivo,$img) {
    $ret =
            '<div class="box">
    <table width="100" height="90" border="0" cellpadding="0" cellspacing="0" style="cursor: pointer;" onclick="javascript:window.location=\'' . $archivo . '\'">
        <tr>
<td height="45" valign="bottom"><div align="center"><img width="52" height="52" src="' . $img . '" class="icon"/></div></td>
        </tr>
        <tr>
          <td height="45"><div align="center" class="boton-text">' . utf8_encode(nl2br($nom_menu)) . '</div></td>
        </tr>
...
function btn($tipo, $url, $accion=0, $caption="") { // Accion 0=location / 1=Form url = form name / 2 JS ** 3 reset **terminar**
    switch ($tipo) {
        ...
        case "generarCalendario":
            $icon = 'list';
            $name = 'Generar Calendario Personal';
        break;
...
function boton_metronic2($tipo, $url, $accion=0, $caption="",$id="") { // Accion 0=location / 1=Form url = form name / 2 JS ** 3 reset **terminar**
    switch ($tipo) {
;
...
;
    ...
    echo '<div '.$identificador.' class="btn btn-sm '.$color.' active" onClick="javascript:' . $js . '"><i class="fa fa-'.$icon.'"></i>&nbsp;' . $name . '</div>
    ';
}

function boton_metronic($tipo, $url, $accion=0, $caption="",$id="") { // Accion 0=location / 1=Form url = form name / 2 JS ** 3 reset **terminar**
    switch ($tipo) {
;
...
;
    ...
    echo '<div '.$identificador.' class="btn '.$color.' button-next" onClick="javascript:' . $js . '"><i class="fa fa-'.$icon.'"></i>&nbsp;' . $name . '</div>
    ';
}
...
function conexion_conf() 
{
    ...
    $usuario  = DB_USUARIO;
    $clave    = DB_CLAVE;
    $conexion = mysqli_connect($host, $usuario, $clave, DB_SELECTRA_CONF);
...
function conexion_contabilidad() 
{
    $host     = DB_HOST;
    $usuario  = DB_USUARIO;
    $clave    = DB_CLAVE;
    $conexion = mysqli_connect($host, $usuario, $clave, $_SESSION['bdc']);
...
{
    $resultado = mysqli_query($conexion, $consulta) or die("No se puede realizar la consulta  " . mysqli_error($conexion));
    return $resultado;
}
...
function pie_pagina($url, $pagina, $campos, $num_paginas) {
  ...
  <tbody>
    <tr>
      ...
      <td><input type="text" name="numero_pagina" onchange="javascript:paginacion(\'' . $url . '\',this.value,\'' . $campos . '\')" value="' . $pagina . '" size="4" style="color: black; text-align: center;"></td>
...
function pie_pagina_bootstrap($url, $pagina, $campos, $tpages) {
$adjacents  = 6;
/*
  echo "<ul class='pagination'>";
  
      echo "<li><a href=".$url."?pagina=1&". $campos ."><b>Primero</b></a></li>";
  ...
  {
    echo "<li><a href=".$url."?pagina=".($pagina - 1)."&". $campos ."><b>&laquo;</b></a></li>";
  }
  
  for($k=$paginas; $k <= $totpaginas; $k++)
  {
    if($pagina == $k)
    {
    ...
    {
      echo "<li><a href=".$url."?pagina=".$k."&". $campos ."'>".$k."</a></li>";
    }
  }
  ...
  {
    echo "<li><a href=".$url."?pagina=".($pagina + 1)."&". $campos ."><b>&raquo;</b></a></li>";
  }
    echo "<li><a href=".$url."?pagina=".$num_paginas."&". $campos ."><b>Último</b></a></li>";
...
*/
  ...
  $out = "<nav class='text-center'>";
    ...
    if($pagina==1) {
        ...
        $out.= "<li><span><a href=".$url."?pagina=1&". $campos .">$prevlabel</a></span></li>";
    }else {
        $out.= "<li><span><a href=".$url."?pagina=".($pagina - 1)."&". $campos .">$prevlabel</a></span></li>";

    }
    
    // first label
    if($pagina>($adjacents+1)) {
        $out.= "<li><a href=".$url."?pagina=1&". $campos .">1</a></li>";
    }
    // interval
    ...
    for($i=$pmin; $i<=$pmax; $i++) {
        if($i==$pagina) {
            ...
            $out.= "<li><a href=".$url."?pagina=1&". $campos .">$i</a></li>";
        }else {
            $out.= "<li><a href=".$url."?pagina=".($i)."&". $campos .">$i</a></li>";
        }
    }
    ...
    if($pagina<($tpages-$adjacents)) {
        $out.= "<li><a href=".$url."?pagina=".($tpages)."&". $campos .">$tpages</a></li>";
    }

    // next

    if($pagina<$tpages) {
        $out.= "<li><span><a href=".$url."?pagina=".($pagina+1)."&". $campos .">$nextlabel</a></span></li>";
    }else {
...
function icono($url, $titulo, $enlace) {
    echo "<td><a target=\"_SELF\" href=\"$url\"><IMG title=\"$titulo\" src=\"../imagenes/$enlace\" width=\"22\" height=\"22\" align=\"left\" border=\"0\"></a></td>
";
}
