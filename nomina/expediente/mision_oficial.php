...
<div id="mision">
    <div class="form-group">
        <label class="col-md-2 control-label" for="txtcodigo" id="mision_label">Tipo Misión: </label>
        <div class="col-md-7">                               

                <select name="tipo_mision" class="form-control" id="tipo_mision">
                    <option value="">Seleccione</option>
                    <?php                     
                       $consulta_mision="SELECT * FROM tipo_mision";
                       $resultado_mision=sql_ejecutar($consulta_mision);
                        while($fila_mision=fetch_array($resultado_mision))
                        {

                                if($fetch33['tipo_mision']==$fila_mision['id'])
                                {?>
                                     <option value="<?php echo $fila_mision['id']; ?>" selected><?php echo $fila_mision['descripcion']; ?></option>
                                <?}
                                else
                                {?>
                                     <option value="<?php echo $fila_mision['id']; ?>"><?php echo $fila_mision['descripcion']; ?></option>
                                <?}         
...
<div id="tiempos">
    <div class="form-group">   
        ...
        <div class="col-md-3">
            <input type="text" class="form-control"  name="dias" id="dias" onblur="validar_saldo_ajuste();calcular_duracion();" maxlength="5" <? if (isset($fetch33['dias'])){ echo "value='$fetch33[dias]'";} else{ echo "value=''";}?>/>                                
        </div>
    </div>
    <div class="form-group">
        <label class="col-md-2 control-label" for="txtcodigo">Horas:</label>    
        <div class="col-md-3">
            <input type="text" class="form-control" size="10" name="horas" id="horas" onblur="validar_saldo_ajuste();calcular_duracion();" maxlength="10" <? if (isset($fetch33['horas'])) echo "value='$fetch33[horas]'"?>/>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="col-md-3">
            <input type="text" class="form-control" size="10" name="minutos" id="minutos" onblur="validar_saldo_ajuste();calcular_duracion();" maxlength="10" <? if (isset($fetch33['minutos'])) echo "value='$fetch33[minutos]'"?>/>
        </div>
    </div>
