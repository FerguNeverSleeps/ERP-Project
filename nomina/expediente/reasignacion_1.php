...
<div class="form-group">
   ...
   <label class="col-md-2 control-label" for="txtcodigo">Planilla:</label>                                
   <div class="col-md-7">
       <select name="planilla_nueva" id="planilla_nueva" class="select2 form-control">
           ...
           while($fila_funcion=fetch_array($resultado4))
            {
                echo "AQUI";
                if(!$fetch33['planilla_nueva'])
                {?>
                    <option  value="<?=$fila_funcion['codtip'];?>"><?=utf8_encode($fila_funcion['descrip']);?></option>
                <?}
                else
                { 
                   if($fetch33['planilla_nueva']==$fila_funcion['codtip'])
                   {?>
                        <option  value="<?=$fila_funcion['codtip'];?>" selected><?=utf8_encode($fila_funcion['descrip']);?></option> 
                   <?}
                   else
                   {?>
                        <option  value="<?=$fila_funcion['codtip'];?>"><?=utf8_encode($fila_funcion['descrip']);?></option>
...
<div class="form-group">
   ...
   <label class="col-md-2 control-label" for="txtcodigo">Funcion:</label>                                
   <div class="col-md-7">
        <select name="funcion_nueva" id="funcion_nueva" class="select2 form-control">
            <option value="">Seleccione</option>
            <?
            $consulta="SELECT nomfuncion_id, descripcion_funcion FROM nomfuncion";
            $resultado4=sql_ejecutar($consulta);    
           
            while($fila_funcion=fetch_array($resultado4))
            {
                echo "AQUI";
                if(!$fetch33['funcion_nueva'])
                {?>
