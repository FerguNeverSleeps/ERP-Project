	...
	if(isset($ext))
	{
		...
		{	
			...
			while ( $objPHPExcel->getActiveSheet()->getCell($j_columna.$i_fila) != '') 
			{
				...
				$j_columna++;
				$conexion = new bd($_SESSION['bd']);
				$sql        = "SELECT ficha from nommovimientos_nomina where fecha = '".$fecha."' AND ficha = '".$ficha."'";
				...
				{
					$conexion = new bd($_SESSION['bd']);
					$insert_s = "INSERT INTO nomcalendarios_personal (fecha,turno_id,ficha) VALUES ('".$fecha."','".$turno."','".$ficha."')";
					$total++;				
