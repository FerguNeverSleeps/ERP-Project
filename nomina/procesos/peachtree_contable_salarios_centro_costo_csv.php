...
$sql1 = "select * from config_reportes_planilla where id = 6";
...
if(!isset($configReporte['id'])){
	print "config_reportes_planilla id=6 no encontrado (el comprobante depende del query definido en id=6)";
	exit;
}
...
while($centro_costo=$res->fetch_array()){
	...
	//print_r($data);exit;
	//NOTA - OBSERVACION 
	//REALIZAR ACA LAS MISMA OPERACION QUE SE HACEN EN EL EXCEL DEL REPORTE ID=6 PARA EL CALCULO DEL TOTAL
	$porcentaje_ss=12.25;
	$porcentaje_se=1.50;
...
$objPHPExcel->getProperties()->setCreator("AMAXONIA")
							 ...
							 ->setTitle("Comprobante Salarios Centro de Costos");
...
switch($_SESSION["bd"]) {
	case 'apjv_rrhh':
		$ctacon_credito="411602";
		$ctacon_credito_descrip="INGRESO POR REEMBOLSO-SALARIOS";

		$ctacon_detalle_debito="611313.02";
		$ctacon_detalle_debito_descrip="GASTO POR REEMBOLSO-ADMIN PJV S. A. (PERSONAL)";
...
$filename = "peachtree-sage-csv_salarios_centro_costo_".$anio."_".$mes;
