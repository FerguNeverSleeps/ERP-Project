...
$consulta="SELECT nnn.*, np.apenom, np.nombres, np.nombres2, np.apellidos, np.telefonos,nnn.forcob  "
        . "FROM nom_nomina_netos nnn "
        . "JOIN nompersonal np ON (np.cedula=nnn.cedula  AND np.ficha=nnn.ficha) "
        . "LEFT JOIN nomnivel1 n1 on (n1.codorg=nnn.codnivel1) "
...
while($fila=$resultado->fetch_assoc())
{
	...
	$detalles.=str_pad($cedula, 12, " ", STR_PAD_RIGHT);
	$detalles.="|";
...
//        $nombre=$proceso->eliminar_espacios($nombre);
        ...
        if(strlen($nombre)>=25)
            $detalles.=substr($nombre,0,25);
        else
            $detalles.=str_pad($nombre, 25, " ", STR_PAD_RIGHT);
	$detalles.="|  71";
	$detalles.="|";
	$detalles.=str_pad(trim($fila[cta_ban]), 17, " ", STR_PAD_RIGHT);
	$detalles.="|".$forcob; 
	$detalles.="|";
        $detalles.=str_pad(trim($neto_empleado), 9, " ", STR_PAD_LEFT);
...
//	elseif(strlen($neto_empleado)==4)
//		$detalles.=$proceso->completar($neto_empleado,7);
//	elseif(strlen($neto_empleado)==3)
//		$detalles.=$proceso->completar($neto_empleado,8);
...
readfile($ruta); 
header('Content-Disposition: attachment; filename="ach_banco_general_cdyp.txt"');
