...
echo $consulta="select nnn.*, np.apenom, np.telefonos,nnpa.descrip ,np.codbancob,np.cuentacob,ban.ruta
...
while($fila=$resultado->fetch_assoc())
{
	...
	$detalles.='REF\r\n';
	//LINEA 2
	$detalles.='APAGO DE PLANILLA '.$fila['descrip'];
	$detalles.="\r\n";
...
readfile($ruta); 
header('Content-Disposition: attachment; filename="banconacionalpanama.txt"');
