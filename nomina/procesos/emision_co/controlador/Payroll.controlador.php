...
Class PayrollControlador
{
    ...
    static function setDevengados($datos)
    {
        ...
        $arrayDevengados = [];
        $arrayDevengados["dias_trabajados"] = ($datos["basico"]["dias_trabajados"] != "0.00" AND !is_null($datos["basico"]["dias_trabajados"])) ? $datos["basico"]["dias_trabajados"] : 0;
        $arrayDevengados["sueldo_trabajado"] =( $datos["basico"]["sueldo_trabajado"]!= "0.00"  AND !is_null($datos["basico"]["sueldo_trabajado"])) ? $datos["basico"]["sueldo_trabajado"] : 0;
        if($datos["transporte"]["auxilio"] != "0.00"  AND !is_null($datos["transporte"]))
            $arrayDevengados["transporte"] = array($datos["transporte"]);

        if($datos["incapacidades"]["pago"] != "0.00"  AND !is_null($datos["incapacidades"])){
            $arrayDevengados["incapacidades"] = array( $datos["incapacidades"] );
        }
        else{
            unset($arrayDevengados["incapacidades"]);
        }
        if(isset($datos["heds"]) )
        ...
        if(isset($datos["vacaciones"]) ){
            if(isset($datos["vacaciones"]["comunes"]) AND  $datos["vacaciones"]["comunes"]["pago"] != "0.00" )
            {
                $arrayDevengados["vacaciones"]["comunes"] = array($datos["vacaciones"]["comunes"]);
            }
            else
                unset($arrayDevengados["vacaciones"]["comunes"]);
            
            if(isset($datos["vacaciones"]["compensadas"]) AND  ($datos["vacaciones"]["compensadas"]["pago"] != "0.00") )
                $arrayDevengados["vacaciones"]["compensadas"] = array($datos["vacaciones"]["compensadas"]);
            else
                unset($arrayDevengados["vacaciones"]["compensadas"]);
        }
        if(isset($datos["primas"]["cantidad"]) AND $datos["primas"]["cantidad"] >0 )
            $arrayDevengados["primas"] = $datos["primas"];
        else
            unset($arrayDevengados["primas"]);

        if(isset($datos["cesantias"]) )
        {
            if($datos["cesantias"]["pago"])
                $arrayDevengados["cesantias"]["pago"] = ($datos["cesantias"]["pago"] != "0.00") ? (int)$datos["cesantias"]["pago"] : "0.00";
            if($datos["cesantias"]["pago_intereses"] != 0 OR $datos["cesantias"]["pago_intereses"] != "0.00")
            {
                $arrayDevengados["cesantias"]["porcentaje"] = (int)$datos["cesantias"]["porcentaje"];
                $arrayDevengados["cesantias"]["pago_intereses"] = (int)$datos["cesantias"]["pago_intereses"]["monto"];

            }else{
                $arrayDevengados["cesantias"]["porcentaje"] = 0;

            }
            if($arrayDevengados["cesantias"]["pago"]== "0.00" AND $arrayDevengados["cesantias"]["pago_intereses"]== "0.00")
                unset($arrayDevengados["cesantias"]);

        }
        else
            unset($arrayDevengados["cesantias"]);
        if(isset($datos["bonificaciones"]) AND  ($datos["bonificaciones"]["s"] != "0.00" AND $datos["bonificaciones"]["s"] != null) )
             $arrayDevengados["bonificaciones"] = $datos["bonificaciones"];
        else
           unset($arrayDevengados["bonificaciones"]);
        if(isset($datos["auxilios"]) )
