...
if(!isset($configReporte['id'])){
	print "config_reportes_planilla id=8 no encontrado (el comprobante depende del query definido en id=8)";
	exit;
}
...
$res_cont=$conexion->query($sql_cont);
$row_cont=$res_cont->fetch_array();
$cont=$row_cont["cont"]+1;
...
while($centro_costo=$res->fetch_array()){
	...
	//print_r($data);exit;
	//NOTA - OBSERVACION 
	//REALIZAR ACA LAS MISMA OPERACION QUE SE HACEN EN EL EXCEL DEL REPORTE ID=8 PARA EL CALCULO DEL TOTAL
	$monto_total=$data["prima_produccion"]+$data["nacimiento"]+$data["matrimonio"]+$data["defuncion"]+$data["diamadre"]
                +$data["cumpleanios"]+$data["lentes"]+$data["joyeria"]+$data["navidad"]+$data["anio_labor"]+$data["asistencia"]+$data["ayuda_escolar"]+$data["calamidad_domestica"];
	$centro_costo["monto_total"]=$monto_total;

	if(!mb_detect_encoding($centro_costo["markar"],["UTF-8"],true))
		$centro_costo["markar"]=utf8_encode($centro_costo["markar"]);
	...
	if(!mb_detect_encoding($centro_costo["cuenta_contable"],["UTF-8"],true))
		$centro_costo["cuenta_contable"]=utf8_encode($centro_costo["cuenta_contable"]);

	$data_centro_costo[]=$centro_costo;
}
$centro_costo=$data_centro_costo;
//print_r($data_centro_costo);exit;

$objPHPExcel = new PHPExcel();

$objPHPExcel->getProperties()->setCreator("AMAXONIA")
							 ->setLastModifiedBy("AMAXONIA")
							 ->setTitle("Comprobante Otros Beneficios Centro de Costos");
...
switch($_SESSION["bd"]) {
	case 'apjv_rrhh':
		$ctacon_credito="411603";
		$ctacon_credito_descrip="INGRESO POR REEMBOLSO OTROS BENEFICIOS";

		$ctacon_detalle_debito="611313.03";
		$ctacon_detalle_debito_descrip="GASTO POR REEMBOLSO-ADMIN PJV S. A. (BENEFICIOS)";

		$ctacon_detalle_credito="112301";
		$ctacon_detalle_credito_descrip=str_replace(',', '', $empresa['nombre']);
		break;
}
...
//	$sheet->setCellValueExplicit("B{$ln}", $centro_costo[$i]["ctacon"],PHPExcel_Cell_DataType::TYPE_STRING);
//	$sheet->setCellValue("C{$ln}", trim($centro_costo[$i]["descrip"],"- "));
//	$sheet->setCellValue("D{$ln}", "CXC - ".trim($centro_costo[$i]["descrip"],"- "));
//	$sheet->setCellValue("E{$ln}", $centro_costo[$i]["monto_total"]);
//	$sheet->setCellValue("F{$ln}", "");	
        
        
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("A$ln", $ffecha.","."0001"."#333CXC - ".trim($centro_costo[$i]["descrip"],"- ")."-".$ffecha.",".",".$cont.",".$centro_costo[$i]["ctacon"].",".trim($centro_costo[$i]["descrip"],"- ").",".$centro_costo[$i]["monto_total"].","."".","."FALSE".","."23".","."8".","."FALSE".","."0".","."0"."", PHPExcel_Cell_DataType::TYPE_STRING);
	$ln++;

	$monto_total+=$centro_costo[$i]["monto_total"];
}
...
$objPHPExcel->getActiveSheet()->setCellValueExplicit("A$ln", $ffecha.","."0001"."#333INGRESO POR REEMBOLSO"."-".$ffecha.",".",".$cont.",".$ctacon_credito.",".$ctacon_credito_descrip.",".(-1)*$monto_total.","."".","."FALSE".","."23".","."8".","."FALSE".","."0".","."0"."", PHPExcel_Cell_DataType::TYPE_STRING);
$ln++;
...
$filename = "peachtree-sage-csv_distribucion_beneficios_centro_costo_".$anio."_".$mes;
...
//	$sheet->setCellValue("F{$ln}", "");	
        ...
        $objPHPExcel->getActiveSheet()->setCellValueExplicit("A$ln", $ffecha.","."0001"."#333GASTO POR REEMBOLSO - ".$centro_costo[$i]["descrip"]."-".$ffecha.",".",".$cont.",".$ctacon_detalle_debito.",".$ctacon_detalle_debito_descrip.",".$centro_costo[$i]["monto_total"].","."".","."FALSE".","."23".","."8".","."FALSE".","."0".","."0"."", PHPExcel_Cell_DataType::TYPE_STRING);
        $ln++;
...
//	$sheet->setCellValue("E{$ln}", "");
//	$sheet->setCellValue("F{$ln}", $centro_costo[$i]["monto_total"]);	

	$objPHPExcel->getActiveSheet()->setCellValueExplicit("A$ln", $ffecha.","."0001"."#333CXP - ".$centro_costo[$i]["descrip"]."-".$ffecha.",".",".$cont.",".$ctacon_detalle_credito.",".$ctacon_detalle_credito_descrip.",".(-1)*$centro_costo[$i]["monto_total"].","."".","."FALSE".","."23".","."8".","."FALSE".","."0".","."0"."", PHPExcel_Cell_DataType::TYPE_STRING);
	...
	$writer->setSheetIndex(0);   // Select which sheet.
        ...
        $writer->setLineEnding("\r\n"); 
	ob_clean();
	$writer->save("{$ruta}/{$filename}__".$centro_costo[$i]["markar"].".csv");
