...
if($_FILES['archivo']['name'] != '')
{
	...
	if(isset($ext))
	{
		...
		while($objPHPExcel->getActiveSheet()->getCell("A".$i)->getValue() != '')
		{
			...
			$categoria     = ($categoria == "Empleado") ? 2 : 1;
			...
			$letra++;
			$dv 				= $objPHPExcel->getActiveSheet()->getCell($letra.$i)->getValue(); // digito de verificacion
			$letra++;
			$cuentacob 			= $objPHPExcel->getActiveSheet()->getCell($letra.$i)->getValue(); // cuenta bancaria
			$letra++;
			$segurosocial_sipe 	= $objPHPExcel->getActiveSheet()->getCell($letra.$i)->getValue(); // seguro social sipe
			$letra++;
