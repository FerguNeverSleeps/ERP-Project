...
$sql = "SELECT DISTINCT np.ficha, np.cedula, np.apenom as nombre, np.suesal as sueldo
		...
		WHERE  nm.codnom=".$codnom." AND nm.tipnom=".$codtip." AND (nm.codcon >=100 AND nm.codcon <=202) AND nm.monto > 0
		ORDER BY np.ficha";
$res = $conexion -> query($sql);
...
while($fila=$res->fetch_assoc()){
	...
	$sql2="SELECT DISTINCT nm.codcon, nm.descrip, nm.tipcon FROM nom_movimientos_nomina nm 
		  WHERE nm.codnom=".$codnom." AND nm.tipnom=".$codtip." AND nm.monto > 0 AND ((nm.codcon >=100 AND nm.codcon <=202) OR (nm.codcon>=500 AND nm.codcon<=599) 
	  OR (nm.codcon>=3000 AND nm.codcon<=3002))
...
header('Content-Disposition: attachment;filename="Reporte_PLANILLA1.xls"');
