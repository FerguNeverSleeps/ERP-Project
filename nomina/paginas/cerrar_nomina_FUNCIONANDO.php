...
$nuevaConsulta = "
                  SELECT
                      ...
                      nc.tipcon
                  FROM
                      nompersonal_pry_cc npcc
                  INNER JOIN
                      proyecto_cc pcc
                      ON npcc.codigo_proyecto_cc = pcc.codigo
                  INNER JOIN
                      nom_movimientos_nomina nmn
                      ON npcc.cedula = nmn.cedula AND nmn.codnom = '{$codnom}'
                   INNER JOIN
                      nomconceptos nc
                      ON nc.codcon = nmn.codcon
                  WHERE
                      npcc.fecha_inicio = '{$periodo_ini}' AND npcc.fecha_fin = '{$periodo_fin}';";
...
while($nuevaFila = $ejecutar->fetch_array()){
    ...
    if ($descripcion == 'SALARIO') {
      $filasInsertadas[] = "('$codnom', '$codigo_proyecto', '$nuevo_valor', '$ficha', '$cedula', '$periodo_ini', '$periodo_fin', '$monto', '$cuentaContableCC', '$descripcion', '$tipo_cuenta')";
    }
    else {
      $filasInsertadas[] = "('$codnom', '$codigo_proyecto', '$nuevo_valor', '$ficha', '$cedula', '$periodo_ini', '$periodo_fin', '$monto', '$cuentaContableReal', '$descripcion', '$tipo_cuenta')";
    }
}
...
if (!empty($filasInsertadas)) {
    ...
    (cod_nomina, codigo_proyecto, monto, ficha, cedula, fecha_inicio, fecha_fin, quincena, cuenta_contable, concepto, tipo_cuenta)
