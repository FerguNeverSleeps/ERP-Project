...
if(isset($_GET['codnom']) && isset($_GET['codtip']))
{
	...
	while($filas=$result->fetch_assoc()) //Inicio
	{
		...
		{	
			$ficha          = $row2['ficha'];
			$sql3           = "	SELECT n.monto as mes_xii , (SELECT n.monto 
			FROM nom_movimientos_nomina n 
			WHERE  n.codnom =".$codnom." AND n.ficha=".$ficha." AND n.codcon=205) as seguro_social_xii
			FROM nom_movimientos_nomina n 
			...
			while($row3 = $res3->fetch_assoc()) //Inicio
			{
				...
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, ($row3['mes_xii'] - $row3['seguro_social_xii']));
				$total_salario+=($row3['mes_xii'] - $row3['seguro_social_xii']);
				$salario_departamento+=($row3['mes_xii'] - $row3['seguro_social_xii']);
				...
				$importe=($row3['mes_xii'] - $row3['seguro_social_xii']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue($Caracter.$i, ($row3['mes_xii'] - $row3['seguro_social_xii']));
				$i++;			

			}
		}
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, 'Total de XIII '. $filas['descrip'] );
	...
	$objPHPExcel->getActiveSheet()->setTitle("Desglose Planilla Mes XIII");	
	//===========================================================================
	//===========================================================================
	//===========================================================================
	//===========================================================================
	...
	$objPHPExcel->getActiveSheet()->setTitle("Desglose Mes XIII");	
	...
	while($filas=$result->fetch_assoc())
	{
		...
		{	
			...
			FROM nom_movimientos_nomina n 
			WHERE  n.codnom =".$codnom." AND n.ficha=".$ficha." AND n.codcon=96";
			$res3           =$db->query($sql3);
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(15);
	$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(15);
	$objPHPExcel->getActiveSheet()->setTitle('Desglose Serv Prof');
...
$filename = "Desglose_Decimo_".$NOMINA."_Del_".fecha($desde).'_Hasta_'.fecha($hasta);
