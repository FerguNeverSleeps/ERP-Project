...
if(isset($_GET['codnom']) && isset($_GET['codtip']))
{
	...
	$objPHPExcel->getActiveSheet()
				...
				->setCellValue('A3', 'PLANILLA BISEMANAL')
	...
	*/
	...
	//{
		...
		*/
		...
		{
			$sql2= "SELECT np.ficha, nnn.suesal as salario, 
				CONCAT_WS(' ', np.nombres, np.apellidos) as nombre,
				SUBSTRING_INDEX(SUBSTRING_INDEX(np.nombres, ' ', 1), ' ', -1) as primer_nombre,
				CASE WHEN np.apellidos LIKE 'De %' THEN 
				SUBSTRING_INDEX(SUBSTRING_INDEX(np.apellidos, ' ', 2), ' ',  2) 
		...
		{	$enc=true;
			...
			$sql3 = "SELECT
					...
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (100,119)),0) as regular,
					COALESCE((SELECT SUM(n.valor) AS monto FROM nom_movimientos_nomina n 
					...
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (100,170,171,169)), 0) as regular_monto,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=189), 0) as com14,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (105,106,107,108,109,111,112,113,114,116,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,142,143,144,149,150,151,152,153)), 0) as sobretiempo,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					...
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND ((n.codcon BETWEEN 500 AND 599) OR (n.codcon BETWEEN 216 AND 225)
                                        OR (n.codcon=197) OR (n.codcon BETWEEN 203 AND 204))), 0) as descuentos,
			...
			if($row3=$res3->fetch_array())
			{
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $row3['regular']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue('O'.$i, $otrosing);
