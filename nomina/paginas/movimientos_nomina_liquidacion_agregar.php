	...
	if($SITUACION!="Inactivo")
	{
		foreach($_POST['seleccion'] as $valor)
		{
			...
			if(num_rows($resultado_mov)==0)
			{
				...
				$filacon=fetch_array($resultado_con);
				$REF=$ref[$valor];

				eval($filacon['formula']);

				if($MONTO<=0 && $filacon['montocero']==1)
				{
					$entrar=0;
				}
				else
				...
				if ($entrar==1)
				{
					$consulta="INSERT into nom_movimientos_nomina (codnom, codcon, ficha, tipcon, valor, monto, unidad, descrip, codnivel1, codnivel2, codnivel3, codnivel4, codnivel5, codnivel6, codnivel7, tipnom, cedula, impdet, mes, anio) values ('".$_POST['nomina']."', '".$concepto[$valor]."','".$_POST['ficha']."','".$tipoconcepto[$valor]."','".$REF."','".$MONTO."','".$unidad[$valor]."','".$descripcion[$valor]."','$fila[codnivel1]','$fila[codnivel2]','$fila[codnivel3]','$fila[codnivel4]','$fila[codnivel5]','$fila[codnivel6]','$fila[codnivel7]','".$_SESSION['codigo_nomina']."','$fila[cedula]','$filacon[impdet]', '{$MES}', '{$ANIO}')";
					$resultado=query($consulta,$conexion);
				}
			}
		}
...
if(isset($_POST['buscar']) || $tipob!=NULL)
{
	...
	{
		case "exacta": 
			$consulta=buscar_palabra_exacta($tabla,$des,"descrip",$des,"codcon" );
			break;
		case "todas":
