...
if(isset($_GET['codnom']) && isset($_GET['codtip']))
{
	...
	$objPHPExcel->getActiveSheet()
				...
				->setCellValue('A4', 'DEL '. $desde1 .' AL '. $hasta1 )
                                    ->setCellValue('A5', "Empleados Cobro Por Cheque");
...
//            ->setCellValue('A6', 'SALARIOS')
            ...
            ->setCellValue('A8', 'No.')
            ->setCellValue('B8', 'NOMBRE EMPLEADO')
...
//	$objPHPExcel->getActiveSheet()->mergeCells('D7:I7');
	$objPHPExcel->getActiveSheet()->getStyle('A8:M8')->applyFromArray(allBordersThin());
	...
	*/
	...
	//{
		...
		$sql2= "SELECT np.ficha, np.suesal as salario, 
				...
				AND    (np.estado = 'REGULAR' OR np.estado = 'Activo' OR np.estado = 'Egresado')
                                AND    (np.forcob LIKE 'Cheque')
				ORDER  BY np.ficha";
		...
		{	$enc=true;
			...
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.$i)->getFont()->setSize(8);
			...
			$sql3 = "SELECT
					...
					COALESCE((SELECT SUM(n.valor) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (110,112,113,114,120,121,123,124,125,126,127,128,129,130,131,132,133,134,135,136,138,139,142,143,144,149,150,151,152,153,163,164,165,166)), 0) as lasdemas,
					COALESCE((SELECT SUM(n.valor) AS monto FROM nom_movimientos_nomina n 
					...
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (105,106,107,108,109,111,112,113,114,117,116,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,142,143,144,149,150,151,152,153,163,164,165,166,167,168,172,118)), 0) as sobretiempo,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					...
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (202,208,212,215)), 0) as isr,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND ((n.codcon BETWEEN 500 AND 599) OR (n.codcon BETWEEN 216 AND 225) OR (n.codcon BETWEEN 203 AND 204) OR (n.codcon IN (198,199)))), 0) as descuentos,
...
//				$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $row2['salario']);
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $regular);
...
//				$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $row3['com14']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, (($salBruto) - $totalDeducciones));
...
//			$objPHPExcel->getActiveSheet()->setCellValue('Y'.$i, "=SUM(Y".$ini.":Y".($i-1).")");
			...
			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':M'.$i)->getFont()->setSize(6);
...
//	$objPHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(10);
	...
	$filename = "Empleados_Cheque_".$NOMINA."_Del_".fecha($desde).'_Hasta_'.fecha($hasta);
