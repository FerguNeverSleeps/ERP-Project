...
$objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('E8', 'TARDANZA/AUSENCIA')
            ->setCellValue('F8', 'INCAPACIDAD')
            ->setCellValue('G7', 'TOTAL')
            ->setCellValue('G8', 'SALARIOS')
            
            ->setCellValue('H6', 'DEDUCCIONES')
            ->setCellValue('H8', 'SEG. SOCIAL')
            ->setCellValue('I8', 'SEG. EDUCATIVO')
            ->setCellValue('J8', 'IMP. s. RENTA')
            ->setCellValue('K8', 'DESC LEY')
            ->setCellValue('L7', 'DESCUENTOS')
            ->setCellValue('L8', 'VARIOS')
            ->setCellValue('M7', 'ACREEDOR')
            ->setCellValue('M8', 'POR PAGAR')
            ->setCellValue('N7', 'NETO')
            ->setCellValue('N8', 'A PAGAR');
            
cellColor('C6:G8', '92d050');
cellColor('H6:M8', 'ffcc00');


//$objPHPExcel->getActiveSheet()->getStyle('A6:Q8')->getFont()->setName('Calibri');
$objPHPExcel->getActiveSheet()->getStyle('A6:N8')->getFont()->setSize(12);
$objPHPExcel->getActiveSheet()->getStyle('A6:N8')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle('A6:N8')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$objPHPExcel->getActiveSheet()->mergeCells('C6:G6');
$objPHPExcel->getActiveSheet()->mergeCells('H6:M6');
$objPHPExcel->getActiveSheet()->getStyle('A6:N8')->applyFromArray(allBordersThin());
...
$total_isr_gastos=$total_isr_salario=$total_ausencia=$total_ausencia_tardanza=$total_descuentos=$total_neto="=";
...
//			ORDER  BY np.ficha";
	...
	{	$enc=true;
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':N'.$i)->applyFromArray(allBordersThin());

		$sql3 = "SELECT
				 COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 

  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (100,103,104,119)),0) as salario,
				 COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
				...
				COALESCE((SELECT  n.monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (190,198)), 0) as ausencia,
                                COALESCE((SELECT  n.monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (140,169)), 0) as incapacidad,
                                COALESCE((SELECT  n.monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (196)), 0) as permiso,
 				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha."
  				  AND    n.codcon IN (202, 215, 601, 605)), 0) as isr,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha."
				...
				(SELECT  COALESCE(SUM(n.monto), 0) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (197,210,299,204))  as descuentos_varios";
				 //echo $sql3;exit;
		...
		if($row3=$res3->fetch_array())
		{
			...
			$sql5 = "SELECT COALESCE(SUM(n.monto), 0) AS total_extra_salario  
					...
					WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha."
					AND  n.codcon IN (105,106,107,108,109,110,111,112,113,116,117,118,120,121,122,123,124,125,126,158,162,165,166)";
			...
			//$impuesto = ($row3['isr_gastos'] >0) ? $row3['isr'] - $row3['isr_gastos'] : $row3['isr'];
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('C'.$i.':G'.$i)->getAlignment()
...
//			$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, );
                        $objPHPExcel->getActiveSheet()->setCellValue('E'.$i, $row3['tardanza']+$row3['ausencia']);
                        $objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $row3['incapacidad']+$row3['permiso']);
			//$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $salario_trabajador);  
                        $objPHPExcel->getActiveSheet()->setCellValue('G'.$i, "=C$i+D$i-E$i+F$i");
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $row3['seguro_social']);
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $row3['seguro_educativo']);
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $row3['isr']);
//			$objPHPExcel->getActiveSheet()->setCellValue('P'.$i, $row3['isr_gastos']);
                        
			$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $row3['descuentos_varios']);
			$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $row3['descuentos']);
			...
			//$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $neto);
                        $objPHPExcel->getActiveSheet()->setCellValue('K'.$i, "=H$i+I$i+J$i");
			$objPHPExcel->getActiveSheet()->setCellValue('N'.$i, "=G$i-K$i-L$i-M$i-".$AJUSTE);
			
			// =+C38+F38+G38+H38+I38+J38-K38-L38-M38-N38-O38-P38
			$neto = "='".$dia_fin." ".$mes_letras."'!N".$i; //$objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getValue();

			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->getNumberFormat()
	...
	//$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':Q'.$i)->getFont()->setName('Calibri');
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->getNumberFormat()
									  ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
		...
		if($nivel==1)
		{
			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->applyFromArray(allBordersMedium());
		}
		else
		{
			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->applyFromArray(allBordersThin());
		}

		$i++;
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':N'.$i)->applyFromArray(allBordersMedium());	
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':N'.$i)->applyFromArray(allBordersMedium());	
//$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':Q'.$i)->getFont()->setName('Calibri');
$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':N'.$i)->getFont()->setSize(12);
...
$objPHPExcel->getActiveSheet()
			...
			->setCellValue('D'.$i, $total_extra_salario)
                        ->setCellValue('E'.$i, $total_ausencia_tardanza)
                        ->setCellValue('F'.$i, $total_incapacidad)
			->setCellValue('G'.$i, $total_salario)
			->setCellValue('H'.$i, $total_segurosocial_salario)
			->setCellValue('I'.$i, $total_seguro_educativo)
			->setCellValue('J'.$i, $total_isr_salario)                        
			->setCellValue('K'.$i, $Total_Desc_Ley)
			->setCellValue('M'.$i, $total_descuentos)
			->setCellValue('L'.$i, $total_desc_varios)
			->setCellValue('N'.$i, $total_neto);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':N'.$i)->getNumberFormat()
									  ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
...
$filename = "Horizontal_".$NOMINA."_CDYP_Del_".fecha($desde).'_Hasta_'.fecha($hasta);
