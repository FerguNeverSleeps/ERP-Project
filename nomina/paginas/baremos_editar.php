...
if(isset($_POST['aceptar'])){
	...
	foreach($indices as $valor){
		$campo=mysqli_fetch_field_direct($resultado, $valor)->name;
		if($cadena==""){
...
//exit(0);
	...
	document.location.href=\"".$url.".php?pagina=".$pagina."\"
	</SCRIPT>";
...
function cerrar(retorno){
	document.location.href=retorno+".php?pagina=<?=$pagina?>"
}
...
<INPUT type="hidden" name="pagina" value="<?echo "$pagina";?>"></td>
	...
	foreach($titulos as $nombre){
		$campo=mysqli_fetch_field_direct($resultado, $indices[$i])->name;
	//echo $campo;
