...
//echo $conexion;
$url = "importar_asistencia_detalle";
$modulo = "Importar Asistencia";
$tabla = "caa_procesar_asistencia";
$titulos = array("Ficha", "Nombre", "Fecha","Referencia","Concepto");
$indices = array("ficha", "apenom", "fecha","referencia","concepto");
...
if (isset($_POST['buscar']) || $tipob != NULL) {
    ...
    if ($busqueda == 'ficha') {
        $consulta = "select *, nompersonal.apenom from " . $tabla . " inner join nompersonal on " . $tabla . ".personal_id=nompersonal.ficha where id_encabezado=" . $reg . " and nompersonal.ficha=".$des." order by nompersonal.ficha, fecha";
    } else {
        switch ($tipob) {
            case "exacta":
    ...
    $consulta = "select *,nompersonal.apenom from " . $tabla . " inner join nompersonal on " . $tabla . ".ficha=nompersonal.ficha where id_encabezado=" . $reg." order by nompersonal.ficha, fecha";
    #echo $consulta;
}
...
<html>
    ...
    <body>
        <form name="<?php echo $url ?>" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" target="_self">
            ...
            titulo($modulo, "importar_asistencia_editar.php?reg=$reg&pagina=$pagina&tipo=agregar", "importar_asistencia.php", "acceso"); ?>
                    ...
                    if ($num_paginas != 0) {
                        ...
                        while ($fila = mysqli_fetch_array($resultado))
                        {
                            ...
                            if ($i % 2 == 0) {
                                ...
                                icono("importar_asistencia_editar.php?id=" . $id . "&pagina=" . $pagina . "&reg=" . $reg, "Editar Asistencia", "edit.gif");
                                icono("javascript:if(confirm('Esta seguro que desea eliminar el registro ?')){document.location.href='importar_asistencia_detalle.php?id=" . $id . "&pagina=" . $pagina . "&accion=eliminar&reg=" . $reg . "';}", "Eliminar Asistencia", "delete.gif");
