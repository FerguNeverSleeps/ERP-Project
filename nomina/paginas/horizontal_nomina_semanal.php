...
$objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('Q7', 'DESCUENTOS')
            ->setCellValue('R7', 'ACREEDOR')
            ->setCellValue('S7', 'SALARIO')
            ->setCellValue('A8', 'No.')
            ->setCellValue('B8', 'NOMBRE')
            ->setCellValue('C8', 'SALARIOS')
            ->setCellValue('D8', 'SERVICIOS PROFESIONALES')
	...
	{	$enc=true;
		...
		$sql3 = "SELECT
				 COALESCE((SELECT n.monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (100)),0) as salario,
				 COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (104)), 0) as comision,
				 COALESCE((SELECT n.monto FROM nom_movimientos_nomina n 
				...
				COALESCE((SELECT  n.monto FROM nom_movimientos_nomina n 
 				...
 				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  ...
  				  AND    n.codcon IN (202,203, 601, 605)), 0) as isr,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha."
		...
		if($row3=$res3->fetch_array())
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $row3['descuentos_varios']);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('S'.$i, "=C$i+D$i+E$i+F$i+G$i+H$i-I$i-J$i-P$i-Q$i-R$i-".$AJUSTE);
...
$objPHPExcel->getActiveSheet()
			...
			->setCellValue('Q'.$i, $total_desc_varios)
