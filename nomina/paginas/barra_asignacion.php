<?php
session_start(); set_time_limit(0); //var_dump($argv);
$registro_id=$argv[1];
...
$db_name=$argv[8];
$_SESSION['bd']=$argv[8];
$SALARIOMIN=$argv[9];//$_SESSION['monsalmin']
...
$query="select pe.apenom, c.descrip, pe.cedula, pe.ficha, pe.suesal, pe.sexo, pe.fecnac, pe.tipnom, pe.fecing, pe.codpro, 
        	   pe.codcat, pe.codcargo, pe.estado, pe.sueldopro, pe.contrato, pe.forcob, pe.codnivel1, pe.codnivel2, pe.codnivel3, 
        	   pe.codnivel4, pe.codnivel5, pe.codnivel6, pe.codnivel7, pe.fechaplica, pe.tipopres, pe.fechasus, pe.fechareisus,
        	   pe.fecharetiro, c.contractual, pe.fechavac, pe.fechareivac, c.proratea, c.formula, c.montocero, c.codcon, c.tipcon, 
               c.unidad, pe.antiguedadap, pe.nominstruccion_id  
		...
		and ct.codtip = '".$codigo_nomina."' and c.tipcon='A' 
...
{ 
   ...
   if(! in_array($CEDULA, $historico_cedulas)){
   		$insert="insert into nom_movimientos_historico 
   		         values ('','".$registro_id."','".$codigo_nomina."','".$NIVEL1."','".$NIVEL2."','".$NIVEL3."',
   		         	     '".$NIVEL4."','".$NIVEL5."','".$NIVEL6."','".$NIVEL7."','".$FICHA."','".$SUELDO."',
   			             '".$CODCARGO."','".$SITUACIONPER."','".$CEDULA."')";
        $result_insert=sql_ejecutar($insert);
        $historico_cedulas[]=$CEDULA;
    }
...
exec("php -f barra_deduccion.php "
	.$registro_id." ".$codigo_nomina." ".$frecuencia." ".$periodo_ini." "
	.$periodo_fin." ".$mes." ".$anio." ".$_SESSION['bd']." ".$SALARIOMIN
	); // ." > /dev/null 2&1 &"
?>