...
function vacacionesproporcionalessalarioacum($FICHA, $fecha_inicio, $fecha_fin) 
{
  ...
  // $fechaingresocolaborador = $fecha_inicio; // se le suma el año actual
   ...
   if ($mesf < $mes) {
    ...
    $fechaingresocolaborador= "$anio-$mes-$dia";
    // Esta consulta se modifico quitando la suma de comisiones
    // Ya que Jose modifico en formularios para que quede todo en salario bruto

    // Nuevos cambios
    $empresaPermitido = "SELECT * FROM nomempresa WHERE nom_emp IN ('MARIPEREZ MOTORS, S.A ')";
    $respuestaPermitido = $selectra->query($empresaPermitido);
    ...
    if ($respuestaPermitido->num_rows == 1) {
        ...
        $consulta = "SELECT (SUM(salario_bruto)+SUM(bono)+SUM(otros_ing)+SUM(prima)+SUM(comisiones)) as monto "
            . "FROM salarios_acumulados "
            . "WHERE ficha='".$FICHA."' and fecha_pago>='$fechaingresocolaborador' and fecha_pago<='$fecha_fin'" ;
    }
...
// Funcion para traer la rata (concepto que segun entiendo es el valor por hora, según las horas trabajadas por mes)
function traerBaseHora($FICHA) {
    include 'globales.php';
    global $conexion;
    $selectra = new bd($_SESSION['bd']);

    $consulta = "SELECT hora_base FROM `nompersonal` WHERE ficha = '$FICHA'";
    ...
    if ($resultado->num_rows <= 0) {
        ...
        return $fila['hora_base'];
    }
}

// Funcion para traer la rata (concepto que segun entiendo es el valor por hora, según las horas trabajadas por mes)
function traerRata($SALARIO, $FICHA) {
    include 'globales.php';
    global $conexion;
    $selectra = new bd($_SESSION['bd']);
...
function horastrabajadas_panama($ficha, $fechaini, $fechafin, $opc)
{
    ...
    switch ($empresa)
    {
        case "zkteco":
            ...
            elseif($opc=="d1.50x1.75")
                $consulta = "SELECT SUM(minutos) AS minutos FROM reloj_procesar WHERE concepto='d1.50x1.75' AND ficha='$ficha'";
...
// Funcion para calcular vacaciones con 
function acumuladosalvaccomi($ficha, $fecha_inicio) 
{
    include 'globales.php';
    $selectra = new bd($_SESSION['bd']);
    ...
    $consulta = "SELECT "
            . "sum(salario_bruto) as monto," 
            . "sum(comisiones) as comisiones,"
            . "sum(gratificaciones) as gratificaciones,"
            . "sum(bono) as bono,"
            . "sum(otros_ing) as otros_ingresos "
            . "FROM salarios_acumulados "
            . "WHERE ficha='".$FICHA."' and (fecha_pago>='$fechames11' and fecha_pago<='$fecha_inicio')"; 
    ...
    if ($resultado->num_rows <= 0) {
        ...
        // Se suman todos los conceptos de los salarios acumulados
        $sum_vac_acu = $fila['monto'] + $fila['comisiones'] + $fila['gratificaciones'] + $fila['bono'] + $fila['otros_ingresos'];
        // Retornamos esa suma
        return $sum_vac_acu;
    }
}
...
// Se crean funciones para totalizar por ficha las comisiones by DavidS
function comisiones_salario_brutoacum($ficha, $fechaini, $fechafin)
{
    include 'globales.php';
    $selectra = new bd($_SESSION['bd']);

    $consulta = "SELECT (SUM(comisiones)) as monto "
            . "FROM salarios_acumulados "
            . "WHERE ficha='".$FICHA."' and fecha_pago >='$fechaini' and fecha_pago <='$fechafin'";
    $resultado = $selectra->query($consulta);
    $fila = $resultado->fetch_assoc();
...
// Se crean funciones para totalizar por ficha las bono by DavidS
function bono_salario_brutoacum($ficha, $fechaini, $fechafin)
{
    include 'globales.php';
    $selectra = new bd($_SESSION['bd']);
...
//    
    ...
    $consulta = "SELECT (SUM(salario_bruto)+SUM(gtorep)) as monto "
            . "FROM "
            . "(SELECT salario_bruto,gtorep FROM salarios_acumulados "
            . "WHERE ficha='".$ficha."' AND fecha_pago <='$fechafin' "
            . "AND frecuencia_planilla IN ( 2, 3 ) "
            . "ORDER BY fecha_pago DESC "
            . "LIMIT 0 , 12) "
            . "AS subquery";
    $resultado = query($consulta, $conexion);
...
function diferencia_fechas_dias($fechaingreso, $fechafin) 
{
    ...
    // Si la fecha de ingreso es mayor que la fecha fin, retorna 0
    if ($fecha1 > $fecha2) {
        return 0;
    }
    ...
    // Convierte todo a días
    $totalDias = ($diff->y * 365) + ($diff->m * 30) + $diff->d; 
...
function diferencia_fechas_completa($fechaingreso, $fechafin) 
{
    $diferencia =[]; 
...
function permiso_con_goce_salario($ficha, $fechaini, $fechafin)
{
    global $conexion;
    $consulta_personal = "SELECT cedula FROM nompersonal WHERE ficha = {$ficha}";
    $result_personal = query($consulta_personal, $conexion);
...
$consulta="SELECT SUM(duracion) AS duracion "
        ...
        . "WHERE cedula='$cedula' "
        . "AND tipo=4 "
        . "AND subtipo IN (18,19,20,22,133,145) "
        . "AND fecha BETWEEN '$fechaini' and '$fechafin'"        
        . "AND duracion >0";       
...
function permiso_sin_goce_salario($ficha, $fechaini, $fechafin)
{
    global $conexion;
    $consulta_personal = "SELECT cedula FROM nompersonal WHERE ficha = {$ficha}";
    $result_personal = query($consulta_personal, $conexion);
...
function incapacidad($ficha, $fechaini, $fechafin)
{
    global $conexion;
    $consulta_personal = "SELECT cedula FROM nompersonal WHERE ficha = {$ficha}";
    $result_personal = query($consulta_personal, $conexion);
   ...
   $consulta="SELECT COALESCE(SUM(dias),0) AS dias "
        . "FROM expediente "
        . "WHERE cedula='$cedula' "
        . "AND tipo=4 "
        . "AND subtipo=21 "
        . "AND fecha_inicio >= '$fechaini' AND fecha_fin <= '$fechafin' "        
        . "AND dias >0";    
...
function dias_pago_vac($ficha, $fechaini, $fechafin)
{
    global $conexion;
    $consulta_personal = "SELECT cedula FROM nompersonal WHERE ficha = {$ficha}";
    $result_personal = query($consulta_personal, $conexion);
    ...
    $consulta="SELECT DATEDIFF(fecha_fin, fecha_inicio ) tiempos
        FROM expediente 
        WHERE cedula='$cedula' 
        AND tipo=11 
        AND fecha_inicio >= '$fechaini' AND fecha_inicio <= '$fechafin'   
        ";    
