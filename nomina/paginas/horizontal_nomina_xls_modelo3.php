...
$objPHPExcel->getActiveSheet()
	    ->setCellValue('B8', 'COLABORADORES')
            ->setCellValue('C6', 'ASIGNACIONES')
            ->setCellValue('C8', 'SALARIO')
            ->setCellValue('D8', 'VIATICOS')
            ->setCellValue('E8', 'COMISION')
            ->setCellValue('F8', 'H. EXTRA')
            ->setCellValue('G8', 'DOMINGO')
            ->setCellValue('H8', 'FERIADO')
            ->setCellValue('I8', 'OTROS ING.')
            ->setCellValue('J8', 'COMP. ESCOLAR')
            ->setCellValue('K8', 'GASTOS REP.')
            ->setCellValue('L8', 'BONIFICACION')
            ->setCellValue('M8', 'XIII MES')
            ->setCellValue('N8', 'TARDANZA')
            ->setCellValue('O8', 'AUSENCIA')
            ->setCellValue('P8', 'INCAPACIDAD')
            ->setCellValue('Q7', 'TOTAL')
            ->setCellValue('Q8', 'ASIGNACIONES')
            
            ->setCellValue('R6', 'DEDUCCIONES')
            ->setCellValue('R8', 'SEG. SOCIAL')
            ->setCellValue('S8', 'SEG. SOCIAL GR.')
            ->setCellValue('T8', 'SEG. SOCIAL XIII M.')
            ->setCellValue('U8', 'SEG. EDUCATIVO')
            ->setCellValue('V8', 'IMP. s. RENTA')
            ->setCellValue('W8', 'IMP. s. RENTA GR')
            ->setCellValue('X8', 'IMP. s. RENTA XII M.')
            ->setCellValue('Y8', 'DESC LEY')
            ->setCellValue('Z7', ' DESCUENTOS ')
            ->setCellValue('Z8', ' VARIOS ')
            ->setCellValue('AA7', ' ACREEDOR ')
            ->setCellValue('AA8', ' POR PAGAR ')
            ->setCellValue('AB7', 'NETO')
            ->setCellValue('AB8', 'A PAGAR');

cellColor('C6:Q8', '92d050');
cellColor('R6:AB8', 'ffcc00');


//$objPHPExcel->getActiveSheet()->getStyle('A6:Q8')->getFont()->setName('Calibri');
$objPHPExcel->getActiveSheet()->getStyle('A6:AB8')->getFont()->setSize(12);
$objPHPExcel->getActiveSheet()->getStyle('A6:AB8')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle('A6:AB8')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$objPHPExcel->getActiveSheet()->mergeCells('C6:Q6');
$objPHPExcel->getActiveSheet()->mergeCells('R6:AB6');
$objPHPExcel->getActiveSheet()->getStyle('A6:AB8')->applyFromArray(allBordersThin());
...
$i=9; $nivel=1;
// $monto_salarios=0;
$total_salarios=$total_salarios_especie=$total_comision=$total_combustible=$total_reembolso=$total_uso_auto=$total_bono=$total_incapacidad="=";
$total_extra_salario=$total_extra_gastosrep=$total_vacaciones=$total_cxc=$total_domingo=$total_feriado=$total_otros_ing="=";
$total_segurosocial=$total_segurosocial_gastosrep=$total_tardanza=$total_seguro_educativo="=";
$total_isr_gastos=$total_isr_salario=$total_ausencia=$total_descuentos=$total_neto="=";
$total_salario=$MesXIII=$Total_Desc_Ley=$SeguroSocialMesXIII=$total_desc_varios=$total_desc1=$total_desc2=$total_desc3="=";
...
//			ORDER  BY np.ficha";
	...
	{	$enc=true;
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':AB'.$i)->getFont()->setSize(12);
		...
		$sql3 = "SELECT
				 ...
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
                                  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=181),0) as salario_especie,
                 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (132, 136, 137, 158, 159, 160, 161, 163, 172)), 0) as horas_extras,
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
                                  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=102),0) as xiii_mes,				 	
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (141, 157, 176)), 0) as comision,
                                 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (105, 162, 167)), 0) as domingo,
                                 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (122, 165, 166)), 0) as feriado,
                                 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (147, 175)), 0) as otros_ing,
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=199), 0) as tardanza,		 
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=189), 0) as bono,			 
				 COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (200)), 0) as seguro_social,
                                COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=203), 0) as seguro_social_gastosrep,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon=201), 0) as seguro_educativo,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (205, 218)), 0) as seguro_social_xiii,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
				...
				COALESCE((SELECT  SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (177)), 0) as uso_auto,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (198)), 0) as ausencia,
                                COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon in (140,169)), 0) as incapacidad,
 				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (202, 601, 605)), 0) as isr,
				COALESCE((SELECT SUM(n.monto) FROM nom_movimientos_nomina n 
                                  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (211, 217)),0) as isr_xiii,
				COALESCE((SELECT  SUM(n.monto) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND n.codcon IN (207, 208, 606, 607)), 0) as isr_gastos,
				(SELECT  COALESCE(SUM(n.monto), 0) FROM nom_movimientos_nomina n 
  				  WHERE  n.codnom=".$codnom." AND n.tipnom=".$codtip." AND n.ficha=".$ficha." AND (n.codcon BETWEEN 500 AND 507 OR n.codcon BETWEEN 508 AND 599))  as descuentos,
				(SELECT  COALESCE(SUM(n.monto), 0) FROM nom_movimientos_nomina n 
		...
		if($row3=$res3->fetch_array())
		{
			...
			$sql5 = "SELECT COALESCE(SUM(n.monto), 0) AS total_extra_salario  
					...
					AND (  n.codcon IN (106, 108, 142, 143, 153, 152)
					 OR    n.codcon IN (111,116,117,118,123, 125, 126, 144, 149, 150, 151)
					 OR    n.codcon IN (107, 109, 152, 153, 158, 159, 160, 161, 163,120, 121, 127, 112, 113, 127, 
                                         128, 129, 130, 131, 132,133, 134, 135, 136, 138, 139) )";
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $row3['salario_especie']);
			...
			{
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $row3['horas_extras']);
				$HORAS_EXTRA_SALARIO = $row3['horas_extras'];
			}
			else
			{
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $MONTO_EXTRA_SALARIO);
			...
			$salario_trabajador=$row3['salario']+$row3['salario_especie']+$row3['uso_auto']+$row3['comision']
                                +$row3['bono']+$row3['domingo']+$row3['feriado']+$row3['otros_ing']
                                +$HORAS_EXTRA_SALARIO+$row3['reembolso']+$row3['vacaciones']+$row3['xiii_mes']+$row3['incapacidad']
                                -$row3['tardanza']-$row3['ausencia'];
                        $objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $row3['domingo']);
                        $objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $row3['feriado']);
                        $objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $row3['otros_ing']);
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $row3['uso_auto']);
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $row3['reembolso']);
			$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $row3['bono']);
			$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $row3['xiii_mes']);
			$objPHPExcel->getActiveSheet()->setCellValue('N'.$i, $row3['tardanza']);
			$objPHPExcel->getActiveSheet()->setCellValue('O'.$i, $row3['ausencia']);
                        $objPHPExcel->getActiveSheet()->setCellValue('P'.$i, $row3['incapacidad']);
			$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $salario_trabajador);                        
			$objPHPExcel->getActiveSheet()->setCellValue('R'.$i, $row3['seguro_social']);
                        $objPHPExcel->getActiveSheet()->setCellValue('S'.$i, $row3['seguro_social_gastosrep']);
			$objPHPExcel->getActiveSheet()->setCellValue('T'.$i, $row3['seguro_social_xiii']);
			$objPHPExcel->getActiveSheet()->setCellValue('U'.$i, $row3['seguro_educativo']);
			$objPHPExcel->getActiveSheet()->setCellValue('V'.$i, $row3['isr']);
			$objPHPExcel->getActiveSheet()->setCellValue('W'.$i, $row3['isr_gastos']);
			$objPHPExcel->getActiveSheet()->setCellValue('X'.$i, $row3['isr_xiii']);			
			$objPHPExcel->getActiveSheet()->setCellValue('Z'.$i, $row3['descuentos_varios']);
			$objPHPExcel->getActiveSheet()->setCellValue('AA'.$i, $row3['descuentos']);

			
			$neto2 = $row3['salario'] +$row3['salario_especie'] + $row3['xiii_mes']+ $row3['vacaciones']+ $row3['reembolso']+ $row3['comision']
                                + $row3['domingo']+ $row3['feriado']+ $row3['otros_ing']+ $row3['bono'] + $row3['incapacidad'] + $HORAS_EXTRA_SALARIO 
				- $row3['seguro_social'] - $row3['seguro_social_gastosrep'] - $row3['seguro_educativo'] - $row3['tardanza'] - $row3['ausencia']
				- $row3['isr'] - $row3['isr_gastos'] - $row3['descuentos_varios'] - $row3['descuentos'] - $row3['cxc']- $row3['seguro_social_xiii'-$row3['isr_xiii']]- $AJUSTE;

			//$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $neto);
                       
			//$objPHPExcel->getActiveSheet()->setCellValue('AA'.$i, "=Q$i+R$i+S$i+T$i+U$i");
			//$objPHPExcel->getActiveSheet()->setCellValue('AB'.$i, "=P$i-V$i-W$i-X$i-".$AJUSTE);
                        $objPHPExcel->getActiveSheet()->setCellValue('Y'.$i, "=R$i+S$i+T$i+U$i+V$i+W$i+X$i");
                        $objPHPExcel->getActiveSheet()->setCellValue('AB'.$i, "=Q$i-Y$i-Z$i-AA$i-".$AJUSTE);
			// =+C38+F38+G38+H38+I38+J38-K38-L38-M38-N38-O38-P38
			$neto = "='".$dia_fin." ".$mes_letras."'!AB".$i; //$objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getValue();

			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->getNumberFormat()
	...
	//$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':Q'.$i)->getFont()->setName('Calibri');
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->getNumberFormat()
									  ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
		...
		if($nivel==1)
		{
			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->applyFromArray(allBordersMedium());
		}
		else
		{
			$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->applyFromArray(allBordersThin());
		}

		$i++;
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':AB'.$i)->applyFromArray(allBordersMedium());	
...
//$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':Q'.$i)->getFont()->setName('Calibri');
$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':AB'.$i)->getFont()->setSize(12);
$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':AB'.$i)->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()
			->setCellValue('B'.$i, 'Total')
			->setCellValue('C'.$i, $total_salarios)
			->setCellValue('D'.$i, $total_salarios_especie)
			->setCellValue('E'.$i, $total_comision)
			->setCellValue('F'.$i, $total_extra_salario)
                        ->setCellValue('G'.$i, $total_domingo)
                        ->setCellValue('H'.$i, $total_feriado)
                        ->setCellValue('I'.$i, $total_otros_ing)
			->setCellValue('J'.$i, $total_uso_auto)
			->setCellValue('K'.$i, $total_reembolso)
			->setCellValue('L'.$i, $total_bono)
			->setCellValue('M'.$i, $total_tardanza)
			->setCellValue('N'.$i, $total_ausencia)
                        ->setCellValue('O'.$i, $total_incapacidad)
			->setCellValue('P'.$i, $total_salario)
			->setCellValue('Q'.$i, $total_segurosocial)
                        ->setCellValue('R'.$i, $total_segurosocial_gastosrep)
			->setCellValue('S'.$i, $total_seguro_educativo)
			->setCellValue('T'.$i, $total_isr_salario)
			->setCellValue('U'.$i, $total_isr_gastos)
			->setCellValue('V'.$i, $Total_Desc_Ley)
			->setCellValue('W'.$i, $total_descuentos)
			->setCellValue('X'.$i, $total_desc_varios)
			->setCellValue('Y'.$i, $total_desc1)
			->setCellValue('Z'.$i, $total_desc2)
			->setCellValue('AA'.$i, $total_desc3)
			->setCellValue('AB'.$i, $total_neto);
		$objPHPExcel->getActiveSheet()->getStyle('C'.$i.':AB'.$i)->getNumberFormat()
									  ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
...
$select_contab_salario = "SELECT if(ctacon != '' OR ctacon is not null, ctacon,  '---' ) ctacon FROM nomconceptos WHERE codcon in (100);";
$res_contab_salario = $conexion->query($select_contab_salario);
$contab_salario     = $res_contab_salario->fetch_array();
$CTA_CONTAB_SALARIO = $contab_salario['ctacon'];

$i++; $k=$i;
$objPHPExcel->getActiveSheet()
			->setCellValue('A'.$i, $CTA_CONTAB_SALARIO )
			->setCellValue('B'.$i, 'Salario Regular')
		//	->setCellValue('C'.$i, "=C".$j."+D".$j."+E".$j."+F".$j."+G".$j."+H".$j."+I".$j."+J".$j."+L".$j."O".$j."-N".$j."-M".$j )
			->setCellValue('C'.$i, "=C".$j."+M".$j."+H".$j."-N".$j."-O".$j )			
			->setCellValue('G'.$i, 'Gasto de Seg. Social')
			->setCellValue('I'.$i++, "=C".$k."*0.1225")
			->setCellValue('B'.$i, 'Comisiones')
			->setCellValue('C'.$i, "=E".$j )
			->setCellValue('G'.$i, 'Gasto de Seg. Educ.')
			->setCellValue('I'.$i++, "=C".$k."*0.015")
			->setCellValue('B'.$i, 'Extra de Salario')
			->setCellValue('C'.$i, "=F".$j."+G".$j )
			->setCellValue('G'.$i, 'Gasto de Riesgos Prof.')
			->setCellValue('I'.$i++, "=C".$k."*0.021" )
			->setCellValue('B'.$i, 'Comp. Escolar')
			->setCellValue('C'.$i, "=J".$j )
