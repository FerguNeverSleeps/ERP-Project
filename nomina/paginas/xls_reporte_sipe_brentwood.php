...
if(isset($_REQUEST['codnom1']) && isset($_REQUEST['codnom2']))
{
	$codnom1 = $_REQUEST['codnom1']; // Primera Quincena - Frecuencia 2
	$codnom2 = $_REQUEST['codnom2']; // Segunda Quincena - Frecuencia 3

	$db = new Database($_SESSION['bd']);

	$sql = "SELECT (SELECT periodo_ini FROM nom_nominas_pago WHERE codnom='{$codnom1}' AND tipnom = '{$_SESSION['codigo_nomina']}') as periodo_ini,
			       (SELECT periodo_fin FROM nom_nominas_pago WHERE codnom='{$codnom2}' AND tipnom = '{$_SESSION['codigo_nomina']}') as periodo_fin";
	$res = $db->query($sql);

	if($obj = $res->fetch_object())
	{
		$periodo_ini = new DateTime($obj->periodo_ini);
		$periodo_ini->setTime(0,0,0);
		$periodo_fin = new Datetime($obj->periodo_fin);
	...
	$sql = "SELECT DATE_FORMAT(periodo_ini, '%M') as nom_mes, DATE_FORMAT(periodo_ini, '%Y') as anio, mes
			...
			WHERE  codnom='{$codnom1}' AND tipnom = '{$_SESSION['codigo_nomina']}'";
	$res = $db->query($sql);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'SIPE ' . strtoupper($mes_planilla) . ' ' . $anio_planilla )
								  ...
								  ->setCellValue('L3', 'Uso Auto')
								  ->setCellValue('M3', strtoupper($mes_planilla))
								  ->setCellValue('N3', 'ISR')
								  ->setCellValue('O3', 'S.E.')
								  ->setCellValue('P3', 'S.S.')
								  ->setCellValue('Q3', 'Fecha de Vacaciones')
								  ->setCellValue('R3', 'Monto Vacaciones')
								  ->setCellValue('S3', 'Vacaciones Proporcionales')
								  ->setCellValue('T3', 'XIII MES Proporcional')
								  ->setCellValue('U3', 'Prima de Antigüedad')
								  ->setCellValue('V3', 'Indem/prima/preaviso')
								  ->setCellValue('W3', 'Observación');

	cellColor('A1:P2',  '0F243E');  // Fondo Azul Oscuro	
	colorTexto('A1:W2', 'FFFFFF');  // Texto Blanco
	...
	$objPHPExcel->getActiveSheet()->mergeCells('T2:W2');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A1:W2')->getFont()->setName('Calibri');

	$objPHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setSize(12);


	$objPHPExcel->getActiveSheet()->getStyle('A1:W3')->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getStyle('A1:W3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

	$objPHPExcel->getActiveSheet()->getStyle('A2:P2')->applyFromArray(borderTop(PHPExcel_Style_Border::BORDER_MEDIUM));
	$objPHPExcel->getActiveSheet()->getStyle('H2')->applyFromArray(borderRight(PHPExcel_Style_Border::BORDER_MEDIUM));
	$objPHPExcel->getActiveSheet()->getStyle('Q2')->applyFromArray(borderRight(PHPExcel_Style_Border::BORDER_MEDIUM));
	$objPHPExcel->getActiveSheet()->getStyle('U2')->applyFromArray(borderRight(PHPExcel_Style_Border::BORDER_MEDIUM));

	$objPHPExcel->getActiveSheet()->getStyle('A3:W3')->applyFromArray(allBorders());
	$objPHPExcel->getActiveSheet()->getStyle('A3:W3')->getAlignment()->setWrapText(true); 
...
'isr') );
	...
	while($codnivel1 = $res->fetch_object())
	{
		if($codnivel1->codorg==2)
		{
			...
			cellColor('A'.$i.':W'.$i,  '0F243E');
			colorTexto('A'.$i.':W'.$i, 'FFFFFF');
		...
		$sql1 = "SELECT  p.ficha, p.cedula, p.suesal, p.fecing, p.estado,
				 ...
				 AND     p.ficha IN (SELECT DISTINCT ficha FROM nom_movimientos_nomina WHERE codnom IN ({$codnom1},{$codnom2}) AND tipnom = '{$_SESSION['codigo_nomina']}')";
		...
		while($personal = $res1->fetch_object())
		{
			...
			foreach($conceptos as $concepto)
			{
				...
				if($concepto["variable"]=='regular'  ||  $concepto["variable"]=='nacional')
				{
					$sql2 .= "SELECT COALESCE(SUM(n.valor),0) as horas, COALESCE(SUM(n.monto),0) as total, '{$concepto["variable"]}1' as variable
							  ...
							  WHERE  n.codnom='{$codnom1}' AND tipnom = '{$_SESSION['codigo_nomina']}' AND n.ficha='{$personal->ficha}' 
							  ...
							  FROM   nom_movimientos_nomina n
							  WHERE  n.codnom='{$codnom2}' AND tipnom = '{$_SESSION['codigo_nomina']}' AND n.ficha='{$personal->ficha}' 
							  AND    {$concepto["condicion"]} ";					
				}
				else
				{
					$sql2 .= "SELECT COALESCE(SUM(n.valor),0) as horas, COALESCE(SUM(n.monto),0) as total, 
							  ...
							  FROM   nom_movimientos_nomina n
							  WHERE  n.codnom IN ({$codnom1}, {$codnom2}) AND n.ficha='{$personal->ficha}'  AND tipnom = '{$_SESSION['codigo_nomina']}'  AND    {$concepto["condicion"]} ";
				}
			}
			...
			$SQL = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (106,107,108,109,112,113,115,116,119,120,121,122,123,124,125,125,126,127,128,129,130,131,132,133,134,135,136,136,139,142,143,144,149,150,151,152,153,154,628) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_horas_extras = $db->query($SQL)->fetch_assoc();
			$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_xiii = $db->query($SQL2)->fetch_assoc();
			$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (101) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_xiii_gp = $db->query($SQL2)->fetch_assoc();
			$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (157) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_comision = $db->query($SQL2)->fetch_assoc();

			$SQL3 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_xiii2 = $db->query($SQL3)->fetch_assoc();

			$SQL4 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (93) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_prima = $db->query($SQL4)->fetch_assoc();

			$SQL5 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_vac = $db->query($SQL5)->fetch_assoc();
			$SQL6 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (145) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			$total_gr = $db->query($SQL6)->fetch_assoc();
			$SQL7 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (156) AND codnom IN ({$codnom1}, {$codnom2}) AND  tipnom = '{$_SESSION['codigo_nomina']}'";
			...
			if($en_vacaciones)
			{
				...
				COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
					WHERE  ficha='{$personal->ficha}' AND n.codnom in ($codigo) AND n.tipnom = '{$_SESSION['codigo_nomina']}' AND n.codcon in (114)), 0) as monto_vac,
				COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
					WHERE  ficha='{$personal->ficha}' AND n.codnom in ($codigo) AND n.tipnom = '{$_SESSION['codigo_nomina']}' AND n.codcon in (117)), 0) as prop_vac";
			...
			->setCellValue('L'.$i, ($total_ua['monto']==0 ? '' : $total_ua['monto']))
			->setCellValue('M'.$i, "=D".$i."+E".$i."+F".$i."+G".$i."+H".$i."+I".$i."+J".$i."+K".$i."+L".$i)
			->setCellValue('U'.$i, ($total_prima['monto']==0 ? '' : $total_prima['monto']))
			->setCellValue('N'.$i, ($total_isr==0 ? '' : $total_isr))
			->setCellValue('O'.$i, ($total_se ? $total_se : '' ))
			->setCellValue('P'.$i, ($total_ss ? $total_ss : '' ))
			->setCellValue('Q'.$i, ($en_vacaciones ? $nombre_periodo : '' ))
			->setCellValue('R'.$i, ($total_vac['monto']==0     ? '' : $total_vac['monto']))
			->setCellValue('S'.$i, ($total_vac_liq['monto']==0     ? '' : $total_vac_liq['monto']))
			->setCellValue('T'.$i, ($total_xiiimes_liq==0    ? '' : $total_xiiimes_liq))
			->setCellValue('W'.$i, ($$vac_sql=="" ? '' : $$vac_sql));
	...
	if($i>4)
	{
		$objPHPExcel->getActiveSheet()->getStyle('A4:W'.($i+5))->applyFromArray(allBorders());

		$objPHPExcel->getActiveSheet()->getStyle('F4:F'.($i-1))->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle('M4:M'.($i-1))->getFont()->setBold(true);
		...
		$objPHPExcel->getActiveSheet()->getStyle('K'.($i+5).':M'.($i+5))->getNumberFormat()->setFormatCode('#,##0.00');
		...
		$objPHPExcel->getActiveSheet()->getStyle('L4:P'.($i+6))->getNumberFormat()->setFormatCode('"B/." * #,##0.00');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F'.($i+5), '=SUM(F4:F'.($i+4).')')
									  ...
									  ->setCellValue('P'.($i+5), '=SUM(P4:P'.($i+4).')')
									  ->setCellValue('G'.($i+6), '=G'.($i+5).'+N'.($i+5))
									  ->setCellValue('H'.($i+6), '=SUM(H'.($i+5).'+I'.($i+5).'+J'.($i+5).')')
									  ->setCellValue('M'.($i+6), '=M'.($i+5).'-N'.($i+5).'-O'.($i+5).'-P'.($i+5));
	...
	$sql3 = "SELECT
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (3000)), 0) as ss_patronal,
	COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (3001)), 0) as se_patronal,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (3002)), 0) as ss_xiii_patronal,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (9009)), 0) as riesgo,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (201)), 0) as ss_emp,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (202)), 0) as se_emp,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codnom1,$codnom2) AND n.tipnom = '{$codtip}' AND n.codcon in (205)), 0) as ss_xiii_emp";
	...
	$total_neto = $objPHPExcel->getActiveSheet()->getCell("M".($i+6))->getValue();;
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$ixx, '=P'.($ixx-4));
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$ixx, '=O'.($ixx-6));
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$ixx, '=D'.$ixx.'+E'.$ixx);
	$ixx++;
	$objPHPExcel->getActiveSheet()->setCellValue('C'.$ixx, 'RIESGO PROF.');
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$ixx, '=M'.($ixx-6).'*0.021');
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$ixx, '=D'.($ixx));
	$ixx++;
	$objPHPExcel->getActiveSheet()->setCellValue('C'.$ixx, 'ISR');
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$ixx, '=N'.($ixx-8));
