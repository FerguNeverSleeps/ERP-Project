...
{	
	...
	$strsql= "select COUNT(*) from $nombre_tabla where tipnom=$codigo_nomina and (estado='Activo' or estado='REGULAR') ";
	$result =sql_ejecutar($strsql);	
	$fila = mysqli_fetch_array($result);	
	$num_total_registros = $fila[0];
	
	$strsql= "select * from $nombre_tabla where tipnom=$codigo_nomina and (estado='Activo' or estado='REGULAR') order by $campo_1,$campo_2 LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
?>
