<?php 
// require_once '../../generalp.config.inc.php';
session_start();
ob_start();

require_once '../lib/common.php';
include ("func_bd.php");
// error_reporting(E_COMPILE_ERROR|E_ERROR|E_CORE_ERROR);
$conexion = conexion();

$registro_id=$_POST['registro_id']; 
$op=$_POST['op'];

if (isset($_GET['nivel']))
{
    $nivel=$_GET['nivel'];
}
else
{
    $nivel=$_POST['nivel'];
}

$tabla_nivel = "nomnivel".$nivel;
$tabla_nivel_anterior="nomnivel".($nivel-1);
// obtiene el nombre del nivel por como este configurado
// en la table empresas
$query = "SELECT nomniv".$nivel." FROM nomempresa";
$res = query($query, $conexion);
$fila = fetch_array($res); 
$nombre_nivel = $fila[0];
if ($op==3) //Se presiono el boton de Eliminar
{ 
  $query = "DELETE FROM $tabla_nivel WHERE codorg=$registro_id";  
    
  $res = query($query, $conexion);
  activar_pagina("subniveles.php?nivel=".$nivel);    
}     
else         
{  
  if($nivel == "1"){
    $strsql = "SELECT * FROM $tabla_nivel a LEFT JOIN subsidiaria s ON a.id_subsidiaria = s.id_subsidiaria ORDER BY codorg,descrip";
    $res = query($strsql, $conexion);

  }else{
    $strsql = "SELECT * FROM $tabla_nivel ORDER BY codorg,descrip";
    $res = query($strsql, $conexion);

  }
}
?>
<?php include("../header4.php"); // <html><head></head><body> ?>
<link href="../../includes/assets/css/custom-datatables.css" rel="stylesheet" type="text/css"/>
<script type="text/javascript">
function enviar(op,id){
  
  if (op==1){   // Opcion de Agregar
    document.frmPrincipal.registro_id.value=id;
    document.frmPrincipal.op.value=op;
    document.frmPrincipal.action="ag_subniveles.php";
    document.frmPrincipal.submit(); 
  }
  if (op==2){   // Opcion de Modificar  
    document.frmPrincipal.registro_id.value=id;   
    document.frmPrincipal.op.value=op;
    document.frmPrincipal.action="ag_subniveles.php";
    document.frmPrincipal.submit();   
  }
  if (op==3){   // Opcion de Eliminar
    if (confirm("\u00BFEst\u00E1 seguro que desea eliminar el registro ?"))
    {         
      document.frmPrincipal.registro_id.value=id;
      document.frmPrincipal.op.value=op;
      //document.frmPrincipal.nivel.value=nivel;
      document.frmPrincipal.submit();
    }   
  }
}
</script>
<div class="page-container">
  <!-- BEGIN SIDEBAR -->
  <!-- END SIDEBAR -->
  <!-- BEGIN CONTENT -->
  <div class="page-content-wrapper">
    <div class="page-content">
      <!-- BEGIN PAGE CONTENT-->
      <div class="row">
        <div class="col-md-12">
          <!-- BEGIN EXAMPLE TABLE PORTLET-->
          <div class="portlet box blue">
            <div class="portlet-title">
              <div class="caption">
                Subniveles de <?php echo $nombre_nivel; ?>
              </div>
              <div class="actions">
                <a class="btn btn-sm blue"  onclick="javascript: enviar(1,0);">
                  <i class="fa fa-plus"></i> Agregar
                  <?php // btn('add','enviar(1,0);',2); ?>
                </a>
                <a class="btn btn-sm blue"  onclick="javascript: window.location='niveles_funcionales.php'">
                  <i class="fa fa-arrow-left"></i> Regresar
                </a>
              </div>
            </div>
            <div class="portlet-body">
              <form action="" method="post" name="frmPrincipal" id="frmPrincipal"><?
              if ($nivel>1) {
              ?>
              <table class="table table-striped table-bordered table-hover" id="table_datatable">
              <thead>
              <tr>
                <th>C&oacute;digo</th>
                <?php 
                if ($nivel>1) {
                  ?>
                  <th>Región</th> 
                  <?php
                }
                
                ?>
                <th>Descripci&oacute;n</th>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
              </tr>
              </thead>
              <tbody>
              <?php
                while( $fila = fetch_array($res) )
                {              
                ?>
                  <tr class="odd gradeX">
                    <td><?php echo $fila['codorg']; ?></td>
                      <?php 
                      if($nivel>1)
                      {
                        $sql="select * from $tabla_nivel_anterior where codorg=".$fila[gerencia];
                        $res2 = query($sql, $conexion);
                        $columna = fetch_array($res2);
                    ?><td><?php

                         echo utf8_encode($columna['descrip']); ?>
                        </td><?php
                      }
                        
                    ?>
                    <td><?php echo utf8_encode($fila['descrip']); ?></td>
                    <td style="text-align: center">
                        <a href="javascript:enviar(<?php echo(2); ?>,'<?php echo($fila[codorg]); ?>');" title="Editar">
                        <img src="../../includes/imagenes/icons/pencil.png" alt="Editar" width="16" height="16">
                        </a>
                    </td>
                    <td style="text-align: center">
                        <a href="javascript:enviar(<?php echo(3); ?>,<?php echo($fila[codorg]); ?>);" title="Eliminar">
                        <img src="../../includes/imagenes/icons/delete.png" alt="Eliminar" width="16" height="16">
                        </a>   
                    </td>
                  </tr>
                  <?php                 
                }
              ?>
              </tbody>
              </table>
              <?php  
              }
              else
              {?>
                <table class="table table-striped table-bordered table-hover" id="table_datatable2">
              <thead>
              <tr>
                <th>C&oacute;digo</th>                
                <th>Descripci&oacute;n</th>
                <?php if($nivel == '1'){ ?>
                <th>Subsidiaria</th>
                <?php } ?>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
              </tr>
              </thead>
              <tbody>
              <?php
                while( $fila = fetch_array($res) )
                {              
                ?>
                  <tr class="odd gradeX">
                    <td><?php echo $fila['codorg']; ?></td>                      
                    <td><?php echo utf8_encode($fila['descrip']); ?></td>
                    <?php if($nivel == '1'){ ?>
                      <td><?php echo utf8_encode($fila['descripcion']); ?></td>
                    <?php } ?>
                    <td style="text-align: center">
                        <a href="javascript:enviar(<?php echo(2); ?>,'<?php echo($fila[codorg]); ?>');" title="Editar">
                        <img src="../../includes/imagenes/icons/pencil.png" alt="Editar" width="16" height="16">
                        </a>
                    </td>
                    <td style="text-align: center">
                        <a href="javascript:enviar(<?php echo(3); ?>,<?php echo($fila[codorg]); ?>);" title="Eliminar">
                        <img src="../../includes/imagenes/icons/delete.png" alt="Eliminar" width="16" height="16">
                        </a>   
                    </td>
                  </tr>
                  <?php                 
                }
              ?>
              </tbody>
              </table>
                <?php
              }
              ?>
              
                    <input name="nivel" type="hidden" value="<?php echo $nivel; ?>">
                    <input name="registro_id" type="hidden" value="">
                    <input name="op" type="hidden" value="">
              </form>
            </div>
          </div>
          <!-- END EXAMPLE TABLE PORTLET-->
        </div>
      </div>
      <!-- END PAGE CONTENT-->
    </div>
  </div>
  <!-- END CONTENT -->
</div>
<?php include("../footer4.php"); ?>
<script type="text/javascript">
   $(document).ready(function() { 

    nivel=$("#nivel").val();
    if(nivel==1)
    {
      $('#table_datatable').DataTable({
              "iDisplayLength": 10,
              "sPaginationType": "bootstrap_extended", 
                "oLanguage": {
                  "sSearch": "<img src='../../includes/imagenes/icons/magnifier.png' width='16' height='16' > Buscar:",
                    "sLengthMenu": "Mostrar _MENU_",
                    //"sInfo": "Showing page _PAGE_ of _PAGES_", // Mostrando 1 to 5 de 18 entradas
                    //"sInfoEmpty": "No hay registros para mostrar",
                    "sInfoEmpty": "",
                    // "sInfo": "",
                    "sInfo":"Total _TOTAL_ registros",
                    "sInfoFiltered": "",
                    "sEmptyTable":  "No hay datos disponibles",//"No data available in table",
                    "sZeroRecords": "No se encontraron registros",//"No matching records found",
                    "oPaginate": {
                        "sPrevious": "P&aacute;gina Anterior",//"Prev",
                        "sNext": "P&aacute;gina Siguiente",//"Next",
                        "sPage": "P&aacute;gina",//"Page",
                        "sPageOf": "de",//"of"
                    }
                }, 
                "aLengthMenu": [ // set available records per page
                    [5, 10, 25, 50, -1],
                    [5, 10, 25, 50, "Todos"]
                ],   
                "aoColumnDefs": [
                    { 'bSortable': false, 'aTargets': [3] },
                    { "bSearchable": false, "aTargets": [ 3 ] },
                    { 'bSortable': false, 'aTargets': [4] },
                    { "bSearchable": false, "aTargets": [ 4 ] },
                    { "sWidth": "8%", "aTargets": [0] },
                    { "sWidth": "8%", "aTargets": [3] },
                    { "sWidth": "8%", "aTargets": [4] }
                ],          
         "fnDrawCallback": function() {
                $('#table_datatable_filter input').attr("placeholder", "Escriba frase para buscar");
         }
        });

        $('#table_datatable').on('change', 'tbody tr .checkboxes', function(){
             $(this).parents('tr').toggleClass("active");
        });
    }
    else
    {
      $('#table_datatable2').DataTable({
          "iDisplayLength": 10,
          "sPaginationType": "bootstrap_extended", 
            "oLanguage": {
              "sSearch": "<img src='../../includes/imagenes/icons/magnifier.png' width='16' height='16' > Buscar:",
                "sLengthMenu": "Mostrar _MENU_",
                //"sInfo": "Showing page _PAGE_ of _PAGES_", // Mostrando 1 to 5 de 18 entradas
                //"sInfoEmpty": "No hay registros para mostrar",
                "sInfoEmpty": "",
                // "sInfo": "",
                "sInfo":"Total _TOTAL_ registros",
                "sInfoFiltered": "",
                "sEmptyTable":  "No hay datos disponibles",//"No data available in table",
                "sZeroRecords": "No se encontraron registros",//"No matching records found",
                "oPaginate": {
                    "sPrevious": "P&aacute;gina Anterior",//"Prev",
                    "sNext": "P&aacute;gina Siguiente",//"Next",
                    "sPage": "P&aacute;gina",//"Page",
                    "sPageOf": "de",//"of"
                }
            }, 
            "aLengthMenu": [ // set available records per page
                [5, 10, 25, 50, -1],
                [5, 10, 25, 50, "Todos"]
            ],   
            "aoColumnDefs": [
                    { 'bSortable': false, 'aTargets': [2] },
                    { "bSearchable": false, "aTargets": [2] },
                    { 'bSortable': false, 'aTargets': [3] },
                    { "bSearchable": false, "aTargets": [3] },
                    { "sWidth": "8%", "aTargets": [2] },
                    { "sWidth": "8%", "aTargets": [3] }
                
            ],          
           "fnDrawCallback": function() {
                  $('#table_datatable_filter input').attr("placeholder", "Escriba frase para buscar");
           }
        });

        $('#table_datatable2').on('change', 'tbody tr .checkboxes', function(){
             $(this).parents('tr').toggleClass("active");
        });

    }


            $('#table_datatable_wrapper .dataTables_filter input').addClass("form-control input-medium input-inline"); 
            $('#table_datatable_wrapper .dataTables_length select').addClass("form-control input-xsmall"); 
   });
</script>
</body>
</html>