...
function buscarFichaEnter(event){
  if(event.keyCode==13){
    var ficha=document.getElementById("textfield").value;
    window.location.href="?ficha="+ficha+'&codt='+document.frmPrincipal.codt.value+'&codigo_nomina='+document.frmPrincipal.codigo_nomina.value+"&pagina=1";
  }
}
...
function enviar(op,id,ficha){
  ...
  if (op==2){   // Opcion de Modificar
    //alert($op);   
    document.location.href="movimientos_liquidaciones_editar.php?nomina="+document.frmPrincipal.txtnomina.value+"&codt="+document.frmPrincipal.codt.value+"&pagina="+document.frmPrincipal.pagina.value+"&concepto="+id+"&ficha="+document.frmPrincipal.textfield.value+"&accion=modificar";
	...
	if (op==3){		// Opcion de Eliminar
		...
		{					
			...
			document.frmPrincipal.op.value=op;
			document.location.href="movimientos_nomina_liquidaciones_eliminar.php?nomina="+document.frmPrincipal.txtnomina.value+"&concepto="+id+"&ficha="+document.frmPrincipal.textfield.value+"&pagina="+document.frmPrincipal.pagina.value+"&liquidaciones=1";
		}		
	}
	...
	if (op==5){		// Opcion de Eliminar
		...
		{					
			document.frmPrincipal.registro_id.value=id;
			document.frmPrincipal.op.value=op;
			document.location.href="movimientos_nomina_pago_eliminar.php?todo=1&auxlia=1&nomina="+document.frmPrincipal.txtnomina.value+"&concepto="+id+"&ficha="+document.frmPrincipal.textfield.value+"&pagina="+document.frmPrincipal.pagina.value
		}		
	}
	...
	{	
		...
		document.frmPrincipal.registro_id.value=id;
		document.frmPrincipal.codigo_nomina.value=nomina;
		AbrirVentana('barraprogreso_liq_vac.php?registro_id='+id+'&codigo_nomina='+nomina+'&ficha='+ficha+'&pagina='+pag,180,480,0);
	}

  function enviar3(id,nomina,ficha,pag)
  { 
    // Generar Nómina
    document.frmPrincipal.registro_id.value=id;
    document.frmPrincipal.codigo_nomina.value=nomina;
    ...
    AbrirVentana('movimientos_nomina_persona_recalcular.php?todo=1&ficha='+ficha+'&nomina='+id+"&pagina2="+pag+"&nombre="+document.frmPrincipal.txtnombre.value+"&cedula="+document.frmPrincipal.txtcedula.value,300,900,0);
  }

	function liquidacion(tiponom,ficha)
	{
		AbrirVentana('liquidacion.php?tipo_nomina='+tiponom+'&ficha='+ficha,460,600,0);
	}
</script>
...
$ficha=isset($_REQUEST["ficha"])?$_REQUEST["ficha"]:"";

$url="movimientos_nomina_liquidaciones";
if(!$ficha)
{
	$consulta= "select pe.foto as foto, pe.ficha as ficha, pe.apenom as apenom, pe.cedula as cedula, pe.suesal as suesal, pe.codnivel1 as codnivel1, pe.codnivel2 as codnivel2, pe.estado as estado, pe.codnivel3 as codnivel3, pe.fecing as fecing, ca.des_car as cargo 
	from nompersonal as pe 
	join nomcargos as ca on(pe.codcargo=ca.cod_car) 
	where 
	pe.ficha in (select ficha from nom_movimientos_nomina where tipnom='".$_SESSION['codigo_nomina']."' and codnom='".$cod_nomina."')  order by ficha";

 //buscar en que pagina corresponde la ficha, cuando se ejecuta el barragrogreso_liq_vac.php
  $resultado_personal=query($consulta,$conexion);
  $m=0;
  while($fila_personal=fetch_array($resultado_personal)) {
    $m++;
    if($fila_personal["ficha"]==$_REQUEST["irficha"]){
      $pagina=$m;
      break;
    }
  }
...
else
{
  ...
  where pe.tipnom='".$_SESSION['codigo_nomina']."' and pe.ficha='".$ficha."' ";
}
...
{
  $btn_eliminar=$btn_agregar="";
  if($_SESSION["planilla_liquidacion_detalle_agregar"])
    $btn_agregar="|btn('add','enviar(1,0)',2);";
  if($_SESSION["planilla_liquidacion_detalle_eliminar"])
    $btn_eliminar="|btn_mejorada('Borrar','enviar(5)','delete.gif');";

	titulo_mejorada("Movimientos de $termino Nº: ".$cod_nomina.", Fecha de Inicio: ".fecha($fila_nomina[periodo_ini]).", Fecha Fin: ".fecha($fila_nomina[periodo_fin]),"","btn('print','VerRecibo();',2,'Imprimir Recibo');$btn_agregar $btn_eliminar","$dir");
}
else
{
...
<input type="hidden" name="ficha" id="ficha" value="<? echo $ficha;?>">
...
<tr><TD colspan="2"><br></TD></tr>
    ...
    <tr class="">
      ...
      <td><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
		...
		value="<?php echo $fila_personal['ficha']?>" size="20" onkeypress="buscarFichaEnter(event)"></font></td>
...
<TD></TD></TR></table>
    ...
    <tr class="">
      <td>&nbsp;</td>
      <td colspan="3"><font size="2" face="Arial, Helvetica, sans-serif" style="white-space: nowrap;">
        <br>
        <?php if($fila_nomina['status']=="A" and $_SESSION['planilla_liquidacion_detalle_generar']){?>
          <a href="javascript:enviar2(<?php echo($cod_nomina); ?>,<?php echo $_SESSION['codigo_nomina']?>,<?php echo($fila_personal['ficha']); ?>,<?php echo($pagina); ?>);"><img src="img_sis/ico_propiedades.gif" title="Generar <?echo $termino?>" width="15" height="15" border="0" align="absmiddle" > Generar Liquidaci&oacute;n</a>&nbsp;&nbsp;&nbsp;
          <div style="width: 100%; height: 5px;"></div>
          <a href="javascript:enviar3(<?php echo($cod_nomina); ?>,<?php echo $_SESSION['codigo_nomina']?>,<?php echo($fila_personal['ficha']); ?>,<?php echo($pagina); ?>);"><img src="img_sis/ico_propiedades.gif" title="Recalcular <?echo $termino?>" width="15" height="15" border="0" align="absmiddle" > Recalcular</a>

          <?php } ?>
        </font></td>
	<td><font size="2" face="Arial, Helvetica, sans-serif"><font size="2" face="Arial, Helvetica, sans-serif"><?php if($fila_nomina['status']=="A"  and $_SESSION['planilla_liquidacion_detalle_editar']){?><a href="javascript:liquidacion(<?php echo $_SESSION['codigo_nomina']?>,<?php echo($fila_personal['ficha']); ?>);"><img src="images/save.gif" title="Datos de liquidaci&oacute;n" width="15" height="15" border="0" align="absmiddle" > &nbsp;&nbsp;Datos de liquidaci&oacute;n</a><?php } ?></font></td>
      <td><div align="center">
...
<table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
    <tr class="tb-head"> 
        ...
        <td width="41%" ><div align="left"><font size="2" face="Arial, Helvetica, sans-serif">Descripci&oacute;n</font></div></td>
	...
	if ($fila[tipcon]=='D')
      ...
      <td ><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><?if($fila_nomina['status']=="A" and $_SESSION["planilla_liquidacion_detalle_editar"]){?> <a href="javascript:enviar(<?php echo(2); ?>,<?php echo($fila['codcon']); ?>);"><img src="../imagenes/edit.gif" alt="Edita el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a><?}?></font></div></td>      

      <td ><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><?if($fila_nomina['status']=="A" and $_SESSION["planilla_liquidacion_detalle_eliminar"]){?> <a href="javascript:enviar(<?php echo(3); ?>,<?php echo($fila['codcon']); ?>);"><img src="../imagenes/delete.gif" alt="Elimina el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a><?}?></font></div></td>
    </tr>
    <?php   
  	}
}
...
<?
//paginacion
pie_pagina($url,$pagina,"codigo_nomina=".$cod_nomina."&codt=".$codt,$num_paginas);
...
$resultado_movimientos=query($consulta,$conexion);
	...
	*/
      ...
      <td   ><div align="center"><? if($fila_nomina['status']=="A" and $_SESSION["planilla_liquidacion_editar"]){?><font size="2" face="Arial, Helvetica, sans-serif"> <a href="javascript:enviar(<?php echo(3); ?>,<?php echo($fila['codcon']); ?>);"><img src="../imagenes/delete.gif" title="Elimina el Registro Actual" alt="Elimina el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a></font><?}?></div></td>
    </tr>
    <?php   
  	}
}
