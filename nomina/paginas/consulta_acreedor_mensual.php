...
$(document).ready(function()
{
	...
	function nomina(anio)
	{
		$.get("ajax/obtenerMesNomina.php",{anio:anio},function(res)
		{
			...
			{
				mes = $("#mes").val();
				$.get("ajax/obtenerPlanillaMesMultiple.php",{anio:anio, mes:mes},function(res1){
					$("#quincena_box").empty();
					$("#planilla_box").empty();
					...
					tipos = $("select#tipos").val();
						...
						else
						{
							...
							var url = "rpt_acreedor_mensual.php?mes="+mes+"&anio="+anio+"&tipos="+tipos; // the script where you handle the form input.
							window.open(url);
						}	
