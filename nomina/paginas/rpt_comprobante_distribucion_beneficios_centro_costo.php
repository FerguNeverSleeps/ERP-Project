...
$ln+=2;
$sheet->mergeCells("A{$ln}:F{$ln}");
$sheet->setCellValue("A{$ln}", "DISTRIBUCIÓN DE OTROS BENEFICIOS (OPERATIVOS)");	
...
$filename = "comprobante_distribucion_beneficios_centro_costo_".$anio."_".$mes;
...
for($i=0; $i<count($centro_costo); $i++){ 
	...
	$ln+=2;
	$sheet->mergeCells("A{$ln}:F{$ln}");
	$sheet->setCellValue("A{$ln}", "DISTRIBUCIÓN DE OTROS BENEFICIOS - ".$centro_costo[$i]["descrip"]);	
	$sheet->getStyle("A{$ln}:F{$ln}")
	        ->getFill()
	        ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
	        ->getStartColor()
