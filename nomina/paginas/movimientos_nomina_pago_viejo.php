	...
	switch($tipob){
		case "exacta": 
			$consulta = "SELECT pe.foto as foto, pe.ficha as ficha, pe.apenom as apenom, pe.cedula as cedula, pe.suesal as suesal, 
			                    pe.codnivel1 as codnivel1, pe.codnivel2 as codnivel2, pe.estado as estado  ,pe.codnivel3 as codnivel3, 
			                    pe.fecing as fecing, ca.des_car as cargo 
			             FROM nompersonal as pe 
			             LEFT JOIN nomcargos as ca on(pe.codcargo=ca.cod_car) 
			             WHERE pe.tipnom='{$_SESSION['codigo_nomina']}'"; //buscar_exacta($tabla,$des,$busqueda);
			break;
		/*case "todas":
...
else
{
	if($_GET['vac']==1  )
	{
		$consulta= "select pe.foto as foto, pe.ficha as ficha, pe.apenom as apenom, pe.cedula as cedula, pe.suesal as suesal, pe.codnivel1 as codnivel1, pe.codnivel2 as codnivel2, pe.estado as estado, pe.codnivel3 as codnivel3, pe.codnivel4 as codnivel4, pe.codnivel5 as codnivel5, pe.codnivel6 as codnivel6, pe.codnivel7 as codnivel7, pe.fecing as fecing, ca.des_car as cargo from nompersonal as pe left join nomcargos as ca on(pe.codcargo=ca.cod_car) inner join nom_progvacaciones as vac on pe.ficha=vac.ficha where pe.estado <> 'Egresado' and vac.fechavac<>'0000-00-00' and vac.fechareivac<>'0000-00-00' and vac.estado='Pendiente' and pe.tipnom='".$_SESSION['codigo_nomina']."' and ((pe.fecharetiro>='".$lista['periodo_ini']."' and pe.fecharetiro<='".$lista['periodo_fin']."') or pe.fecharetiro='0000-00-00') and pe.estado='Activo' group by pe.cedula";
		...
		$consulta ="SELECT pe.foto AS foto, pe.ficha AS ficha, pe.apenom AS apenom, pe.cedula AS cedula, pe.suesal AS suesal, 
					...
					WHERE pe.estado <> 'Egresado' AND pe.tipnom='{$_SESSION['codigo_nomina']}'
					AND ((pe.fecharetiro>='{$lista['periodo_ini']}' AND pe.fecharetiro<='{$lista['periodo_fin']}') OR pe.fecharetiro='0000-00-00')
		...
		if($_SESSION['codigo_nomina']!=2){
		 ...
		 $consulta= "SELECT pe.foto as foto, pe.ficha as ficha, pe.apenom as apenom, pe.cedula as cedula, pe.suesal as suesal, 
			                   pe.codnivel1 as codnivel1, pe.codnivel2 as codnivel2, pe.estado as estado, pe.codnivel3 as codnivel3, 
			                   pe.codnivel4 as codnivel4, pe.codnivel5 as codnivel5, pe.codnivel6 as codnivel6, pe.codnivel7 as codnivel7, 
			                   pe.fecing as fecing, ca.des_car as cargo , n1.descrip as nivel1
			            FROM nompersonal as pe 
			            LEFT JOIN nomcargos as ca ON (pe.codcargo=ca.cod_car)
			            LEFT JOIN nomnivel1 as n1 ON (pe.codnivel1= n1.codorg) 
			            WHERE pe.tipnom='{$_SESSION['codigo_nomina']}'";
		}else{
...
<table class="tb-head" width="100%">
  <tr>
	<td><input type="text" name="buscar" size="20" style="color: black"></td>
	<td><SELECT name="busqueda" style="color: black">
		<option value="ficha">C&oacute;digo ficha</option>
		<!--<option value="Denominacion">Nombre de Acción Especifica</option>-->
     </SELECT></td>
	<td><?php btn('search',"frmPrincipal",1); ?></td>
	<td>
	<?php
		$url_show_all = $url.".php?pagina=".$pagina."&codigo_nomina=".$cod_nomina."&codt=".$codt;

		if(isset($_GET["vac"]))
		{
			$url_show_all = $url.".php?pagina=".$pagina."&codigo_nomina=".$cod_nomina."&codt=".$codt."&vac=1";
		}

		btn('show_all', $url_show_all); 
	?>
	</td>
...
<tr class="">
      ...
      <td colspan="4"><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
        <label>
        <input name="niv_funcional" type="text" readonly="true" id="niv_funcional" value="<?echo $fila_personal[nivel1] . "-" . $fila_personal['codnivel2'] ;?>">
        </label>
      </font></td>
      <td><label></label></td>
    </tr>
...
<table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
	...
	if ($fila[tipcon]=='D')
      ...
      <td ><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><?if($fila_nomina['status']=="A"){?> <a href="javascript:enviar(<?php echo(2); ?>,<?php echo($fila['codcon']); ?>);"><img src="../imagenes/edit.gif" alt="Edita el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a><?}?></font></div></td>
