...
if ($respuestaPermitido->num_rows == 1) {
        ...
        $fechaInicio = new DateTime($periodo_ini);
        $fechaFin = new DateTime($periodo_fin);

        $diasDiferencia = $fechaFin->diff($fechaInicio)->days;

        if ($diasDiferencia > 119) {
                //Colocamos el sql modificado
                $nuevaConsulta = "SELECT
                        nmn.cedula,
                        npcc.codigo_proyecto_cc,
                        COALESCE(npcc.porcentaje, '100.00') AS porcentaje,
                        COALESCE(LEFT(pcc.cuenta_contable, 2), LEFT(nc.ctacon, 2)) AS mayorCtaCon,
                        COALESCE(pcc.cuenta_contable, nc.ctacon) AS cuenta_contable,
                        nmn.monto,
                        nmn.ficha,
                        nmn.codcon,
                        nmn.descrip,
                        ...
                        nc.ccosto
                FROM 
                        nom_movimientos_nomina AS nmn
                        INNER JOIN nom_nominas_pago nmp ON nmp.codnom = nmn.codnom AND nmp.anio = nmn.anio AND nmp.mes = nmn.mes
                        LEFT JOIN nompersonal_pry_cc AS npcc ON npcc.cedula = nmn.cedula 
                                AND npcc.fecha_inicio <= nmp.periodo_fin
                                AND (npcc.fecha_fin >= nmp.periodo_ini OR npcc.fecha_fin IS NULL)
                        LEFT JOIN proyecto_cc pcc ON npcc.codigo_proyecto_cc = pcc.codigo
                        INNER JOIN nomconceptos nc ON nc.codcon = nmn.codcon
                WHERE 
                        nmn.codnom = '{$codnom}' 
                        AND nmp.periodo_ini = {$periodo_ini} 
                        AND nmp.periodo_fin = '{$periodo_fin}'
                ORDER BY 
                        nmn.cedula, 
                        npcc.codigo_proyecto_cc, 
                        nmn.codcon ASC";
        } else {
                //Colocamos el sql que ya existe y es funcional
                $nuevaConsulta = "SELECT
                                nmn.cedula,
                                npcc.codigo_proyecto_cc,
                                COALESCE(npcc.porcentaje, '100.00') AS porcentaje,
                        ...
                        FROM 
                                ...
                                INNER JOIN nom_nominas_pago nmp ON nmp.codnom = nmn.codnom AND nmp.anio = nmn.anio AND nmp.mes = nmn.mes
                                LEFT JOIN nompersonal_pry_cc AS npcc ON npcc.cedula = nmn.cedula 
                                AND nmp.periodo_ini BETWEEN npcc.fecha_inicio AND npcc.fecha_fin
                                AND nmp.periodo_fin BETWEEN npcc.fecha_inicio AND npcc.fecha_fin
                                LEFT JOIN proyecto_cc pcc ON npcc.codigo_proyecto_cc = pcc.codigo
                                INNER JOIN nomconceptos nc ON nc.codcon = nmn.codcon
                        WHERE 
                                ...
                                AND nmp.periodo_ini = {$periodo_ini} 
                                AND nmp.periodo_fin = '{$periodo_fin}'
                        ORDER BY npcc.codigo_proyecto_cc ASC";
        }
...
while ($fila = $res->fetch_assoc()) {
        ...
        ORDER BY nmn.ficha,nmn.codcon";
                ...
                if ($neto3_empleado < 0) {
                        $fichas['Estado'] = "0";
                        $fichas['data'][] =  array(
                                "ficha" => (is_null($fila['ficha']) ? "-" : $fila['ficha']),
