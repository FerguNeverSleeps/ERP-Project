...
//            ->setCellValue('A6', 'SALARIOS')
 ...
 $sql2= "SELECT np.ficha, np.suesal as salario, 
            ...
            END as primer_apellido, np.cedula as cedula, nc.des_car as cargo, np.fecing as fechaingreso, np.inicio_periodo as fechaini, TIMESTAMPDIFF(YEAR,np.fecnac,CURDATE()) AS edad, pry.idProyecto, pry.descripcionLarga AS proyecto,  np.codnivel1
            FROM   nompersonal np
            left join nomcargos nc on (np.codcargo = nc.cod_cargo)
            left join proyectos pry on (pry.idProyecto = np.proyecto)
            WHERE (np.estado = 'REGULAR' OR np.estado = 'Activo')
