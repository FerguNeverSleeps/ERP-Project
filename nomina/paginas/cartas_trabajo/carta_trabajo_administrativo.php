...
if(isset($_GET['ficha']) && isset($_GET['tipnom']))
{
	...
	$sql = "SELECT n.nombres, n.apellidos, n.cedula,n.seguro_social, c.des_car as cargo,
				   ...
				   n.suesal, n.hora_base, n.inicio_periodo, n.fin_periodo, n.seguro_social,n.fecing
			FROM   nompersonal n
	...
	$fecing = formato_fecha($trabajador->fecing,3);
	...
	$salario_mensual   = $trabajador->suesal;
	...
	$seguro_educativo  = $trabajador->suesal*0.0125;
		...
		$total_deducciones = $seguro_social + $seguro_educativo + $seguro_colectivo + $isr+ $monto; 			
		...
		$fecha_cont        = ($fecha_cont=="") ? '-' : $fecha_cont;	
		//$isr               = ($isr==0.00) ? '-' : $isr;	
		$salario_neto      = number_format($salario_neto, 2, '.', '');
		$V                 = new EnLetras(); 
		$monto_letras      = strtoupper($V->ValorEnLetras(($trabajador->suesal),"balboas"));
		
    $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('templates/carta_trabajo_administrativo.docx');
	...
	$templateProcessor->setValue('segsoc', htmlspecialchars($trabajador->seguro_social));
	...
	$templateProcessor->setValue('monto',      htmlspecialchars($monto));
	...
	$templateProcessor->setValue('fechacont',        htmlspecialchars($fecha_cont));
	...
	$templateProcessor->setValue('ger_rrhh', $ger_rrhh);
	...
	$filename = 'Carta-Trabajo-'.$trabajador->cedula.'.docx';
