...
if( isset($_GET['ficha']) && isset($_GET['tipnom']) && isset($_GET['constancia_id']) )
{
	...
	$pdf->SetTitle('Constancia Modelo dos');
	...
	//$margen_sup=-200;
	$margen_sup = ($constancia->margen_sup!=null) ? $constancia->margen_sup : 30; // PDF_MARGIN_TOP 27
	...
	// consultar todos los prestamos de un colaborador
	//===================================================================================================
	$sqls = "SELECT p.descrip as detalle,n.mtocuota as monto FROM nomprestamos_cabecera  as n
		LEFT JOIN nomprestamos as p ON n.codigopr=p.codigopr 
	     WHERE  ficha='{$FICHA}' AND estadopre='Pendiente'";
	$res = $db->query($sqls);	
	$info = "";
	$contador=0;
	while($fila = $res->fetch_array()) {
		$CASA_COMERCIAL_NOMBRE = utf8_decode($fila['detalle']);
		$CASA_COMERCIALl = $fila['monto']*2;
                $CASA_COMERCIALl=number_format($CASA_COMERCIALl, 2, '.', ',');
		if ($CASA_COMERCIALl>0) {			
			$info.="<tr>
			<td style='width:175px'>&nbsp;</td>
			<td style='width:70px'>&nbsp;</td>
			<td style='width:60px'>&nbsp;</td>
			<td style='width:169px'>$CASA_COMERCIAL_NOMBRE</td>
			<td style='text-align:right; width:70px'><strong>$CASA_COMERCIALl</strong></td>	
			</tr>";
		} 		
		 $contador+=$CASA_COMERCIALl;
	}	
	//===================================================================================================
	...
	$TOTAL_DESCUENTOS =  $SEGURO_SOCIAL + $SEGURO_EDUCATIVO + $IMPUESTO_SOBRE_RENTA + $contador;
   ...
   // $CASA_COMERCIAL        = number_format($ontador, 2, '.', ',');
