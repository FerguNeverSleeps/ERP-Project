...
if(isset($_GET['ficha']) && isset($_GET['tipnom']))
{
    ...
    $nombre_carta_ficha       = $empresa->nombre_carta_ficha;
    if ($nombre_carta_ficha == null) {
    	echo "nombre de reporte vacio en BD";
    	exit;
    }
	$sql = "SELECT n.nombres, n.apellidos, n.cedula,n.seguro_social, c.des_car as cargo, 
				   ...
				   n.suesal, n.hora_base, n.inicio_periodo, n.fin_periodo, p.descripcionLarga as proyecto
			FROM   nompersonal n
	...
	//====================================================================================================
	/*$logo="../../../includes/imagenes/".$_SESSION['logo_empresa'];
	...
	$salario_bruto     = obtenerSueldo($ficha, $tipnom, $mes, $anio);
    ...
    $ruta = 'templates/'.$nombre_carta_ficha.'.docx';
    
    $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor($ruta);
	//$section = $templateProcessor->createSection();
	/*if(file_exists("../../../includes/imagenes/".$_SESSION['logo_empresa'])){
    $logo_empresa = $_SESSION['logo_empresa']);
	}
	else{
	    $logo_empresa = "no_logo.png";
	}*/
	//$section->addImage($logo);
	...
	$templateProcessor->setValue('nom_empresa', htmlspecialchars($empresa->nom_emp));
	$templateProcessor->setValue('seg_social', htmlspecialchars($empresa->numero_patronal));
	...
	$filename = 'carta_ficha_'.$trabajador->cedula.'.docx';
...
function obtenerSueldo($ficha, $tipnom, $mes, $anio)
{
	global $db;
