...
if(isset($_GET['ficha']) && isset($_GET['tipnom']))
{
	...
	// Consultar Datos Necesarios del Trabajador
		$sql = "SELECT n.nombres, n.apellidos, n.cedula, n.direccion, n.suesal, n.codcat,
		 			   ...
		 			   n.ficha,n.telefonos, n.hora_base
				FROM   nompersonal n
	...
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('templates/contrato_obra_determinada_itesa.docx');
	...
	$templateProcessor->cloneRow('familiarNombres', $num_familiares);
	...
	while($familiar = $res->fetch_object())
	{
		...
		$templateProcessor->setValue('familiarNombres#' . $i, htmlspecialchars($familiar->nombre));
		$templateProcessor->setValue('familiarParentescos#' . $i, htmlspecialchars($parentesco));
	...
	while($i<=4)
	{
		$templateProcessor->setValue('familiarNombres#'.$i, htmlspecialchars(''));
		$templateProcessor->setValue('familiarParentescos#' . $i, htmlspecialchars(''));
		$i++;
	}
	//=====================================================================================================
	...
	$pago_texto  = mb_strtoupper(trim($numeroTexto->convertir($trabajador->hora_base, '2')));
	$pago_numero = number_format($trabajador->hora_base, 2, '.', ',');
	...
	$templateProcessor->setValue('pago_hora', htmlspecialchars( $pago_texto . ' (B./'.$pago_numero.') ' . $frecuencia));
	...
	$filename = 'ContratoObraTerminada'.$ficha.'.docx';
