...
if(isset($_GET['ficha']) && isset($_GET['tipnom']))
{
	...
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('templates/carta_ficha_panaconstruct.docx');
	...
	$mes_ini = strtolower($mes_ini);
	$mes_ini = substr($mes_ini, 0, 3);

	$fecha_ingreso = $trabajador->dia_ini .'-'.$mes_ini.'-'.$trabajador->anio_ini;

	$templateProcessor->setValue('fecha_ingreso', htmlspecialchars($fecha_ingreso));
	...
	$filename = 'CartaFicha'.$ficha.'.docx';
