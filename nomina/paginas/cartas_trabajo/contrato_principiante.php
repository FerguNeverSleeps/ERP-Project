...
if(isset($_GET['ficha']) && isset($_GET['tipnom']))
{
	...
	// Consultar Datos Necesarios del Trabajador
		$sql = "SELECT n.nombres, n.apellidos, n.cedula, n.seguro_social, n.direccion, n.suesal, n.codcat,
		 			   ...
		 			   n.ficha,n.telefonos, n.hora_base, p.descripcionLarga as proyecto
				FROM   nompersonal n
	...
	//====================================================================================================
        ...
        $sql_bisemana = "SELECT *
			FROM   bisemanas 
			WHERE  '{$fecha_actual}'>=fechaInicio AND '{$fecha_actual}'<=fechaFin";
//        echo $sql_bisemana;
//        exit;
	$res_bisemana = $db->query($sql_bisemana);
        $bisemana_row = $res_bisemana->fetch_object();
        $bisemana = $bisemana_row->numBisemana;
        
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('templates/contrato_principiante.docx');
	...
	$templateProcessor->setValue('ficha', htmlspecialchars($trabajador->ficha));
        $templateProcessor->setValue('bisemana', htmlspecialchars($bisemana));
	...
	$filename = 'ContratoObraTerminada'.$trabajador->cedula.'.docx';
