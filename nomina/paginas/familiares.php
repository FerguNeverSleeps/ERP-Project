...
elseif ($cadena <> ""){ // Condicion para filtrado
	...
	$strsql= "select COUNT(*) from nomfamiliares where cedula='$cedula'";
        //$strsql= "select COUNT(*) from nomfamiliares where cedula='$cedula' and tipnom='".$_SESSION['codigo_nomina']."'";
        //echo $strsql; echo " ";
	$result =sql_ejecutar($strsql);	
	$fila = fetch_array($result);	
	$num_total_registros = $fila[0];
	...
	//$strsql= "select * from nomfamiliares where cedula=$cedula and tipnom='".$_SESSION['codigo_nomina']."' order by cedula,nombre LIMIT $TAMANO_PAGINA OFFSET $limit";
        //echo $strsql;
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
