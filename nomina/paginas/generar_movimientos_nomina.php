...
$url = "movimientos_nomina_persona_generar.php";
$ficha=$_GET['ficha'];
...
if ($op==1)
{
	...
	if($_GET['continua_laborando'] == "true"){
		$continua_laborando = 1;
	}
	else
	{
		$continua_laborando = 0;
	}
	if($_GET['ultimo_ingreso'] == "true"){
		$ultimo_ingreso = 1;
	}
	else
	{
	...
	while ($fila = fetch_array($result2))
	{
		...
		if ($fila['formula']!='')
		{
			...
			if ($fila[contractual]==1){
				...
				if ($entrar==1)
				{
//                                    echo "<br>";echo "MONTO TIPO: ".gettype ( $MONTO );echo "<br>";
                                    if(gettype ( $MONTO )=='array')
                                    {
//                                        echo "Entro MONTO ARRAY";echo "<br>";
//                                        //print_r($MONTO);echo "<br>";
//                                        echo "<br>";echo "MONTO CONTEO: ".count($MONTO);echo "<br>";
...
//                                            echo "<br>";
	...
	$query_check   ="UPDATE nom_movimientos_nomina SET continua_laborando='".$continua_laborando."', ultimo_ingreso='".$ultimo_ingreso."' WHERE tipnom='".$_SESSION['codigo_nomina']."' AND 
	 			      codnom='".$registro_id."' AND 
	 			      ficha='".$_GET['ficha']."' AND 
					   codcon='114'";
	sql_ejecutar($query_check);
	...
	sql_ejecutar($consulta);
        ...
        $res_log = sql_ejecutar($sql_log) or die("no se actualizo el Tipo de personal");
