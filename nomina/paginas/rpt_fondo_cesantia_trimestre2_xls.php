...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E8', 'Segundo Trimestre (Abril-Mayo-Junio)');
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F10', 'PRIMER MES (ABRIL)')
            ->setCellValue('G10', 'SEGUNDO MES (MAYO)')
            ->setCellValue('H10', 'TERCER MES (JUNIO)')
...
$sql = "SELECT DISTINCT np.ficha, np.cedula as cedula, np.apenom as nombre, np.seguro_social as seguro, np.fecing as fecha_ingreso
					...
					WHERE  nm.cedula=np.cedula  AND nm.tipnom='".$codtip."' AND (nm.mes=4 OR nm.mes=5 OR nm.mes=6)
					ORDER BY np.apenom";
$i=12;
...
while($fila=fetch_array($res))
{
    ...
    $sql_mes4 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=4";
    $res_mes4=query( $sql_mes4, $conexion);
    $mes4=0;
    while($fila_mes4=fetch_array($res_mes4))
    {
        $mes4=$mes4+$fila_mes4['monto'];
    }
    $sql_mes5 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=5";
    $res_mes5=query( $sql_mes5, $conexion);
    $mes5=0;
    while($fila_mes5=fetch_array($res_mes5))
    {
        $mes5=$mes5+$fila_mes5['monto'];
    }
    $sql_mes6 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=6";
    $res_mes6=query( $sql_mes6, $conexion);
    $mes6=0;
    while($fila_mes6=fetch_array($res_mes6))
    {
        $mes6=$mes6+$fila_mes6['monto'];
    }
    $salario_trimestral=$mes4+$mes5+$mes6;
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$i, $mes4);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$i, $mes5);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$i, $mes6);
...
header('Content-Disposition: attachment;filename="Reporte_Fondo_Cesantia_Trimestre2.xls"');
