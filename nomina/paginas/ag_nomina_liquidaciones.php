...
else
{
	...
	if($op_tp==1)
	{
		...
		$fecha   = DateTime::createFromFormat('d/m/Y', $_POST['fecha_contab']); 
		$fecha   = ($fecha !== false) ? $fecha->format('Y-m-d') : '';
		...
		$sql = "INSERT INTO nom_nominas_pago 
		        ...
		        ('{$codigo}', '{$descripcion}', '{$fecha_pago}', '{$fecha_inicio}', '{$fecha_final}', '{$anio}', '{$mes_final}',
		         '{$frecuencia}', '0', 'A', '{$_SESSION['codigo_nomina']}', '{$_SESSION['codigo_nomina']}','{$anio_sipe}','{$mes_sipe}','{$fecha}');";
		$res = $db->query($sql);
	...
	$codigo_nuevo = $res->fetch_object()->codigo;
}
		...
		<div class="col-md-12">
		  ...
		  <ul class="page-breadcrumb breadcrumb">
			<li><i class="glyphicon glyphicon glyphicon-sort"></i>
			  <a style="text-decoration: none;">Transacciones</a><i class="fa fa-angle-right"></i>
			</li>
			<li><a href="../nomina_de_liquidaciones.php">Liquidaciones</a></li>
		  </ul>
		</div>
	  </div>
				  ...
				  <div class="portlet box blue">
					<div class="portlet-title">
						...
						<?php
							if($registro_id=='')
							{?>
								<i class="fa fa-plus"></i> Agregar <?php echo $termino; ?> de Liquidaciones
							<?php
							}
							else
							{?>
								<i class="fa fa-edit"></i> Editar <?php echo $termino; ?> de Liquidaciones
							<?php
							}
							?>
						</div>
					  ...
					  <form action="#" id="frmPrincipal" name="frmPrincipal" class="form-horizontal" method="post">
						...
						<div class="form-body">
							...
							<div class="form-group">
							  ...
							  <div class="col-md-10 padding-right-20">
							  		<?php
							  			$sql = "SELECT codfre, descrip FROM nomfrecuencias WHERE codfre='10'";
							  			$res = $db->query($sql); 
							  		?>
							...
							<div class="form-group">
								<label class="control-label col-md-2"  data-toggle="tooltip" title="Fecha último XIII">Fecha de Inicio</label>
								<div class="col-md-2">
									<div class="input-group date date-picker" data-provide="datepicker"><!-- data-date-end-date="0d" -->
										<input data-toggle="tooltip" title="Fecha último XIII" type="text" class="form-control" name="fecha_inicio" id="fecha_inicio" value="<?php echo isset($planilla->periodo_ini) ? $planilla->periodo_ini : ''; ?>">
										<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
									</div>	
								</div>

							    <label class="control-label col-md-2 text-right"  data-toggle="tooltip" title="Fecha liquidación">Fecha Final</label>
							    <div class="col-md-2">
									<div class="input-group date date-picker" data-provide="datepicker"><!-- data-date-end-date="0d" -->
										<input data-toggle="tooltip" title="Fecha liquidación" type="text" class="form-control" name="fecha_final" id="fecha_final" value="<?php echo isset($planilla->periodo_fin) ? $planilla->periodo_fin : ''; ?>">
										<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
									</div>	
							    </div>
							...
							<div class="form-group">
								<div class="col-md-2 text-right"  data-toggle="tooltip" title="Fecha aplica SIPE">
									<label for="txtfechainicio">Mes Aplica:</label>
								</div>
								<div class="col-md-2">
									<select name="mes_sipe" id="mes_sipe" class="form-control"  data-toggle="tooltip" title="Fecha aplica SIPE">
									...
									<div class="col-md-2 text-right">
										<label for="txtfechafinal"  data-toggle="tooltip" title="Fecha aplica SIPE">Año Aplica:</label>
									</div>
									<div class="col-md-2">
										<select name="anio_sipe"  data-toggle="tooltip" title="Fecha aplica SIPE" id="anio_sipe" class="form-control">
							...
							<div class="form-group">
								...
								<div class="col-md-10 padding-right-20">
								<input type="text" name="descripcion" id="descripcion" class="form-control" value="<?php  echo isset($planilla->nombre) ? $planilla->nombre : $_SESSION['nomina'].' - Liquidaciones'; ?>">
								</div>
							</div>
						...
						<div class="form-actions fluid">
						  <div class="col-md-12 text-center">
							...
							onclick="javascript: document.location.href='nomina_de_liquidaciones.php';">Cancelar</button>
						  </div>
						</div>
					  </form>
					</div>
