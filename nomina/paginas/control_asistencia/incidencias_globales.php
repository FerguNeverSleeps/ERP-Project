...
//---------------------------------------------------------------
//---------------------------------------------------------------
$turnos = $conexion->query("SELECT * FROM nomturnos") or die(mysqli_error($conexion));
...
if (isset($_POST['btn_enviar']))
{
	...
	if ($fecha < date("Y-m-d"))
	{
		$res = $conexion->query("UPDATE nomcalendarios_personal SET `turno_id` = '".$_POST['id_turno']."' WHERE `fecha` ='".$fecha."'") or die(mysqli_error($conexion));
		if ($res) {
			header("location:incidencias_globales.php?&msj=1");
		}else{
			header("location:incidencias_globales.php?&msj=2");
		}
	}else{
		header("location:incidencias_globales.php?&msj=3");
	}
}
//---------------------------------------------------------------
	...
	<div class="col-md-12">
		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<img src="../../../nomina/imagenes/21.png" width="22" height="22" class="icon"> Aplicar Incidencias Globales <?php if (isset($_GET['msj'])) {echo $_GET['msj'];} ?>
				</div>
			</div>
			<div class="portlet-body">
				<form action="incidencias_globales.php" method="POST">
					<!--
					<div class="row">
						...
						<div class="col-sm-4 col-md-4 col-lg-4">
							<div class="form-group">
			  					...
			  					<select id="id_turno" name="id_turno" class="form-control">
			  						<?php while($row = mysqli_fetch_array($turnos)): ?>
			  							<option textvalue="<?php echo $row['turno_id']; ?>"><?php echo $row['descripcion']; ?></option>
			  						<?php endwhile ?>
			  					</select>
							</div>	
						</div>
						<div class="col-sm-4 col-md-4 col-lg-4">
							<div class="form-group">
			  					...
			  					<select id="id_incidencia" name="id_incidencia" class="form-control">
			  						<?php while($row = mysqli_fetch_array($res_inc)): ?>
			  							<option textvalue="<?php echo $row['id']; ?>"><?php echo $row['descripcion']; ?></option>
			  						<?php endwhile ?>
			  					</select>
							</div>	
						</div>
