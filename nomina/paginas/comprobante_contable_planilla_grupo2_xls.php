...
if(isset($_GET['codnom']) && isset($_GET['codtip']))
{
	...
	$sql2 = "SELECT np.periodo_ini as desde, np.periodo_fin as hasta, np.fechapago as fechapago, np.mes, np.anio,
			DATE_FORMAT(np.periodo_ini,'%d') as dia_ini, DATE_FORMAT(np.periodo_fin, '%d') as dia_fin,
                        nf.descrip as frecuencia
			FROM nom_nominas_pago np 
                        LEFT  JOIN nomfrecuencias nf ON(np.frecuencia=nf.codfre)
	...
	$objPHPExcel->getActiveSheet()
				...
				->setCellValue('A3', strtoupper(utf8_encode($NOMINA." - ".$mes_letras." - ".$frecuencia)))
...
//                            }
                            ...
                            if($codcon==100 || $codcon==103 || $codcon==169 || $codcon==171 || $codcon==177 || $codcon==178 || $codcon==604)
                            {                                
...
//                    $totalD=$neto+$deducciones;
                    if($bandera==1)
                    {
                        $consulta_cuenta_salario="SELECT c.codcon as codcon_salario, c.descrip as descrip_salario, c.ctacon as ctacon_salario, "
                                            . "cc.Descrip as cuenta_contable_salario  "
...
//                                    exit;
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("A$pos", $fechapago, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("B$pos", $ctacon_salario, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("C$pos", $cuenta_contable_salario, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("D$pos", $codcon_salario."-".utf8_encode($descrip_salario), PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValue('E'.$pos, number_format($salario, 2, '.', ''));
                        
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->getStyle('B'.$pos)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->getStyle('C'.$pos)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->getStyle('D'.$pos)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos.':G'.$i)->applyFromArray(allBordersThin());
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos.':G'.$i)->getFont()->setSize(8);
                        $objPHPExcel->getActiveSheet()->getStyle('E'.$pos.':G'.$i)->getNumberFormat()
                                                                        ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
...
//                    $totalD=$neto+$deducciones;
                    if($bandera==1)
                    {
                        ...
                        $consulta_cuenta_salario="SELECT c.codcon as codcon_salario, c.descrip as descrip_salario, c.cta_contab_reserva as ctacon_salario, "
                                            . "cc.Descrip as cuenta_contable_salario  "
                                            . "FROM nomconceptos as c "
                                            . "LEFT JOIN cwconcue as cc ON (c.cta_contab_reserva=cc.Cuenta) "
                                            . "WHERE c.codcon='100'";
...
//                                    exit;
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("A$pos2", $fechapago, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("B$pos2", $ctacon_descuento, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("C$pos2", $cuenta_contable_descuento, PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValueExplicit("D$pos2", $codcon_descuento."-".utf8_encode($descrip_descuento), PHPExcel_Cell_DataType::TYPE_STRING);
                        $objPHPExcel->getActiveSheet()->setCellValue('F'.$pos2, number_format($descuentos, 2, '.', ''));
                        
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->getStyle('B'.$pos2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                        $objPHPExcel->getActiveSheet()->getStyle('C'.$pos2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->getStyle('D'.$pos2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos2.':G'.$i)->applyFromArray(allBordersThin());
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$pos2.':G'.$i)->getFont()->setSize(8);
                        $objPHPExcel->getActiveSheet()->getStyle('E'.$pos2.':G'.$i)->getNumberFormat()
                                                                        ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_ACCOUNTING);
