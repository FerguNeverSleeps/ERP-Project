...
if(isset($_GET['codnom']) && isset($_GET['codtip']))
{
	...
	$sql  = "SELECT e.nom_emp as empresa
			 FROM   nomempresa e";
	...
	$sql = "SELECT UPPER(descrip) as descrip
	...
	$sql= "SELECT np.ficha, CONCAT_WS(' ', np.nombres, np.apellidos) as nombre,
	...
	while($row=fetch_array($res))
	{
		...
		$objPHPExcel->getActiveSheet()->setTitle($primer_nombre.' '.$primer_apellido[0]);
		...
		$objDrawing->setPath('imagenes/logo_empresa.png');
		...
		if( $sobreTiempo <=0 && $sobreTiempoGR <=0 && ($HORAS_EXTRA > 0 || $HORAS_EXTRA_GASTO_REP >0 ) )
		{
			$SUBTOTAL_SALARIO_BRUTO    .= '+'. $HORAS_EXTRA;
			$SUBTOTAL_GASTOS_REP_BRUTO .= '+'. $HORAS_EXTRA_GASTO_REP;
		}
		*/
		...
		/*
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, 'Cuenta por cobrar');
