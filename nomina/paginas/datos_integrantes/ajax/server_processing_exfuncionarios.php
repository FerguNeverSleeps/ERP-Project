<?php
    ...
    if (isset($_SESSION['ver_regiones'])) {
        $sWhere = "WHERE (estado ='Egresado' OR estado ='De Baja') AND tipnom IN 
                (SELECT  id_nomina from ".SELECTRA_CONF_PYME.".nomusuario_nomina where id_usuario=".$id_usuario." AND  acceso=1)";
    }else{
        $sWhere = "WHERE (estado ='Egresado' OR estado ='De Baja') AND codnivel1 =".$_SESSION['region']." AND tipnom IN 
                (SELECT  id_nomina from ".SELECTRA_CONF_PYME.".nomusuario_nomina where id_usuario=".$id_usuario." AND  acceso=1)";
    }
    ...
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
        ...
        if (isset($_SESSION['ver_regiones'])) {
            $sWhere .= ") AND (estado ='Egresado' OR estado ='De Baja') AND tipnom IN 
                (SELECT  id_nomina from ".SELECTRA_CONF_PYME.".nomusuario_nomina where id_usuario=".$id_usuario." AND  acceso=1)";
        }else{
            $sWhere .= ") AND (estado ='Egresado' OR estado ='De Baja') AND codnivel1 =".$_SESSION['region']." AND tipnom IN 
                (SELECT  id_nomina from ".SELECTRA_CONF_PYME.".nomusuario_nomina where id_usuario=".$id_usuario." AND  acceso=1)";
        }
    }
