...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E8', 'Tercer Trimestre (Julio-Agosto-Septiembre)');
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F10', 'PRIMER MES (JULIO)')
            ->setCellValue('G10', 'SEGUNDO MES (AGOSTO)')
            ->setCellValue('H10', 'TERCER MES (SEPTIEMBRE)')
...
$sql = "SELECT DISTINCT np.ficha, np.cedula as cedula, np.apenom as nombre, np.seguro_social as seguro, np.fecing as fecha_ingreso
					...
					WHERE  nm.cedula=np.cedula  AND nm.tipnom='".$codtip."' AND (nm.mes=7 OR nm.mes=8 OR nm.mes=9)
					ORDER BY np.apenom";
$i=12;
...
while($fila=fetch_array($res))
{
    ...
    $sql_mes7 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=7";
    $res_mes7=query( $sql_mes7, $conexion);
    $mes7=0;
    while($fila_mes7=fetch_array($res_mes7))
    {
        $mes7=$mes7+$fila_mes7['monto'];
    }
    $sql_mes8 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=8";
    $res_mes8=query( $sql_mes8, $conexion);
    $mes8=0;
    while($fila_mes8=fetch_array($res_mes8))
    {
        $mes8=$mes8+$fila_mes8['monto'];
    }
    $sql_mes9 = "SELECT nm.monto as monto
            FROM   nom_movimientos_nomina nm
            WHERE  nm.tipnom=".$codtip." AND nm.ficha=".$ficha." AND ( nm.codcon=100 OR nm.codcon=145) AND nm.mes=9";
    $res_mes9=query( $sql_mes9, $conexion);
    $mes9=0;
    while($fila_mes9=fetch_array($res_mes9))
    {
        $mes9=$mes9+$fila_mes9['monto'];
    }
    $salario_trimestral=$mes7+$mes8+$mes9;
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$i, $mes7);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$i, $mes8);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$i, $mes9);
...
header('Content-Disposition: attachment;filename="Reporte_Fondo_Cesantia_Trimestre3.xls"');
