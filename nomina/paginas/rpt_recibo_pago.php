...
$codti = $_GET['codt'];
$consulta="select * from nom_movimientos_nomina where ficha='".$registro_id."' AND codnom = '".$codnomn."' AND tipnom = '".$codti."' ";
$resultado=sql_ejecutar($consulta);
$fila=mysqli_fetch_array($resultado);
$nomina_id=$fila['codnom'];
$query="select * from nom_nominas_pago where codnom = '".$nomina_id."' AND tipnom = '".$codti."' ";	
...
$cons_obs="SELECT COALESCE(observacion,'') observacion from nom_movimientos_nomina where codcon='114' AND ficha='".$registro_id."' AND codnom = '".$codnomn."' AND tipnom = '".$codti."' ";
$res_obs=sql_ejecutar($cons_obs);
$fila_obs=mysqli_fetch_array($res_obs);
?>
</head>
