...
// Generar netos
$sql = "SELECT ficha, cedula, apenom, cuentacob, forcob,codbancob, codnivel1, codnivel2, codnivel3, codnivel4, codnivel5, suesal, codcargo  "
		...
		while($fila2 = $res2->fetch_assoc())
		{
			...
			if($fila2['codcon']>=500 && $fila2['codcon']<=599)
			{
				...
				// Para optimizar la consulta crear el �ndice (Esta tabla no tiene primary key ni �ndices):
                                ...
                                $sql_prestamos = "SELECT numpre,count(*) cuotas from nomprestamos_detalles WHERE fechaven BETWEEN '{$periodo_ini}' AND '{$periodo_fin}' AND ficha='{$fila['ficha']}' GROUP BY numpre";
                                ...
                                while($fila_pres = $res_prestamos->fetch_assoc())
                                {
                                        ...
                                        if($fila_cabecra['cuotas'] == $fila_pres['cuotas'] ){
