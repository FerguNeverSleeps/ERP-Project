...
$objPHPExcel->getActiveSheet()       
            ...
            ->setCellValue('I11', 'REMUNERACIONES RECIBIDAS DURANTE EL AÑO EN SALARIOS')
            ->setCellValue('J11', 'REMUNERACIONES RECIBIDAS DURANTE EL AÑO EN SALARIOS EN ESPECIES')                   
            ->setCellValue('K11', 'REMUNERACIONES RECIBIDAS DURANTE EL AÑO EN GASTOS DE REPRESENTACION')
            ->setCellValue('L11', 'REMUNERACIONES RECIBIDAS DURANTE EL AÑO EN SALARIOS SIN RETENCIONES')
...
while($fila=fetch_array($res))
{
   ...
   /* $sql_salarios_integral = "SELECT nm.monto as monto
    ...
    SUM(liquida) liquida,SUM(bono) bono,SUM(otros_ing) otros_ing
   ...
   /* $sql_salarios = "SELECT SUM(salario_bruto+vacac+xiii+gtorep+xiii_gtorep+liquida+bono+otros_ing) monto
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $grupo1);
    $objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $grupo2);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $total_grep);
    $objPHPExcel->getActiveSheet()->setCellValue('L'.$i, '0');
    $objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $monto_deducciones);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('S'.$i, "=M".$i."+O".$i."+P".$i."+Q".$i."+R".$i);
    $objPHPExcel->getActiveSheet()->setCellValue('T'.$i, "=I".$i."-M".$i);
    $objPHPExcel->getActiveSheet()->setCellValue('U'.$i, $impuesto_causado);
    $objPHPExcel->getActiveSheet()->setCellValue('V'.$i, '0');
    $objPHPExcel->getActiveSheet()->setCellValue('W'.$i, '0');
    $objPHPExcel->getActiveSheet()->setCellValue('X'.$i, $islr);
    $objPHPExcel->getActiveSheet()->setCellValue('Y'.$i, ($islr_gr+$islr_gr13));
    $objPHPExcel->getActiveSheet()->setCellValue('Z'.$i, '0');
    $objPHPExcel->getActiveSheet()->setCellValue('AA'.$i, '0');
    if(($impuesto_causado-$islr)>0)
    {
        $objPHPExcel->getActiveSheet()->setCellValue('AB'.$i, abs($impuesto_causado-$total_islr));
        $objPHPExcel->getActiveSheet()->setCellValue('AC'.$i, '0');
    }
    else
    {
        $objPHPExcel->getActiveSheet()->setCellValue('AB'.$i, '0');
        $objPHPExcel->getActiveSheet()->setCellValue('AC'.$i,  abs($impuesto_causado-$total_islr));
    }
