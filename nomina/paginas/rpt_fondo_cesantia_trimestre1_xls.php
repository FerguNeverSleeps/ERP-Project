...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F10', 'PRIMER MES (ENERO)')
            ->setCellValue('G10', 'SEGUNDO MES (FEBRERO)')
            ->setCellValue('H10', 'TERCER MES (MARZO)')
...
$sql = "SELECT DISTINCT np.ficha, np.cedula as cedula, np.apenom as nombre, np.seguro_social as seguro, np.fecing as fecha_ingreso
					...
					WHERE  nm.cedula=np.cedula  AND nm.tipnom='".$codtip."' AND (nm.mes=1 OR nm.mes=2 OR nm.mes=3)
					ORDER BY np.apenom";
$i=12;
...
header('Content-Disposition: attachment;filename="Reporte_Fondo_Cesantia_Trimestre1.xls"');
