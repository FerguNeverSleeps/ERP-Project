...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D10', 'SEGURO SOCIAL')
            ->setCellValue('E10', 'NOMBRE')
            ->setCellValue('F10', 'SUELDO');
...
$res=query($sql, $conexion);
$letra='G'; $letra_tb=$letra_td=$letra_tp=$letra_t='';
while($fila=fetch_array($res)){
	...
	$letra_td=$letra;
	/*$objPHPExcel->getActiveSheet()->setCellValue($letra.'10', 'TOTAL ACREEDORES');
	$objPHPExcel->getActiveSheet()->getStyle($letra.'10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$objPHPExcel->getActiveSheet()->getStyle($letra.'10')->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getColumnDimension($letra)->setAutoSize(true);
	$letra++;*/
	...
	$letra_ta=$letra;
	$objPHPExcel->getActiveSheet()->setCellValue($letra.'10', 'TOTAL DEDUCCIONES');
	$objPHPExcel->getActiveSheet()->getStyle($letra.'10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$objPHPExcel->getActiveSheet()->getStyle($letra.'10')->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getColumnDimension($letra)->setAutoSize(true);
	$letra++;
...
$sql = "SELECT DISTINCT np.ficha, np.cedula, np.apenom as nombre, np.suesal as sueldo, seguro_social
		FROM   nom_movimientos_nomina nm, nompersonal np
		WHERE  nm.cedula=np.cedula  AND nm.codnom=".$codnom." AND nm.tipnom=".$codtip."
		ORDER BY np.ficha";
$res=query($sql, $conexion);
$i=11; $total_bruto=0; $total_deducir=0; $total_patronales=0; $total=0; $total_acree=0;
while($fila=fetch_array($res)){
	$ficha=$fila['ficha'];
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$i, $fila['seguro_social']);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$i, utf8_encode($fila['nombre']) );
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$i, $fila['sueldo']);
	...
	$res2=query($sql2, $conexion);
	$letra='G'; $total_asignacion=0; $total_deduccion=0; $total_patronal=0; $total_acreedores=0;
	while($fila2=fetch_array($res2)){
		$codcon=$fila2['codcon'];

		$sql3 = "SELECT nm.tipcon, nm.valor as ref, nm.monto, nm.descrip, nm.codcon
...
//$objPHPExcel->getActiveSheet()->getStyle($letra_ta.$i)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
...
$objPHPExcel->getActiveSheet()->mergeCells('F'.$i.':G'.($i+4));
$objPHPExcel->getActiveSheet()->getStyle('F'.$i.':G'.($i+4))->applyFromArray(allBordersThin());
$i=$i+5;
...
$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, 'AUTORIZADO POR');
...
$objPHPExcel->getActiveSheet()->getStyle('F'.$i.':G'.$i)->applyFromArray(allBordersThin());
$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, 'PREPARADO POR');
...
//$objPHPExcel->getProtection()->setSheet(true);

// Redirect output to a client’s web browser (Excel5)
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Reporte_Horizontal_PLANILLA_Agrupado_Acreedores.xls"');
