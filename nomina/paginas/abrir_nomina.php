...
$consulta="delete from nomina_proyecto_cc where cod_nomina='".$_GET['codigo_nomina']."'";
$resultado=$selectra->query($consulta);

$consulta="delete from nomacumulados_det where codnom='".$_GET['codigo_nomina']."' and tipnom='".$_SESSION['codigo_nomina']."'";
$resultado=$selectra->query($consulta);

$consulta_acumulados="DELETE FROM salarios_acumulados where cod_planilla='".$_GET['codigo_nomina']."' and tipo_planilla='".$_SESSION['codigo_nomina']."'";
$resultado_acumulados=$selectra->query($consulta_acumulados);
...
while($fila=$resultado->fetch_assoc())
{
	...
	if($resultado_mov->num_rows!=0)
	{
		while($fila_mov=$resultado_mov->fetch_assoc())
		{
			...
			if(($fila_mov['codcon']>=500)&&($fila_mov['codcon']<=599))
			{
				$consulta_upd="UPDATE nomprestamos_detalles SET estadopre='Pendiente' WHERE ficha='".$fila['ficha']."' and codnom=".$_SESSION['codigo_nomina']." and estadopre='Cancelada' and fechaven between '$fetch33[periodo_ini]' and '$fetch33[periodo_fin]' ";
				$resultado_upd=$selectra->query($consulta_upd);
			}
		}	
	}
