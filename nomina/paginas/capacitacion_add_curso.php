...
$sql = "SELECT b.apenom,b.cedula,c.nombre_curso FROM nomcursos_personal as a,nompersonal as b, nomcursos as c  
WHERE a.id_curso=c.id AND a.id_personal=b.personal_id AND a.id_personal=".$id;
				...
				<!-- BEGIN EXAMPLE TABLE PORTLET-->
					<div class="portlet box blue">
						<div class="portlet-title">
							...
							<div class="actions">
								<a class="btn btn-sm blue"  onclick="javascript: window.location='capacitacion_list.php#tab_1'">
						...
						<div class="portlet-body">
							<div class="row">
								<div class="form-group">
									<label class="col-md-2"><h4>Nombre del Colaborador</h4></label>
									<div class="col-md-9">
									<?php
									$sql   = "SELECT personal_id,apenom,cedula FROM nompersonal WHERE personal_id=".$id;
									$res   = query($sql, $conexion);
									$fila1 = mysqli_fetch_array($res);
									echo "<h5>".$fila1[apenom]."</h5>";
									?>
									<input type="hidden" id="personal" value="<?php echo $id; ?>">
									</div>
								</div>
							</div>
							<div class="row">
								<div class="form-group">
									<label class="col-md-2"><h4>Agregar a un curso</h4></label>
									<div class="col-md-9">
									<?php
									$tabla="nomcursos";
									...
									?>
										<select id="nombre_curso" name="nombre_curso" class="form-control select2_category" autofocus>
										<option value="">Seleccione...</option>
										<?php 
										while($fila=mysqli_fetch_array($resultado)){
										echo "<option value='".$fila["id"]."'>".$fila["nombre_curso"]."</option>";}
										?>
										</select>
									</div>
								</div>
							...
							<div class="form-action">
								<table class="table table-striped table-bordered table-hover" id="table_datatable">
									...
									<tbody>
									<?php
										$SQL = "SELECT a.id, c.nombre_curso, a.id_curso,a.id_personal
									FROM nomcursos_personal AS a, nomcursos AS c
									WHERE  a.id_curso = c.id AND id_personal=".$id;
										$res = query($SQL, $conexion);
										while($filas=fetch_array($res))
										{ 
											$id=$filas["id_personal"];								
										?>
											<tr class="odd gradeX">
												<td><?php 										
												echo $filas[id_curso]; ?></td>
												<td><?php
												echo $filas[nombre_curso]; ?></td>
												<td style="text-align: center">
			
												<?php // icono("usuarios_delete.php?codigo=".$codigo, "Eliminar", "delete.gif"); 
													echo "<a  href='capacitar.php?codigo=".$filas[id_curso]."&personal=".$id."'&pagina=2 title='Eliminar integrantes'>"; ?>
  ...
  $("#sig1").on("click", function()
   {
		...
		{
			location.href="capacitar.php?codigo="+codigo+"&personal="+personal+"&pagina=2";
		}
   });
