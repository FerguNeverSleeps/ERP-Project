...
//        exit;
	...
	while ($obj = $res->fetch_object() ) 
	{
		...
		$sql = "SELECT DATE_FORMAT(periodo_ini, '%M') as nom_mes, DATE_FORMAT(periodo_ini, '%Y') as anio, mes
				FROM   nom_nominas_pago
				WHERE  codnom='{$obj->codnom}' AND tipnom = '{$obj->tipnom}'";
		$res = $db->query($sql);
	...
	'isr') );
		...
		while($codnivel1 = $res->fetch_object())
		{
			...
			$sql1 = "SELECT  p.ficha, p.cedula, p.suesal, p.fecing, p.estado,
					 ...
					 AND     p.ficha IN (SELECT ficha FROM nom_movimientos_nomina WHERE codnom IN ('{$obj->codnom}') AND tipnom = '{$_SESSION['codigo_nomina']}')";
			...
			while($personal = $res1->fetch_object())
			{
				...
				foreach($conceptos as $concepto)
				{
					...
					if($concepto["variable"]=='regular'  ||  $concepto["variable"]=='nacional')
					{
						$sql2 .= "SELECT COALESCE(SUM(n.valor),0) as horas, COALESCE(SUM(n.monto),0) as total, '{$concepto["variable"]}1' as variable
								  ...
								  WHERE  n.codnom='{$obj->codnom}' AND tipnom = '{$_SESSION['codigo_nomina']}' AND n.ficha='{$personal->ficha}' 
					...
					else
					{
						$sql2 .= "SELECT COALESCE(SUM(n.valor),0) as horas, COALESCE(SUM(n.monto),0) as total, 
								  ...
								  FROM   nom_movimientos_nomina n
								  WHERE  n.codnom IN ('{$obj->codnom}') AND n.ficha='{$personal->ficha}'  AND tipnom = '{$obj->tipnom}'  AND    {$concepto["condicion"]} ";
					}
				}
				...
				$SQL = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (106,107,108,109,112,113,115,116,119,120,121,122,123,124,125,125,126,127,128,129,130,131,132,133,134,135,136,136,139,142,143,144,149,150,151,152,153,154,628) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_horas_extras = $db->query($SQL)->fetch_assoc();
				$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_xiii = $db->query($SQL2)->fetch_assoc();
				$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (101) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_xiii_gp = $db->query($SQL2)->fetch_assoc();
				$SQL2 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (157) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_comision = $db->query($SQL2)->fetch_assoc();

				$SQL3 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_xiii2 = $db->query($SQL3)->fetch_assoc();

				$SQL4 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (93) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_prima = $db->query($SQL4)->fetch_assoc();

				$SQL5 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (102) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_vac = $db->query($SQL5)->fetch_assoc();
				$SQL6 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (145) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				$total_gr = $db->query($SQL6)->fetch_assoc();
				$SQL7 = "SELECT ifnull(sum(monto),0) as monto FROM nom_movimientos_nomina  where ficha='{$personal->ficha}' AND codcon IN (156) AND codnom IN ('{$obj->codnom}') AND  tipnom = '{$obj->tipnom}'";
				...
				if($personal->estado == 'Deb Baja')
				{
					...
					COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
						WHERE  ficha='{$personal->ficha}' AND n.codnom in ($codigo) AND n.tipnom = '{$obj->tipnom}' AND n.codcon in (114)), 0) as monto_vac,
					COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
						WHERE  ficha='{$personal->ficha}' AND n.codnom in ($codigo) AND n.tipnom = '{$obj->tipnom}' AND n.codcon in (117)), 0) as prop_vac";
				...
				->setCellValue('N'.$i, ($total_isr==0 ? 0.00 : $total_isr))
				->setCellValue('O'.$i, ($total_se==0 ? 0.00: $total_se ))
				->setCellValue('P'.$i, ($total_ss==0 ? 0.00: $total_ss ))
				...
				->setCellValue('W'.$i, $personal->estado);
	...
	$sql3 = "SELECT
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (3000)), 0) as ss_patronal,
	COALESCE((SELECT SUM(n.monto) AS monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (3001)), 0) as se_patronal,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (3002)), 0) as ss_xiii_patronal,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (9009)), 0) as riesgo,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (201)), 0) as ss_emp,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (202)), 0) as se_emp,
	COALESCE((SELECT SUM(n.monto) as monto FROM nom_movimientos_nomina n 
		WHERE  n.codnom in ($codigos) AND n.tipnom = '{$codtip}' AND n.codcon in (205)), 0) as ss_xiii_emp";
		//echo $sql3;exit();
