...
if(isset($_GET['fecha_inicio']) && isset($_GET['fecha_fin']) && isset($_GET['tipo_planilla']))
{
    ...
    $sql2 = "SELECT np.periodo_ini as desde, np.periodo_fin as hasta, np.mes,
            ...
            WHERE  np.fechapago BETWEEN '".$fecha_inicio."' AND '".$fecha_fin."' AND np.tipnom=".$codtip;
...
/*
    ...
    $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('C5', 'Desde el '. $fecha_inicio .' hasta el '. $fecha_fin );
    ...
    from nomnivel1 a inner join nom_movimientos_nomina b on a.codorg=b.codnivel1 where b.codcon= 590 and b.codnom in (SELECT np.codnom  
    FROM nom_nominas_pago np WHERE np.tipnom=$codtip AND np.periodo_fin BETWEEN '$fecha_inicio' AND '$fecha_fin') order by a.codorg";
    $res2=$conexion->query($sql2);
    $i++;
    $totalCuotas = 0;
    $totalSaldos = 0;
    ...
    {    
        ...
        $sql3 = "SELECT a.ficha FROM `nom_movimientos_nomina` a WHERE `codnom` in (SELECT np.codnom  
        FROM nom_nominas_pago np WHERE np.tipnom=$codtip AND np.periodo_ini BETWEEN '$fecha_inicio' AND '$fecha_fin') AND `codcon` = 590 and a.codnivel1='$codNivel1' 
        GROUP BY a.ficha
        ORDER BY a.codnivel1 ASC, a.cedula ASC";
...
$replacenom = $ficha." AND codnom=".$codnom;
       ...
       $sql4 = "SELECT a.apenom,a.ficha,a.cedula,c.codorg,c.descrip, 
            COALESCE((SELECT n.descrip FROM nom_movimientos_nomina n WHERE n.codcon=590 and n.ficha= ".$ficha." GROUP BY n.codcon),0) as concepto_ahorro, 
            COALESCE((SELECT np.descrip FROM nomcampos_adic_personal na LEFT JOIN nomprestamos np ON na.valor=np.codigopr WHERE na.id=23 and na.ficha= ".$ficha." ),0) as acreedor_ahorro, 
            COALESCE((SELECT sum(n.monto) FROM nom_movimientos_nomina n WHERE n.codcon=590 and n.ficha= ".$ficha." and n.anio='$anio' and n.mes='$mes' GROUP BY n.codcon),0) as cuota_ahorro,           
            COALESCE((SELECT na.valor FROM nomcampos_adic_personal na WHERE na.id=22 and na.ficha= ".$ficha." ),0) as saldo_ahorro 
            FROM nompersonal a 
