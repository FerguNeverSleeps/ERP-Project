...
if(!isset($configReporte['id'])){
	print "config_reportes_planilla id=11 no encontrado";
	exit;
}
...
for($i=0; $i <count($configReporteColumnas[$i]); $i++){ 
	...
	if($configReporteColumnas[$i]["col_letra"]=="A")
		continue;
...
$sheet->setCellValue("A".$ln,"Gran Total");
$sheet->getStyle("A".$ln.":{$ultima_columna}".$ln)->getFont()->setBold(true);
...
$sheet->getStyle("B".$ln_inicio.":{$ultima_columna}$ln")->getFont()->setSize(11);
$sheet->getStyle("B".$ln_inicio.":{$ultima_columna}$ln")->getNumberFormat()->setFormatCode('_-* #,##0.00_-;[RED]-* #,##0.00_-;_-* -_-;_-@_-');
$sheet->getStyle("B".$ln_inicio.":{$ultima_columna}$ln")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

$sheet->getStyle("{$ultima_columna}".$ln_inicio.":{$ultima_columna}".$ln)->getFont()->setBold(true);
$sheet->getStyle("A".$ln_inicio.":A$ln")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
...
$sheet->setCellValue('A'.$ln, "GRUPO MAS ME DAN ".strtoupper($empresa['nombre']));
$sheet->setCellValue('B'.$ln, "PROMEDIO %");
$ln++;

$sheet->setCellValue('A'.$ln, "SUCURSAL");
$sheet->setCellValue('B'.$ln, "");
$sheet->setCellValue('C'.$ln, "INGRESOS");
$sheet->setCellValue('D'.$ln, "GASTO POR REEMBOLSO BENEFICIOS ADMIN");
$sheet->getRowDimension($ln)->setRowHeight(50);
$ln++;

$sheet->getStyle("A".($ln-2).":B".($ln-1))
        ->getFill()
...
$sheet->getStyle("A".($ln-2).":D".($ln-1))
		->getFont()
		->setSize(10)
        ->setBold(true);

$sheet->getStyle("D".($ln-1))->getAlignment()->setWrapText(true);
$sheet->getStyle("C".($ln-1).":D".($ln-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
while($centro_costo=$res->fetch_array()){	
	...
	$sheet->setCellValue('D'.$ln, "=O{$ln_total}*C{$ln}%");
...
$sheet->getStyle("A".$ln_inicio.":D".$ln)->getFont()->setSize(10);
