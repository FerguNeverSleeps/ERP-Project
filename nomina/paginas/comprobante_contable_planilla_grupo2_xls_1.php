...
//            $departamento=$row1['departamento'];
    ...
    //            ->setCellValue('A6', 'SALARIOS')
            ...
            $total_salarios=$total_comision=$total_combustible=$total_reembolso=$total_uso_auto=$total_bono="=";	
                    ...
                    $consulta1="SELECT SUM(nm.monto) AS suma, nm.codcon, nm.descrip, nm.tipcon, c.ctacon, cc.Descrip as cuenta_contable  "
                            ...
                            . "WHERE nm.codnom='".$codnom."' AND nm.tipnom='".$codtip."' AND nm.codnivel1='1' "
                            . "GROUP BY nm.codcon "
                            . "ORDER BY nm.codcon";
    //		echo $consulta;
    //                exit;
                    $res1=$conexion->query($consulta1);
                    $i++;
                    $ini=$i; $enc=false;$cont=0;
                    $salario=$neto=0;
                    ...
                    {	
                            $enc=true;
                            $codcon = $row1['codcon'];
                            $descrip = utf8_decode($row1['descrip']);
                            $tipcon = $row1['tipcon'];
                            $ctacon = $row1['ctacon'];
                            $cuenta_contable = $row1['cuenta_contable'];
                            $suma = $row1['suma'];
                            if($codcon==100)
                            {
                                $salario=$suma;
                           ...
                           if($codcon==103 || $codcon==177 || $codcon==178 || $codcon==604)
...
//                    $totalD=$neto+$deducciones;
            ...
            $total_salarios=$total_comision=$total_combustible=$total_reembolso=$total_uso_auto=$total_bono="=";	
                    ...
                    $consulta2="SELECT SUM(nm.monto) AS suma, nm.codcon, nm.descrip, nm.tipcon, c.cta_contab_reserva as ctacon, cc.Descrip as cuenta_contable  "
                            . "FROM nom_movimientos_nomina AS nm "
                            . "LEFT JOIN nomconceptos AS c ON (nm.codcon=c.codcon) "
