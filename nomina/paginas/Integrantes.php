...
function getIntegrantes($request)
{
    ...
    if($request['texto_buscar'] != ""){
        $texto = mysqli_real_escape_string($request['texto_buscar']);
        $andBusqueda = " and (cedula like '%".$texto."%' or nombres like '%".$texto."%' or apellidos like '%".$texto."%' or ficha like '%".$texto."%')";
    }

    $query = "SELECT cedula,nombres, apellidos, ficha,foto,estado,descrip 
    FROM nomvis_integrantes 
    WHERE descrip = '".$_SESSION['nomina']."' and estado <> 'Egresado' ".$andBusqueda;
    $result = sql_ejecutar($query);
    $totalCount = numero_filas($result);


    $query = "SELECT cedula,nombres, apellidos, ficha,foto,estado,descrip ,year(curdate()) anio
    FROM nomvis_integrantes 
    WHERE descrip = '".$_SESSION['nomina']."' and estado <> 'Egresado' ".$andBusqueda."   
    ...
    while($fila = fetch_array($result))
    {
        ...
        $datos[$i]["anio"] = $fila[anio];
        $i++;
    }
    return  array('totalCount'=>$totalCount,'Integrantes'=>$datos);
