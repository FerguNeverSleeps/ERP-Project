...
$(document).ready(function()
{
	...
	function nomina(anio)
	{
		$.get("ajax/obtenerMesNomina.php",{anio:anio},function(res)
		{
			...
			{
				mes = $("#mes").val();
				$.get("ajax/obtenerPlanillaMesSipe.php",{anio:anio, mes:mes},function(res1){
					$("#quincena_box").empty();
					$("#planilla_box").empty();
					...
					tipos = $("select#tipos").val();
						...
						else
						{
							...
							var url = "../../reportes/excel/excel_acreedores_mes_sipe.php?mes="+mes+"&anio="+anio+"&tipos="+tipos; // the script where you handle the form input.
							window.open(url);
						}	
					...
					$("#generar_rpt_xls").on("click", function(){
						...
						var url = "../../reportes/excel/excel_acreedores_sanmiguelito.php?mes="+mes+"&anio="+anio+"&tipos="+tipos; // the script where you handle the form input.
						window.open(url);	
