...
if(!isset($configReporte['id'])){
	print "config_reportes_planilla id=6 no encontrado";
	exit;
}
...
$objPHPExcel->getProperties()->setCreator("AMAXONIA")
							 ...
							 ->setTitle("Resumen de Salarios por Centros de Costo");
...
$sheet->setCellValue("A1", "RESUMEN DE SALARIOS POR CENTROS DE COSTOS    ( ".letra_mes($mes)." ".$anio." )");
...
while($centro_costo=$res->fetch_array()){	
	...
	$sheet->setCellValue('A'.$ln, $centro_costo['markar']." ".$centro_costo['descripcion_completa']." (".$centro_costo['descrip'].")");
...
$sheet->getStyle("A".$ln.":R".$ln)->getFont()->setBold(true);


$sheet->getStyle("A".$ln_inicio.":A".($ln-1))->getFont()->setSize(12);
$sheet->getStyle("A".$ln_inicio.":A".($ln-1))->getFont()->setBold(true);

$sheet->getStyle("A".$ln_inicio.":R$ln")->getFont()->setSize(11);
//$sheet->getStyle("B".$ln_inicio.":P$ln")->getNumberFormat()->setFormatCode('#,##0.00');
$sheet->getStyle("B".$ln_inicio.":R$ln")->getNumberFormat()->setFormatCode('_-* #,##0.00_-;[RED]-* #,##0.00_-;_-* -_-;_-@_-');
$sheet->getStyle("B".$ln_inicio.":R$ln")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
$sheet->getStyle("A".$ln_inicio.":R".$ln)->applyFromArray($border);
