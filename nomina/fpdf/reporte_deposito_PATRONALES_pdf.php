	...
	*/
	...
	while($totalwhile>=$contar)
	{
		...
		if($fila_mov['monto']!=0)
		{
			...
			$this->Row(array(utf8_decode($fila['apenom']),$cedula,fecha($fila['fecing']),number_format($fila_mov['monto'],2,',','.'),number_format($fila_mov_pat['monto'],2,',','.'),number_format($fila_mov['monto']+$fila_mov_pat['monto'],2,',','.')));
