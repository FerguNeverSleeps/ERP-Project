...
function detalle($gerencia,$nomina,$antiguedad1,$antiguedad2){
	...
	if($gerencia!='Todos'){
		$cadenaselect=$cadenaselect.' and codnivel1='.$gerencia;
	}
	if($nomina!='Todos'){
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal where codnivel1<>'' $cadenaselect  ORDER BY  codnivel1,tipnom,apenom ";
	}else{
		
		$consulta="select * from nompersonal where codnivel1<>'' $cadenaselect  ORDER BY tipnom, codnivel1,apenom ";
...
//  				$this->Cell(188,5,$cantidad_registros,0,1);
				if($cantidad_registros<=2){
					...
					$this->SetFont("Arial","I",11);
					$this->SetWidths(array(10,65,18,69,30,13,30,30));
				...
				if($fila['fecharetiro']!='0000-00-00'){
					...
					$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),$fila['cedula'],utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos,date("d/m/Y",strtotime($fila['fecharetiro'])),$anti));$cantidad_registros-=1;
				}else{
					$anos=antiguedad($fila['fecnac'],date("Y-m-d"),"A");
					$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),$fila['cedula'],utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos,'-',$anti));$cantidad_registros-=1;
	...
	// 				$this->Cell(188,5,strlen($fetchcargo1['des_car']),0,1);
				...
				$this->SetWidths(array(20,40,125,75));
				$this->SetFont("Arial","I",11);
				...
				while($fila2=fetch_array($query2)){
					...
					$this->Row(array($fila2['tipo_tiporegistro'],utf8_decode($fila2['nombre_especialista']),utf8_decode($fila2['descripcion']),utf8_decode($fila2['institucion'])));
					$cantidad_registros-=1;
					if($cantidad_registros<=0){
						$this->Ln(300);
						$cantidad_registros=30;
				...
				while($fila3=fetch_array($query3)){
					...
					$this->Row(array(date("d/m/Y",strtotime($fila3['fecha_salida'])),utf8_decode($fila3['descripcion'])));
					$cantidad_registros-=1;
					if($cantidad_registros<=0){
						$this->Ln(300);
						$cantidad_registros=30;
