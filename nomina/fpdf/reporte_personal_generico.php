...
function Header()
{
	...
	$this->Cell(188,5,'LISTADO DE COLABORADORES',0,0,'C');
	$this->Ln(10);
...
function Footer(){
	...
	$this->Cell(188,10,''.$var_encabezado1,0,0,'L');
...
function detalle($gerencia,$nomina){
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal where estado<>'Egresado' and codnivel1<>'' $cadenaselect  ORDER BY  tipnom,ficha  ";
	}else{
	...
	while($fila=fetch_array($query)){
		...
		if($gerencia!='Todos' && $nomina=='Todos'){
			...
			if($codnom!=$cod_nomina){
				...
				$this->SetWidths(array(10,65,20,58,21,13));
				$this->SetAligns(array('C','C','C','C','C','C'));
				$this->Setceldas(array(1,1,1,1,1,1));
				$this->Setancho(array(5,5,5,5,5,5));
				$this->Row(array(utf8_decode('Cód'),'Nombre y Apellido',utf8_decode('Cédula'),'Cargo','Fecha Ing.','Edad'));
				$paso=0;
...
// 			$this->Cell(188,5,$cantidad_registros,0,1);
			...
			$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),$fila['cedula'],utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos));
			$cantidad_registros-=1;
