...
//fin
    ...
    function personas($nomina_id, $codt, $pdf) {
        ...
        WHERE  nom.codnom = '$nomina_id' and nom.tipnom = " . $codt . " and per.tipnom=" . $codt . "  AND
        (per.estado NOT LIKE '%Egresado%' AND per.estado  NOT LIKE '%De Baja%')
        group by per.ficha order by per.ficha, per.codnivel3, per.apenom";
...
// 			}
			...
			where pe.ficha = '" . $sficha . "' and pe.tipnom='" . $codt . "' and mn.codnom = '" . $nomina_id . "' and mn.tipnom = '" . $codt . "' and mn.tipcon<>'P' AND(pe.estado NOT LIKE '%Egresado%' AND pe.estado  NOT LIKE '%De Baja%')
            order by mn.tipcon,c.codcon";
                //exit(0); 
                ...
                $this->Row(array(utf8_decode('Cédula: ' . $fila['cedula']) . '                Fecha Ing:' . fecha($fila['fecing']), 'Ficha: ' . $fila['ficha'] . "            Sueldo Basico:  " . $fila[suesal]));
    ...
    function firmas($total_asig, $total_dedu, $cantidad) {
        ...
        $consultaa = "SELECT ficha, apenom FROM nompersonal WHERE tipnom =" . $_SESSION['codigo_nomina'] . " AND (estado NOT LIKE '%Egresado%' AND estado  NOT LIKE '%De Baja%')";
