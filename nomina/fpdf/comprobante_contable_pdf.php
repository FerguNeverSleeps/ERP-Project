...
function header(){
	...
	$nomina_id      =$_GET['codnom'][0];
	$nomina_id2     =$_GET['codnom'][1];
	if($nomina_id2 == '')
	{
	...
	else
	{
		$consulta = "SELECT periodo_ini,periodo_fin FROM nom_nominas_pago WHERE codnom='".$nomina_id."' and tipnom=$tipnom
					UNION
					SELECT periodo_ini,periodo_fin FROM nom_nominas_pago WHERE codnom='".$nomina_id2."' and tipnom=$tipnom";
		$resultado = query($consulta,$conexion);
		$ixx=0;
		while($fetch = fetch_array($resultado))
		{
			$fecha_inicio[$ixx] = fecha($fetch['periodo_ini']);
			$fecha_final[$ixx]  = fecha($fetch['periodo_fin']);
			$ixx++;
		}
		$fecha_ini =$fecha_inicio[0];
		$fecha_fin =$fecha_final[1];
...
function Cuentas($nomina_id,$codtip,$mes,$anio){
	for ($i=0; $i < count($nomina_id); $i++) 
	{ 
		$conexion=conexion();
		$consulta="SELECT nc.orden_reporte_contable,mn.codcon, mn.descrip,mn.mes,mn.anio,mn.tipcon,nc.ctacon,n1.descrip departamento,
		...
		count(mn.codcon) cantidad_concepto
		 ...
		 inner join nomnivel1 n1 on n1.codorg = np.codnivel1 
		  WHERE mn.anio='".$anio[$i]."' AND mn.mes='".$mes[$i]."' AND mn.tipnom='".$codtip."'
		     and mn.tipcon not in ('P') 
		   ...
		   union all (
		   	SELECT nc.orden_reporte_contable,'' codcon, 'SOBRE TIEMPO' descrip,mn.mes,mn.anio,mn.tipcon,nc.ctacon,n1.descrip departamento,
		sum(mn.monto) suma,
		0 suma2,
		count(mn.codcon) cantidad_concepto
		 FROM nom_movimientos_nomina mn 
		 ...
		 inner join nomnivel1 n1 on n1.codorg = mn.codnivel1 
		  WHERE mn.anio='".$anio[$i]."' AND mn.mes='".$mes[$i]."' AND mn.tipnom='".$codtip."'
		   ...
		   group by mn.codnivel1
		   	...
		   	union all (
		   	SELECT 9998 orden_reporte_contable,'' codcon, 'OTRAS DEDUCCIONES' descrip,mn.mes,mn.anio,mn.tipcon,nc.ctacon,n1.descrip departamento,
		0 suma,
		sum(mn.monto) suma2,
		count(mn.codcon) cantidad_concepto
		 FROM nom_movimientos_nomina mn 
		 ...
		 inner join nomnivel1 n1 on n1.codorg = np.codnivel1 
		  WHERE mn.anio='".$anio[$i]."' AND mn.mes='".$mes[$i]."' AND mn.tipnom='".$codtip."'
		   ...
		   group by np.codnivel1
		   	...
		   	union all (
		   	SELECT 9999 orden_reporte_contable,'' codcon, 'GRAVAMENES POR PAGAR' descrip,mn.mes,mn.anio,mn.tipcon,nc.ctacon,n1.descrip departamento,
		0 suma,
		sum(mn.monto) suma2,
		count(mn.codcon) cantidad_concepto
		 FROM nom_movimientos_nomina mn 
		 ...
		 inner join nomnivel1 n1 on n1.codorg = np.codnivel1 
		  WHERE mn.anio='".$anio[$i]."' AND mn.mes='".$mes[$i]."' AND mn.tipnom='".$codtip."'
		   ...
		   group by np.codnivel1
		   	...
		   	union all (
		   	SELECT 10000 orden_reporte_contable,'' codcon, 'SALARIOS POR PAGAR' descrip,mn.mes,mn.anio,mn.tipcon,nc.ctacon,n1.descrip departamento,
		0 suma,
		...
		$jxx=0;
		while($filas=fetch_array($resultado_mov))
		{   
			$fila[departamento][$jxx] =$filas[departamento];
			$fila[Concepto][$jxx]     =$filas[Concepto];
			$fila[descrip][$jxx]      =$filas[descrip];
			$fila[ctacon][$jxx]       =$filas[ctacon];
			$fila[suma][$jxx]         =$fila[suma][$jxx]+$filas[suma];
			$fila[suma2][$jxx]        =$fila[suma2][$jxx]+$filas[suma2];
			$jxx++;
		}
	...
	*/
	...
	while($totalwhile>=$contar)
	{
		...
		if($fila['departamento'][$cont] != $departamentoNuevo){
			if($conta!=1){


				$this->SetFont('Arial','',12);
				$this->Cell(80,5,'TOTALES:  ','T',0,'L');
				$this->Cell(38,5,number_format($totalA,2,',','.'),'T',0,'C');
				$this->Cell(38,5,number_format($totalD,2,',','.'),'T',0,'C');
				$conta  =0;
				$totalA = 0;
				$totalD = 0;
				$this->Ln(300);	
			...
			$this->SetFont('Arial','',10);
	        $this->SetWidths(array(100,40,24,24,30));
			$this->SetAligns(array('C','C','C','C','C'));
	        $this->Setceldas(array('TB','TB','TB','TB','TB'));
			$this->Setancho(array(5,5,5,5,5));
	        $this->Row(array(utf8_decode('Concepto                     '),'            ','            ','Cuenta     '));
			...
			$this->Row(array("Departamento ",utf8_decode($fila[departamento][$cont]),'          ' ,'         '));
			$departamentoNuevo = $fila['departamento'][$cont];		
		}
		/*$conexion=conexion();
		...
		*/
		...
		$this->Row(array(' Gastos de '. utf8_decode($fila[descrip][$cont]),number_format($fila['suma'][$cont],2,',','.'),	number_format($fila['suma2'][$cont],2,',','.'),$fila[ctacon][$cont]));
		$totalA  =$totalA+$fila['suma'][$cont];
		$totalD  =$totalD+$fila['suma2'][$cont];
		$totalTA =$totalTA+$fila['suma'][$cont];
		$totalTD =$totalTD+$fila['suma2'][$cont];
...
$pdf->SetFont('Sanserif','',12);
	$pdf -> nominapdf = $codtip;


$pdf->nominapdf=$nomina_id;

$pdf->Cuentas($nomina_id,$codtip,$mes,$anio);
