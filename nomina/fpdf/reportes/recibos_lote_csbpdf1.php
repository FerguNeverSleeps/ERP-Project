...
class PDF extends FPDF
{
    ...
    function personas($nomina_id,$codtp){
        ...
        AND (mn.codcon >=100 AND mn.codcon <=209)";
        $query .= $consulta_WHERE;
        $query .= "group by pe.ficha 
        order by pe.ficha ASC"; 

        $result_lote=$conexion->query($query);	
        //$totalbd=$result_lote->num_rows;
    	
        while ($fila=$result_lote->fetch_array())
        {
        	//Datos personal
            ...
            while ($row = $result->fetch_array())
            {
                ...
                if ($row[tipcon]=='D')
                {
                    ...
                    if(($row[codcon]>=500)&&($row[codcon]<=599))
                    {
                        ...
                        WHERE pd.ficha='".$fila[ficha]."' and pd.fechaven between '".$fetch5['periodo_ini']."' and '".$fetch5['periodo_fin']."' 
