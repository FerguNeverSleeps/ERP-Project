...
//	$consulta = "SELECT periodo_ini,periodo_fin FROM nom_nominas_pago WHERE codnom=$nomina_id and tipnom=$tipnom";
	...
	$this->Cell(38,5,'Hora: '.$date2,0,1,'R');
        $this->Cell(188,6,''.$descrip_planilla.' II',0,1,'C');
//	if($this->nomina==4){
	...
	$codnom=$_GET['codnom'];
		...
		WHERE  nm.codnom=".$codnom." AND nm.tipnom=".$codtip." AND nm.codcon = '210' AND nm.monto > 0
                       AND (np.forcob LIKE '%Cheque%' OR np.forcob LIKE '%cheque%' 
                            ...
                            OR np.forcob LIKE '%Deposito%' OR np.forcob LIKE '%deposito%'
...
//                $row2 = mysqli_fetch_array($resultado_banco);     
                ...
                $sql_movimientos="SELECT nm.codcon, nm.tipcon, nm.monto 
                                ...
                                WHERE nm.ficha=".$ficha." AND nm.codnom=".$codnom." AND nm.tipnom=".$codtip." AND nm.monto > 0 
                                    AND ((nm.codcon >=210 AND nm.codcon <=299) OR (nm.codcon>=500 AND nm.codcon<=599)) 
                ...
                $cambio[$ixx]=floor(($importe*100)/($monedas[$ixx]*100)); 
                $importe=($importe*100)%($monedas[$ixx]*100);
                
                while($importe>0)
                {
                    $ixx++;
                    $cambio[$ixx]=floor($importe/($monedas[$ixx]*100)); 
                    $importe=($importe)%($monedas[$ixx]*100);
                    switch ($ixx) 
                    {
                            case 0:
