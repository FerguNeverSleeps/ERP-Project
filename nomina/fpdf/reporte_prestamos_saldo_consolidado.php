...
function header(){
	...
	$this->SetFont('Arial','',10);
        $this->SetWidths(array(12,25,100,25,25));
	$this->SetAligns(array('C','R','L','R','R'));
        $this->Setceldas(array(0,0,0,0,0));
	$this->Setancho(array(5,5,5,5,5,5));
        $this->Row(array('COD.',utf8_decode('CÉDULA'),'APELLIDOS Y NOMBRES','PREST.','SALDO'));
	
}

function movimientos($codcon1,$frecuencia,$mes,$ano)
{
	$conexion=conexion();
	$consulta="SELECT np.apenom, nmn.codnom, nmn.codcon, nmn.ficha, sum(nmn.monto) as monto, nmn.cedula, nmn.tipnom FROM nom_movimientos_nomina nmn join nom_nominas_pago nmp on (nmn.codnom=nmp.codnom and nmn.tipnom=nmp.tipnom) JOIN nompersonal np on (np.cedula=nmn.cedula) WHERE nmn.codcon in ($codcon1) and nmp.frecuencia in ($frecuencia) and nmn.mes='$mes' and nmn.anio='$ano' group by nmn.cedula order by nmn.ficha";
	...
	while($fila=fetch_array($resultado))
	{
		...
		if($fila['monto']!=0)
		{
			...
			$this->Setceldas(array(0,0,0,0,0));
			$this->Setancho(array(5,5,5,5,5,5));
			$this->Row(array($fila['ficha'],$cedula,utf8_decode($fila['apenom']),number_format($fila['monto'],2,',','.'),number_format($fila_mov['monto'],2,',','.')));
	...
	$this->SetFont('Arial','',10);
	$this->Cell(137,5,'CANTIDAD DE PERSONAS: '.$totalwhile,0,'C');
	$this->Cell(25,5,number_format($totalPrest,2,',','.'),0,0,'R');
	$this->Cell(25,5,number_format($totalSaldo,2,',','.'),0,1,'R');
...
$pdf->movimientos($codcon1,$frecuencia,$mes,$ano);
//$pdf->finalizar();
$pdf->Output();
?>