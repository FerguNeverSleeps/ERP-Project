...
$_SESSION['bd']= 'brentwood_planillaexpress'; //Nombre de la base de datos EW_CONN_DB
...
function header(){
	...
	$date1=date("d/m/Y");
	$date2=date("h:m:s");
	if($var_imagen_izq!='' && file_exists("../imagenes/".$var_imagen_izq)){
...
function documentos($id)
{
	...
	$sql = "SELECT imagen_cedula, apenom, cedula FROM nompersonal WHERE personal_id=$id";
	...
	//busqueda de docuemntos en el expediente LM
	$sql = "SELECT * 
...
function datos($id){
	...
	$consulta="select * from nompersonal where personal_id=$id";
