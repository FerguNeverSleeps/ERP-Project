...
function header(){
	...
	$this->Cell(188,5,utf8_decode('VACACIONES DE ').$_SESSION['nomina'],0,1,'C');
	$this->Ln(10);
...
function datos($rc){
	...
	$this->Setancho(array(5,5,5,5));
        $this->Row(array(utf8_decode($rc['apenom']),$rc['cedula'],utf8_decode($rc['cargo']),$rc['ficha']));
	//fin
}
...
function imprimir_tabla($pdf){
	...
	while ($total>=$contar)
	{
		...
		$ficha=$fila['ficha'];
		$cedula=$fila['cedula'];
		$consulta="SELECT * FROM nom_progvacaciones WHERE ficha={$ficha} AND ceduda='{$cedula}' AND estado<>'Pagado' AND estado<>'Pagada' ORDER BY periodo DESC";
		...
		while(($fila1=fetch_array($filav))!=null){
			...
			{	
				...
				$this->Row(array($fila2['periodo'],fecha($fila2['fecha_venc']),number_format(($ddisfrute-$fila2['dpagob']), 0, ',', '.'),number_format($dbono, 0, ',', '.'),number_format($mb, 2, ',', '.'), (fecha($fila2['fechavac'])!='00/00/0000' ? fecha($fila2['fechavac']) : '-'), (fecha($fila2['fechareivac'])!='00/00/0000' ? fecha($fila2['fechareivac']) : '-') ));
