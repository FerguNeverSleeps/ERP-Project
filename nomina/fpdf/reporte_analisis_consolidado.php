...
function header(){
	...
	$this->SetFont('Arial','',10);
        $this->SetWidths(array(12,30,100,25,25));
	$this->SetAligns(array('C','R','L','R','R'));
        $this->Setceldas(array(0,0,0,0,0));
	$this->Setancho(array(5,5,5,5,5));
        $this->Row(array('COD.',utf8_decode('CÉDULA'),'APELLIDOS Y NOMBRES','REF','MONTO'));
...
function movimientos($codcon1,$nomina,$frecuencia,$mes,$ano)
{
	...
	$consulta="SELECT np.apenom, nmn.codnom, nmn.codcon, nmn.ficha, nmn.valor, sum(nmn.monto) as monto, nmn.cedula, nmn.tipnom "
                ...
                . "JOIN nompersonal np on (np.cedula=nmn.cedula) "
                . "WHERE nmn.codcon in ($codcon1) and nmp.frecuencia in ($frecuencia) and nmn.mes='$mes' and nmn.anio='$ano' "
                . "and nmn.tipnom in ($nomina) "
                . "group by nmn.cedula "
                . "order by nmn.cedula";
...
//			}
			...
			$this->SetWidths(array(12,30,100,25,25));
                        ...
                        $this->Setceldas(array(0,0,0,0,0));
			$this->Setancho(array(5,5,5,5,5));
			$this->Row(array($fila['ficha'],$cedula,utf8_decode($fila['apenom']),number_format($fila['valor'],2,'.',''),number_format($fila['monto'],2,'.','')));
			$total+=$fila['monto'];
		}
	}
	$this->Ln(2);
	...
	$this->SetFont('Arial','',10);
	$this->Cell(162,5,'CANTIDAD DE EMPLEADOS: '.$totalwhile,0,'C');
