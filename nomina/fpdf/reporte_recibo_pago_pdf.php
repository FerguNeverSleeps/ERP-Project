...
function personas($nomina_id,$codtp,$registro_id){
	...
	$row2 = mysqli_fetch_array($resultado);
        $this->Row(array('Ficha: '.$fila[ficha],utf8_decode('Nombre:').utf8_decode($fila[apenom]),utf8_decode('Cédula:').number_format($fila[cedula],0,'.','.')));
	...
	$this->Cell(40,6,number_format($sub_total_asig,2,',','.'),'T',0,'R');
	$this->Cell(31,6,number_format($sub_total_dedu,2,',','.'),'T',1,'R');
	//	$this->Ln(1);
	$this->Cell(120,5,'Neto a Depositar Bs.: ',0,0,'R');
	$this->Cell(100,5,number_format($sub_total_asig-$sub_total_dedu,2,',','.'),0,1,'C');
