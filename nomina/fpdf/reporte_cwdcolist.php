...
<!--<table width="800" border="1" align="center">-->
    ...
    $result_cuenta = mysqli_query($conectar, "SELECT * FROM cwcondco WHERE Numcom='$Numcom'");

    $Debito_total = 0;
    $Credito_total = 0;


    if (mysqli_num_rows($result_cuenta)) {
        while ($row = @mysqli_fetch_array($result_cuenta)) {
            $Cuenta_query = $row["Cuenta"];
            $result = mysqli_query($conectar, "SELECT * FROM cwconcue where Cuenta='$Cuenta_query' ORDER BY Cuenta DESC");
...
//	echo '<table width="800" border="0" align="center" cellpadding="2" cellspacing="2" class="">';
        ...
        $result_sum = mysqli_query($conectar,"SELECT Sum(Debito) as suma_Debito FROM cwcondco WHERE Numcom='$Numcom'"); //SUMA DE DEBITOS DE LISTA DE ASIENTOS
        $Debito_total_array = mysqli_fetch_array($result_sum);

        $result_sum = mysqli_query($conectar,"SELECT Sum(Credito) as suma_Credito FROM cwcondco WHERE Numcom='$Numcom'"); //SUMA DE DEBITOS DE LISTA DE ASIENTOS
        $Credito_total_array = mysqli_fetch_array($result_sum);
        ...
        $result_lineas = mysqli_query($conectar, "SELECT COUNT(*) FROM cwcondco WHERE Numcom='$Numcom'"); //SUMA DE DEBITOS DE LISTA DE ASIENTOS
        $Total_lineas_row = mysqli_fetch_row($result_lineas);

        $Total_lineas = $Total_lineas_row[0];

        $result_estado_contabilizado = mysqli_query($conectar, "SELECT * FROM cwconhco WHERE Numcom='$Numcom'"); //VALIDA SI ESTA CONTABILIZADO
        $Total_estado_contabilizado = mysqli_fetch_array($result_estado_contabilizado);
        $Estado_array_valida = $Total_estado_contabilizado["Estado"];
        if (($Estado_array_valida == 1) || ($Estado_array_valida == 3)) {
            if ($Total <> 0) {
                $descuadrado = mysqli_query($conectar, "UPDATE cwconhco SET Estado='3' WHERE Numcom='$Numcom'");  //DESCUADRADO	 
            } else if ($Total == 0) {
                $descuadrado = mysqli_query($conectar, "UPDATE cwconhco SET Estado='1' WHERE Numcom='$Numcom'");  //EN TRANSITO	 
            }
        }
    }
