...
function Header()
{
	...
	$this->Image($var_derecha,240,6,30,15);
	$this->Ln(10);
...
function detalle($gerencia,$nomina,$tipo,$desde,$hasta){
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula and ne.tipo_registro='Permisos o Ausencias' and fecha between '$desdea' and '$hastaa'   $cadenaselect  ORDER BY  np.codnivel4,np.tipnom,np.apenom";
...
// 			$this->Ln();
			...
			if($cod_trabajador!=$fila['ficha']){
				...
				if($cantidad_registros<=2){
					...
					$this->SetFont("Arial","I",11);
					$this->SetWidths(array(10,65,18,64,30,13,30,30));
					$this->SetAligns(array('C','C','C','C','C','C','C','C'));
					$this->Setceldas(array(1,1,1,1,1,1,1,1));
					$this->Setancho(array(5,5,5,5,5,5,5,5));
					$this->Row(array(utf8_decode('Cód'),'Nombre y Apellido',utf8_decode('Cédula'),'Cargo','Fecha Ingreso','Edad','Fecha  Egreso',utf8_decode('Adm. Pública')));
					$this->SetFont('Arial','I',8);
					$this->SetWidths(array(10,65,18,64,30,13,30,30));
				...
				if($fila['fecharetiro']!='0000-00-00'){
					$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),$fila['cedula'],utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos,date("d/m/Y",strtotime($fila['fecharetiro'])),$anti));
				}else{
					$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),$fila['cedula'],utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos,'-',$anti));
				}
				$cantidad_registros-=2;
				...
				$cantidad_registros-=1;
				$this->SetFont("Arial","I",11);
				$this->Row(array('Tipo',utf8_decode('Horas'),utf8_decode('Fecha Salida'),utf8_decode('Fecha Ret.'),utf8_decode('Descripción')));
				$this->SetFont("Arial","I",8);
				...
				if($fila['fecha_retorno']!='0000-00-00'){
					$this->Row(array($nombrepermisoper,(($fila['dias']*8)+($fila[horas])+($fila[minutos]/60)),date("d/m/Y",strtotime($fila['fecha_salida'])),date("d/m/Y",strtotime($fila['fecha_retorno'])),utf8_decode($fila['descripcion'])));
				}else{
					$this->Row(array($nombrepermisoper,(($fila['dias']*8)+($fila[horas])+($fila[minutos]/60)),date("d/m/Y",strtotime($fila['fecha_salida'])),'-',utf8_decode($fila['descripcion'])));
				}
				$this->Setancho(array(5,5,5,5,5));
				$cantidad_registros-=1;
