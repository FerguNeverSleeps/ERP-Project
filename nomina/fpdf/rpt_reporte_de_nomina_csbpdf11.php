...
function personas($nomina_id,$codt,$pdf){
	...
	$conexion=conexion();
	$query="SELECT per.codnivel1, per.codnivel2, per.ficha, per.apellidos, per.nombres, car.des_car, nmh.sueldo, nmh.cedula
	FROM nompersonal AS per
	LEFT JOIN nom_movimientos_nomina AS nom ON ( per.ficha = nom.ficha ) 
	LEFT JOIN nomcargos AS car ON per.codcargo = car.cod_car
	JOIN nom_movimientos_historico nmh ON ( nmh.ficha = per.ficha
	AND nmh.cedula = per.cedula ) 
	WHERE nmh.codnom =  '$nomina_id'
	AND nmh.tipnom =  '$codt'
	GROUP BY per.ficha
	ORDER BY per.codnivel2, per.nombres ";
...
// 			}
			...
			where pe.ficha = '".$sficha."' and mn.codnom = '".$nomina_id."' and mn.tipnom = '".$codt."' and mn.tipcon<>'P' order by mn.tipcon,c.codcon";	
			//exit(0); 
			...
			if(num_rows($result1)+9>$CANTIDAD){
				$this->Ln(300);
				$this->SetFont('Arial','',9);
				/*$consulta_n4="SELECT descrip FROM nomnivel3 WHERE codorg='".$fila['codnivel4']."'";
				$resultado_n4=query($consulta_n4,$conexion);
			...
			$this->Row(array(utf8_decode('Cédula: '.number_format($fila['cedula'],0,',','.')),'Ficha: '.$fila['ficha']."            Sueldo Basico:  ".number_format($fila['sueldo'],2,',','.')));
			...
			while ($row = fetch_array($result1))
			{
				...
				$this->Row(array($row[codcon].'  '.utf8_decode($row[descrip]),$row[valor],$valor1,$valor2));
...
function firmas($total_asig,$total_dedu,$cantidad){
	...
	$consultaa="SELECT ficha FROM nom_movimientos_historico WHERE codnom = '".$_GET['nomina_id']."' and tipnom =".$_SESSION['codigo_nomina']." ";
	...
	$this->Cell(188,5,'Total General: '.number_format($total_asig,2,',','.').'       '.number_format($total_dedu,2,',','.'),0,1,'R');
	...
	$this->Setancho(array(5,5,5));
        $this->Row(array('COORDINADOR DE RRHH','DIR. ADMIN. Y PTTO.','PRESIDENTE'));
