...
function Cuentas($nomina_id,$codtip){
	...
	$this->Setancho(array(5,5,5,5,5));
        $this->Row(array(utf8_decode('Código y Descripción del Concepto'),utf8_decode('Cuenta Contable'),'Asignaciones','Deducciones','Patronales'));
	...
	while($totalwhile>=$contar)
	{
		...
		$consulta8 = "SELECT ctacon FROM nomconceptos WHERE codcon = '".$fila['codcon']."' ";	
		...
		if($fetch7['tipcon']=='A')
		{
			...
			$this->Row(array($fila[codcon].' - '. utf8_decode($fetch7[descrip]),$fetch8[ctacon],number_format($fetch6['suma'],2,',','.'),'',''));
