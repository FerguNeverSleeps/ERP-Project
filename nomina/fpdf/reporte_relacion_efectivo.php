...
function header(){
	...
	if($this->nomina==4){
		$this->Cell(188,8,'EFECTIVO',0,1,'C');
	}else{
     		$this->Cell(188,8,'PAGOS COLABORADORES EN EFECTIVO',0,1,'C');
	}
	
	$this->Cell(188,5,'LAPSO DEL '.$fecha_ini.' AL '.$fecha_fin,0,1,'C');
	if($this->nomina==3){
		$this->Cell(188,8,''.$_SESSION['nomina'].' SEMANAL',0,1,'C');
	}else{
...
function Cuentas($nomina){
	...
	$this->SetFont('Arial','',12);
        $this->SetWidths(array(20,25,80,43,25));
	$this->SetAligns(array('C','C','C','C','C'));
        $this->Setceldas(array('TB','TB','TB','TB','TB'));
	$this->Setancho(array(5,5,5,5,5));
        $this->Row(array('# FICHA',utf8_decode('CÉDULA'),'APELLIDOS Y NOMBRES','MONTO'));
	...
	while($totalwhile>=$contar)
	{
		...
		$consulta="select apenom from nompersonal where ficha='".$fila['ficha']."' and cedula='".$fila[cedula]."'";
		$conexion=conexion();
		...
		$this->Row(array($fila['ficha'],$fila['cedula'],$fila_personal['apenom'],number_format($fila['neto'],2,'.',',')));
