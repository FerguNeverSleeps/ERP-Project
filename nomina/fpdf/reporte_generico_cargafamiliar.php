...
function Header()
{
	...
	$this->Cell(188,6,'CARGA FAMILIAR',0,1,'C');
// 	$this->SetFont('Arial','B',12);
//      	$this->Ln(30);
	...
	while($fila=fetch_array($query)){
		...
		if($codper!=$cod_persona){
			...
			if($anos>=$edad1 && $anos<=$edad2){
				...
				$cod_persona=$codper;
				$this->SetFont("Arial","I",10);
				$this->SetWidths(array(65,20,20,25,20,40));
				$this->SetAligns(array('C','C','C','C','C','C'));
				$this->Setceldas(array(1,1,1,1,1,1));
				$this->Setancho(array(5,5,5,5,5,5));
				$this->Row(array('Nombre y Apellido','Edad','Sexo','Talla Franela','Talla Mono','Parentesco'));
			}
		}
		...
		if($anos>=$edad1 && $anos<=$edad2){
			...
			$this->SetFont('Arial','I',8);
			$this->SetWidths(array(65,20,20,25,20,40));
			$this->SetAligns(array('J','C','C','C','C','C'));
			$this->Setceldas(array(1,1,1,1,1,1));
			//$this->Setancho(array(5,5,5,5));
			$this->Row(array($fila['apellido'].', '.$fila['nombre'],$anos,$fila['sexo'],$fila['tallafranela'],$fila['tallamono'],utf8_decode($rp['descrip'])));
			$cantidad_registros-=1;
		}
