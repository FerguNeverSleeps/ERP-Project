...
{
	public $fechainicio, $fechafinal, $fecharegistro, $quincena, $mes, $año, $id, $numero;
	//Page header
...
//		$this->Cell(0, 20, 'HOJA N/TOTAL DE HOJAS '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
	...
	public function CuerpoTablaHtml($reg,$fecha_inicio,$fecha_fin,$fecha_registro)
	{
		...
		$sql1              = " 
			...
			AND A.quincena = ".$this->quincena." AND A.mes =".$this->mes." AND A.ano=".$this->ano." AND A.num_decreto ='".$this->numero."'";
	    ...
	    while($fila=fetch_array($res1))
	    {
			...
			$this->Cell($w[$ixx++], $altura, $fila['nombres']." ".$fila['apellido_paterno']." ".$fila['apellido_materno']." ".$fila['apellido_casada'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
