...
$fechasolicitud = date('d-m-Y', strtotime($persona->fecha_registro)) ; // resta 6 mes
...
if ($persona->dias != 0) 
{
	$cant_dias = $persona->dias." dia(s)";
} 
elseif ($persona->horas != 0) 
{
	$cant_horas = $persona->horas." dia(s)";
}
elseif ($persona->minutos != 0) 
{
	$cant_minutos = $persona->minutos." dia(s)";
}
$tiempo = $cant_dias." ".$cant_minutos." ".$cant_horas;
...
$hora_inicio    = date('H:i:s', strtotime($persona->fecha_inicio));
$hora_fin       = date('H:i:s', strtotime($persona->fecha_fin));
...
$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('plantillas/tiempo_compensatorio.docx');

$templateProcessor->setValue('nombre', htmlspecialchars($nombre));
$templateProcessor->setValue('cedula', htmlspecialchars($cedula));
$templateProcessor->setValue('puesto', htmlspecialchars($cargo));
...
$templateProcessor->setValue('tiempo', htmlspecialchars($tiempo));
$templateProcessor->setValue('fecha', htmlspecialchars($fechasolicitud));
...
$templateProcessor->setValue('director', htmlspecialchars($director));
$templateProcessor->setValue('unidad', htmlspecialchars($unidad_ad));
...
$filename = 'Solicitud Tiempo Compensatorio '.$idcarajo.'.docx';
