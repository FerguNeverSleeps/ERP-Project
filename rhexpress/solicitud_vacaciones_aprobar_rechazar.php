...
if ($con == 1)
{
	...
	if ($aprobado == 1) 
	{
		...
		$update = "UPDATE solicitudes_casos
			...
			//insertando en dias de incapacidad
			...
			$conexion->query($q);
			//insertando en expediente
			$qexp ="INSERT INTO expediente (cedula, descripcion, fecha, usuario, horas,dias, minutos, tipo,subtipo,nivel_actual)
			VALUES ('{$cedula}', '{$observacion_jefe}', '{$fecha_aprobacion}', '$cedula', '$hora','{$dias}', '$minutos','11', '61','{$nivel1['numero_departamento']}')";
			$conexion->query($qexp);
			...
			$tit='SOLICITUD ';			
			$mail->Subject = $tit;
			$asunto="SOLICITUD Nº: ".$id."  APROBADA DEL COLABORADOR ".$_SESSION['nombre_rhexpress']." #".$_SESSION['ficha_rhexpress'];
			$mail->Body = $asunto;
			$mail->IsHTML(true);
						...
						<div class="portlet-body">
							...
							<div class="row">
								<div class="form-group">
									...
									<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7">
										<textarea cols="5" rows="5" name="observacion"  class="form-control" type="text" id="observacion" value="<?php echo $caso['observacion']; ?>" disabled><?php echo $caso['observacion']; ?></textarea>
									</div>
								</div>
							</div>
						</div>
