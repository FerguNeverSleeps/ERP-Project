...
//-------------------------------------------------
  ...
  $sql_jefe    = "SELECT IdJefe,uid_subjefe,IdDepartamento,uid_aprueba FROM departamento where IdJefe = '{$cedula}' OR uid_subjefe = '{$cedula}' OR uid_aprueba= '{$cedula}'";
...
if ($esjefe) 
{
    ...
    if ($jefes['IdJefe'] == $cedula) 
    {
        // "es jefe departamento";
            ...
            WHERE se.id_solicitudes_estatus in (2,4) AND dep.IdDepartamento IN ($valores_departamentos)
    ...
    elseif ($jefes['uid_subjefe'] == $cedula)
    {
        // "es director";
            ...
            WHERE se.id_solicitudes_estatus in (2,4)
            ORDER BY sc.id_solicitudes_casos  DESC";
    }

    elseif ($jefes['uid_aprueba'] == $cedula)
    {
        // "RRHH";
        $sql = "SELECT * 
            FROM solicitudes_casos as sc 
...
else
{
    ...
    LEFT JOIN nompersonal as np on (sc.cedula = np.cedula)
    LEFT JOIN departamento as dep on (dep.IdDepartamento = np.IdDepartamento)
    WHERE se.id_solicitudes_estatus in (2,4) AND np.cedula = '{$cedula}'
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue bold uppercase">CASOS APROBADOS</span>
                </div>
                <!-- <div class="actions">
            ...
            <div class="portlet-body">
                <div class="table-scrollable">
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        $carpeta = 'reportes';
                         switch ($fila['id_tipo_caso']) {
                            case '7':    
                                $url     = "solicitud_actualizacion_anual_bienes_aprobar_rechazar.php";
                                $archivo = "/pdf/solicitud_actualizacion_anual_bienes_pdf.php";
                                break;    
                            case '6':    
                                $url     = "solicitud_prestamos_y_compras_aprobar_rechazar.php";
                                $archivo = "/pdf/solicitud_prestamos_compras_creditos_pdf.php";
                                break;
                            case '5':    
                                $url     = "solicitud_reclamo_pago_aprobar_rechazar.php";
                                $archivo = "/pdf/solicitud_reclamo_pago_pdf.php";
                                break;
                            case '4':
                                $url     = "solicitud_horas_extraordinarias_aprobar_rechazar.php";
                                $archivo = "reporte_horas_extraordinarias.php";
                                break;
                                
                            case '3' :
                                $url     = "solicitud_tiempo_compensatorio_aprobar_rechazar.php";
                                $archivo = "reporte_tiempo_compensatorio.php";
                                break;
                             case '1' :
                                $url     = "solicitud_permisos_aprobar_rechazar.php";
                                $archivo = "/word/reporte_permisos.php";
                                break;
                            case '2' :
                                $url     = "solicitud_vacaciones_aprobar_rechazar.php";
                                $archivo = "/word/reporte_vacaciones.php";
                                break;
                        ...
                        ?>
                            <tr>
                                ...
                                <?php if($fila['id_tipo_caso']=="2" || $fila['id_tipo_caso']=="1")
                                    {?>

                                        <td>&nbsp;</td>
                                        <?php }else{
                                            ?>
                                            <td><a href="<?= $carpeta.$archivo ?>?id=<?= $fila['id_solicitudes_casos'] ?>&tipo=<?= $fila['id_tipo_caso'] ?>" target="_NEW"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a></td>
                                    
                                <?}
                                ?>
                                <!--  -->
                                <?php if($fila['id_solicitudes_estatus'] == '2' AND $esRRHH){ ?>
                                    <!-- <td><a href="<?php echo $url;?>?id=<?= $fila['id_solicitudes_casos'] ?>"><i class="fa fa-edit" aria-hidden="true"></i></a></td> -->
                                    <td>&nbsp;</td>
                                <? 
                                }
                                else
                                ...
                                if ($esjefe) {
                                    ?>
                                    <td><a href="<?php echo $anular; ?>?id=<?= $fila['id_solicitudes_casos'] ?>"><i class="fa fa-ban" aria-hidden="true"></i></a></td>
                                  <?php  
                                } else {
                                    ?>
