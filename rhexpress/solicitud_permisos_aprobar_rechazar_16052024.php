	...
	if ($aprobado == 1) 
	{
		$fecha_aprobacion  = (isset($_REQUEST['fecha_aprobacion'])) ? date('Y-m-d') : date('Y-m-d') ;	
		 ...
		 $update = "UPDATE solicitudes_casos
			...
			// VALUES ('".$SESSION['ficha']."', '5', '{$fecha_aprobacion}', '{$tiempo_nuevo}', '{$dias}',  '{$hora}','{$minutos}', '{$dias}',  '{$hora}','{$minutos}', '{$observacion_rrhh}','{$cedula}')";
			//echo $q,"<br>";
			$conexion->query($q);
			...
			if($resultado_exp)
			{
				...
				if($archivo_caso != "")
				{
					...
					$consulta = "INSERT INTO expediente_adjunto 
									...
									VALUES 
									('', '{$nombre_archivo}', '{$caso['observacion']}', '{$direccion_archivo}', '1','{$fecha_aprobacion}', '{$ultimo_id}');";
