...
FROM solicitudes_campos WHERE solicitudes_tipo_id = '5'";
$cantidad_campos = $conexion->query($sql);

$sql="SELECT correlativo FROM solicitudes_tipos WHERE id_solicitudes_tipos='5'";
...
if ($con == 1)
{
	...
	if (isset($_POST['fecha'])) {
		...
		//insertamos en la tabla solicitudes, que tiene los valores "generales"
		...
		VALUES ('','".$_SESSION['cedula_rhexpress']."','5','".$persona['IdDepartamento']."',NOW(),'{$fecha_ini}','{$fecha_fini}','{$observacion}','$c','1','{$dias}','{$dias}','{$horas}','{$minutos}');";
		...
		if($insertado){
			...
			// $tit='SOLICITUD ';
			// $mail->Subject = $tit;
			// $asunto="SOLICITUD DE RECLAMO DE PAGO DEL COLABORADOR ".$_SESSION['nombre_rhexpress'].", POR FAVOR DIRIGIRSE BANDEJA DE ENTRADA PARA APROBAR O RECHAZAR.";
			// $mail->Body = $asunto;
			// $mail->IsHTML(true);
			...
			$mail->ClearAttachments();
			 ...
			 echo "<script>alert('SOLICITUD DE RECLAMO GUARDADA EXITOSAMENTE');location.href='rhexpress_menu.php';</script>";
		}
		else
		{
					...
					<?php
						while($campos=mysqli_fetch_array($result)) {
							if ($campos['tipo_campo']=='text') {?>							
								...
								<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3">
									<input name="<?php echo $campos['id_solicitudes_campos']?>"  class="form-control input-circle" style="margin-top:12px;" type="text" id="<?php echo $campos['id_solicitudes_campos']?>" required><p></p>
								</div>
							<?php
							}
							if ($campos['tipo_campo']=='number') {?>
								<label class="text-right col-xs-offset-2 col-xs-3 col-sm-offset-2 col-sm-3 col-md-offset-2 col-md-3 col-lg-offset-2 col-lg-3 control-label"><?php echo $campos['nombre']?></label>
								<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7">
									<input name="<?php echo $campos['id_solicitudes_campos']?>"  class="form-control input-circle" type="number" id="<?php echo $campos['id_solicitudes_campos']?>" required><p></p>
								</div>
							<?php
							}
							if ($campos['tipo_campo']=='textarea') {?>
								<label class="text-right col-xs-offset-2 col-xs-3 col-sm-offset-2 col-sm-3 col-md-offset-2 col-md-3 col-lg-offset-2 col-lg-3 control-label"><?php echo $campos['nombre']?></label>
								<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7">
									<textarea cols="5" rows="5" name="<?php echo $campos['id_solicitudes_campos']?>"  class="form-control input-circle" type="text" id="<?php echo $campos['id_solicitudes_campos']?>" value="<?php ?>" required></textarea><p></p>
								</div>
								<?php
							}
							...
							<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3" style="margin-top: 10px;">
								<select name="<?php echo $campos['id_solicitudes_campos']?>" class="form-control input-circle"  id="<?php echo $campos['id_solicitudes_campos']?>" required><p></p>
                                                <option value="N/A">Seleccione opcion:</option>
...
<?php
 ...
 <option value="1"><?php echo $valores_select[1]; ?></option>
                                                <option value="2"><?php echo isset($valores_select[2]) ? $valores_select[2] : '' ?></option>
                                                <option value="3"><?php echo isset($valores_select[3]) ? $valores_select[3] : '' ?></option>