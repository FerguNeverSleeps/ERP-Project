...
$posiciones = array();
$pos = $conn->fetchAll('SELECT nomposicion_id, descripcion_posicion FROM nomposicion');
...
if ($action == 'edit') {
    $persona2 = $conn->fetchAssoc('SELECT * FROM mov_ajuste WHERE id_mov_contraloria = ?', array($id));
...
if ( !empty($_POST['guardar']) ){
    ...
    $log = array(
            ...
            'fecha_hora' => $flog,
            'modulo' => 'Ajustes Datos Contraloria',
            'url' => 'ajustes.php',
            'accion' => $accion,
            'valor' => '',
    ...
    $data2 = array(
        ...
        'ajuste_dias' => $_POST['ajuste_dias'],
        'ajuste_monto' => $_POST['ajuste_monto'],
    );

    if ($action == 'add') {
        $conn->insert('mov_ajuste', $data2);
    } else {
        unset($data2['id_mov_contraloria']);
        $conn->update('mov_ajuste', $data2,  array('id_mov_contraloria' => $id));
    }
?>
<script>
...
<div class="container" style="margin-top: 50px;">
    ...
    <form name="form1" method="post" class="form-horizontal">
        ...
        <div class="form-group">
            <label for="" class="col-sm-2 control-label">Dias a descontar:</label>
            <div class="col-sm-2">
                <input type="text" name="ajuste_dias" value="<?= $persona['ajuste_dias'] ?>" class="form-control">
            </div>
            <label for="" class="col-sm-3 control-label">Monto a descontar:</label>
            <div class="col-sm-2">
                <input type="text" name="ajuste_monto" value="<?= $persona['ajuste_monto'] ?>" class="form-control">
            </div>
        </div>
