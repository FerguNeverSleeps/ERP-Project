...
while($row = mysqli_fetch_array($result2))
{
    ...
    for( $i = 1;  $i<=12 ; $i++)
    {
        ...
        SUM( IF(nmn.tipcon = 'A', ROUND(nmn.monto,2),0) ) AS asignaciones,
        SUM( IF(nmn.tipcon = 'D', ROUND(nmn.monto,2),0) ) AS deducciones,
        ...
        WHERE nmn.mes = '{$i}' and nmn.anio = '{$anio}' and nmn.codnivel1 = '{$nivel1}' and nmn.tipcon in ('A','D')
        and nmn.codcon in ('100')
        GROUP BY nmn.codnivel1";
        $result_row = $db->query($sql_row);
        $celda = $result_row->fetch_array();
        $monto = ( $celda['monto']!= "") ?  $celda['monto'] : 0 ;
        $asignaciones = ( $celda['asignaciones']!= "") ?  $celda['asignaciones'] : 0 ;
        $deducciones = ( $celda['deducciones']!= "") ?  $celda['deducciones'] : 0 ;
        $salario_bruto = ( $celda['salario_bruto']!= "") ?  $celda['salario_bruto'] : 0 ;
        $monto = $asignaciones-$deducciones;
...
header('Content-Disposition: attachment;filename="diferencia_neto_bruto.xlsx"');
