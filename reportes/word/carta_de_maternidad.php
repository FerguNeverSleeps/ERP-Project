...
date_default_timezone_set('America/Panama');
	...
	function formato_fecha($fecha,$formato)
    {
	    ...
	    switch ($formato)
	    {
	        case 1:
	            ...
	            break;
	        case 2:
	            $f = "Panama, ".$dia." de ".$meses[$mes-1]." de ".$anio;
	            break;
	        default:
	            break;
	    }
	...
	$sql = "SELECT a.apenom, a.cedula as ced_trabajador,h.fecha as fecha_c,h.fecha_inicio as inic,h.fecha_fin as fini,a.sexo,a.sueldopro,a.estado_civil, a.seguro_social, a.lugarnac, a.fecnac,
	 a.suesal,a.num_decreto,a.fecha_decreto,a.condicion,a.nomposicion_id, a.tipo_empleado, a.codcargo,
	 b.des_car,d.nombre AS provincia,e.descrip as donde,a.inicio_periodo as ini,a.fin_periodo as fin
	FROM nompersonal AS a 
    INNER JOIN expediente as h ON a.cedula=h.cedula
	...
	WHERE a.ficha='$ficha' and h.tipo=17 and h.subtipo=52";
	$res = $db->query($sql);	
	...
	// $sql6="SELECT e.apellido as apellido_hermana,e.nombre as nombre_hermana,f.descrip as parentesco_hermana
	// FROM nompersonal AS a 
	// LEFT JOIN nomfamiliares as e ON e.ficha=a.ficha		
	// LEFT JOIN nomparentescos AS f ON f.codorg=e.codpar
	...
	// }
	// $nombre_hijo   = $familiahijo->nombre_hijo;
	// $apellido_hijo   = $familiahijo->apellido_hijo;
	// $parentesco_hijo   = $familiahijo->parentesco_hijo;
	...
	$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('plantillas/carta_de_maternidad.docx');
    ...
    $templateProcessor->setValue('fecha_c', htmlspecialchars($fecha_c));
    $templateProcessor->setValue('inic', htmlspecialchars($inic));
    $templateProcessor->setValue('fini', htmlspecialchars($fini));
	...
	$templateProcessor->setValue('ger_rrhh', utf8_decode($ger_rrhh));
	$templateProcessor->setValue('ced_rrhh', htmlspecialchars($ced_rrhh));
	$templateProcessor->setValue('numero_patronal', htmlspecialchars($numero_patronal));
	...
	$filename = 'Carta de maternidad'.$ficha.'.docx';
