...
while($fila = $res_max->fetch_assoc()){
    ...
    while($row = $res->fetch_assoc()) {
        ...
        //$MONTO_VAC=$REF_VAC*$VAC11;
        //$MONTO_VAC = $pdf->calcularvacacionesproporcionales($FICHA,$FECHAINGRESO, $FECHANOMINA,$TIPNOM);
        ...
        else 
        {
            ...
            //$CESANTIA = $pdf->calcularcesantia($FICHA,$FECHAINGRESO,$FECHANOMINA,$TIPNOM,$ANIOFECHA);
            $CESANTIA = $MONTO_VAC*0.06;
            $prima = 0;
            $indem = 0;
        }

       // $prima = ($acum_liq+$row['sum_vacac']+$MONTO_XIII)*$tasa_prima;

        $total_salario       += $row_acum['sum_sal'];
        $total_prima         += $prima;
        $total_indem         += $indem;
        //$total_comision      += $row_acum['sum_comision'];
        //$total_sobretiempo   += $row_acum['sum_sobretiempo'];
        $total_bono          += $BONO_ASISTENCIA;
        //$total_altura        += $row_acum['sum_altura'];
        //$total_otros         += $row_acum['sum_otros'];
        $total_vacac_gtorep  += $VACXIII;
        $total_vacac         += $MONTO_VAC;
        $total_xiii          += $MONTO_XIII;
        $total_xiii_gtorep   += $GRXIII;
        //$total_util          += $row_acum['sum_util'];
       // $total_liquida       += $row_acum['sum_liquida'];
        //$total_licencia      += $row_acum['sum_licencia'];
        $total_gtorep        += $row_acum['sum_gtorep'];
        $total_otros_ing     += $CESANTIA;
        //$total_adelanto      += $row_acum['sum_adelanto'];
        ...
        /*$db           = new Database($_SESSION['bd']);
        $sql_empleado = "SELECT FROM nompersonal WHERE ficha = '{$row_acum[ficha]}'";
        $res_emp     = $db->query($sql_empleado);
