...
//--------------------------------------------------------------------------------------------------------------------------------------
//consulta de datos
$sql = "SELECT a.nomposicion_id,a.apellidos,a.nombres,a.cedula,a.fecnac, a.fecing,SUM(a.suesal) as saltotal,a.estado,a.num_decreto,b.descrip,b.codnom,c.des_car,COUNT(a.personal_id) as todos,d.Descripcion as descripcion FROM nompersonal AS a,nom_nominas_pago AS b,nomcargos AS c, departamento as d WHERE a.tipnom = b.codnom AND a.codcargo = c.cod_car AND d.IdDepartamento=a.IdDepartamento";
$res1 = $db->query($sql);
//------------------------------------------------------------------------------------------------------------------------------------
...
$MiPDF->AddPage('L');
    ...
    while ($data = fetch_array($res1))
    {
        ...
        $MiPDF->Cell(108,8,$data['saltotal'], $border='R', $ln, $align='C', $fill,$link,$stretch,$ignore_min_height,$calign,$valign);
        $MiPDF->Cell(108,8,$data['nombres'], $border='R', 1, $align='C', $fill,$link,$stretch,$ignore_min_height,$calign,$valign);
    ...
    $MiPDF->Output('resumen_planilla_siclar.pdf','I');
?>