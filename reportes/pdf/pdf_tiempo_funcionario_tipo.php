...
//--------------------------------------------------------------------------------------------------------------------------------------
//consulta de datos
$sql = "SELECT b.apenom as nombre, a.tipo_justificacion as justificativo, SUM(a.tiempo) as tiempo,SUM(a.dias) as dias,SUM(a.horas) as horas,SUM(a.minutos) as minutos FROM dias_incapacidad as a,nompersonal as b, departamento as c WHERE a.cod_user=b.personal_id AND a.tipo_justificacion='$justificacion' AND c.IdDepartamento='$departamento' AND b.IdDepartamento=c.IdDepartamento GROUP BY b.apenom";
$res = $db->query($sql);
...
$MiPDF->AddPage('L');
    ...
    $MiPDF->Multicell(205,5,'TIEMPO POR TIPO DE FUNCIONARIOS',0,$alineacion='C',$fondo=false,$salto_de_linea=0,$x='40',$y='20',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=20,$alineacion_vertical='T', $fitcell=false);
    ...
    $MiPDF->Output('tiempo_funcionario_tipo.pdf','I');
?>