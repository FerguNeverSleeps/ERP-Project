...
$fsql=date_format($datef,'Y-m-d');
$sql = "SELECT a.apenom,a.cedula,a.seguro_social,a.tipnom,a.suesal,b.des_car,c.nomposicion_id,c.partida,d.descrip FROM nompersonal AS a,nomcargos AS b,nomposicion as c, nomnivel1 as d WHERE a.codcargo = b.cod_car and a.nomposicion_id = c.nomposicion_id and a.codnivel1=d.codorg and a.fecha_decreto='$fsql' and a.num_decreto = '$nd' and a.estado <> 'Egresado' GROUP BY d.descrip ORDER BY d.descrip";
$res1 = $db->query($sql);

$sql2 = "SELECT dir_emp FROM nomempresa";
$res2 = $db->query($sql2);
$data2 = fetch_array($res2);
$ministro=strtoupper($data2['dir_emp']);

$sql3 = "SELECT a.num_decreto FROM nompersonal AS a WHERE a.fecha_decreto='$fecha' LIMIT 1";
$res3 = $db->query($sql3);
...
$MiPDF->Multicell(210,5,'Decreto Numero '.$nd.' de '.date('Y'),0,$alineacion='C',$fondo=false,$salto_de_linea=0,$x='0',$y='20',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=20,$alineacion_vertical='T', $fitcell=false);
...
$MiPDF->Multicell(210,5,'El Presidente de la Republica',0,$alineacion='C',$fondo=false,$salto_de_linea=0,$x='0',$y='65',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=20,$alineacion_vertical='T', $fitcell=false);

$MiPDF->SetFont('times','', 12);

$MiPDF->Multicell(210,5,'en uso de sus facultades constitucionales y legales',0,$alineacion='C',$fondo=false,$salto_de_linea=1,$x='0',$y='70',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=20,$alineacion_vertical='T', $fitcell=false);

$MiPDF->Multicell(210,5,'',0,$alineacion='C',$fondo=false,$salto_de_linea=1,$x='0',$y='75',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=20,$alineacion_vertical='T', $fitcell=false);
...
$MiPDF->Multicell(120,25,'',0,$alineacion='C',$fondo=false,$salto_de_linea=1,$x='0',$y='',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=50,$alineacion_vertical='T', $fitcell=false);
$MiPDF->SetFont('times','B', 12);
$MiPDF->Multicell(180,5,$ministro,0,$alineacion='C',$fondo=false,$salto_de_linea=1,$x='',$y='',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=50,$alineacion_vertical='T', $fitcell=false);
$MiPDF->SetFont('times','', 12);
$MiPDF->Multicell(180,5,'Ministro de Salud',0,$alineacion='C',$fondo=false,$salto_de_linea=1,$x='',$y='',$reseth=true,$ajuste_horizontal=0,$ishtml=false,$autopadding=true,$maxh=50,$alineacion_vertical='T', $fitcell=false);
