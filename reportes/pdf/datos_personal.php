...
function datos($ficha,$cedula){
	...
	if($rc['foto']!='fotos/' && $rc['foto']!=''){
		if(file_exists("../../nomina/paginas/".$rc['foto'])){
			$this->Image("../../nomina/paginas/".$rc['foto'],160,25,33);
		}else{
			$this->Image("../../nomina/paginas/fotos/silueta.gif",160,30,43);
			//$this->Image("../paginas/fotos/girl2.jpg",160,30,43);
		}
	}else{
			//$this->Image("../paginas/fotos/silueta.gif" ,160,30,43);
			$this->Image('../../includes/assets/img/profile/profile_black.jpg',160,30,43);
	...
	$this->Setancho(array(5,5));
        $this->Row(array(utf8_decode($rc['apenom']),$rc['cedula']));
	...
	$this->Row(array(utf8_decode('CARGO'),'COLAB. #','POSICION'));
	$this->SetFont('Arial','I',10);
	$this->SetAligns(array('L','C'));
	$this->Setceldas(array(0,0));
	$this->Row(array(utf8_decode($cargo),$rc['ficha'],$rc['nomposicion_id']));
	...
	$edad=antiguedad($rc['fecnac'],date('Y-m-d'),"A");
	$this->Row(array(
				antiguedad($rc['fecing'],date("Y-m-d"),"A").utf8_decode(" Años"),
				$rc['antiguedadap'].utf8_decode(" Años"),
				($rc['antiguedadap']+antiguedad($rc['fecing'],date("Y-m-d"),"A")).utf8_decode(" Años"))
			   );
